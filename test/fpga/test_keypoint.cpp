...
static const char* g_inception = "../models/keypoint";
static const char* image = "../input/input.jpeg";
...
int main() {
  ...
  if (paddle_mobile.Load(model, params, true)) {
    ...
    Tensor input_tensor;
    SetupTensor<float>(&input_tensor, {1, 3, 256, 256}, static_cast<float>(1),
                       static_cast<float>(1));
    float* data = input_tensor.mutable_data<float>({1, 3, 256, 256});
