...
void test(int argc, char *argv[], bool fuse) {
    ...
    if (paddle_mobile.Load("./checked_model/model", "./checked_model/params", fuse, false, 1, true)) {
      auto time2 = time();
      std::cout << "auto-test" << tag << " load-time-cost :" << time_diff(time1, time1) << "ms" << std::endl;
      ...
      auto time4 = time();
      std::cout << "auto-test" << tag << " predict-time-cost " << time_diff(time3, time4) / 50 << "ms"
                << std::endl;
      ...
      for (auto var_name : var_names) {
          ...
          std::cout << "auto-test" << tag << " var " << var_name << sample << std::endl;
      }
      std::cout << std::endl;
    }
}
