...
#include <vector>
#include "driver/qualcomm_qnn/converter/htp/gpc.h"
...
void Array2Poly(const T*& box,  // NOLINT
  ...
  size_t pts_num = box_size / 2;
  poly.num_contours = 1;
  poly.hole = static_cast<int*>(malloc(sizeof(int)));
  poly.hole[0] = 0;
  poly.contour = static_cast<gpc_vertex_list*>(malloc(sizeof(gpc_vertex_list)));
  poly.contour->num_vertices = pts_num;
  poly.contour->vertex =
      static_cast<gpc_vertex*>(malloc(sizeof(gpc_vertex) * pts_num));
  for (size_t i = 0; i < pts_num; ++i) {
