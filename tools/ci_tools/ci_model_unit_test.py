...
def ratio_diff(arrA, arrB):
    ...
    rel_arr[idxNoneZeros] = np.absolute(np.absolute(arrA[idxNoneZeros]) / np.absolute(arrB[idxNoneZeros]))
        ...
        for acc in accuracy:
            ...
            for root,dirs,files in os.walk(lite_output_path_arm_acc):
                for output_txt in files:         
                    ...
                    paddle_txt = paddle_output_path + "/" + ((output_txt.split("."))[0]) + "/" + output_txt                 
                    lite_results=list()
                    ...
                    with open(lite_txt) as flite_result:
                        for data in flite_result.readlines():
                            try:
                                ...
                                print(" output data is not number, maybe inf or nan")
                    ...
                    if len(lite_results) != len(paddle_results):
                        print(arm , acc , ((output_txt.split("."))[0]) , " Outshape Diff")
                        return True
                    a = abs_diff(lite_results, paddle_results)
                    r = ratio_diff(lite_results - paddle_results, paddle_results)
                    for i in range(len(a)):
                        if (a[i] > aerr and r[i] > rerr) or (a[i] > aerr and r[i] == 0):
                            print(arm, acc, ((output_txt.split("."))[0]), " Output Data Diff , Diff element:", i , " lite result:" , lite_results[i], " paddle result:", paddle_results[i], " abolute error:", a[i], " relative error:", r[i])
                            return True
