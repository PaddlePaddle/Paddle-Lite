...
def feed_ones_shape(block, feed_target_names, input_shape):
    """ 
    """ 
    feed_dict = dict()
    def set_batch_size(shape, input_shape):
        return input_shape
    def fill_ones(var_name, shape):
        var = block.var(var_name)
        np_shape = set_batch_size(list(var.shape), shape)
        print('shape: ', np_shape)
        var_np = {
            core.VarDesc.VarType.BOOL: np.bool_,
            core.VarDesc.VarType.INT32: np.int32,
    ...
    for feed_target_name in feed_target_names:
        feed_dict[feed_target_name] = fill_ones(feed_target_name, input_shape)
...
def fluid_inference_test(model_path):
    ...
    with fluid.scope_guard(scope):
        ...
        if GLB_txt_path is None or GLB_txt_path == "":
            if GLB_input_shape is not None:
                feed_list = feed_ones_shape(global_block, feed_target_names, GLB_input_shape)
            else:
                feed_list = feed_ones(global_block, feed_target_names, 1)
        else:
            feed_list = feed_txt(global_block, feed_target_names, 1)
            #feed_list = feed_txt(global_block, feed_target_names, GLB_input_shape)
...
if __name__ == "__main__":
    ...
    if args.channel is not None:
        GLB_channel = args.channel
    if args.height is not None:
        GLB_height = args.height
    if args.width is not None:
        GLB_width = args.width
    if args.channel is not None:
        GLB_input_shape = [GLB_batch_size, GLB_channel, GLB_height, GLB_width]
    else:
        GLB_input_shape = None

        print('GLB_input_shape', GLB_input_shape, type(GLB_input_shape))
    # print('GLB_model_path: ', GLB_model_path)
    # print('GLB_out_txt: ', GLB_out_txt)
    # print('GLB_batch_size: ', GLB_batch_size)
    # print('GLB_channel: ', GLB_channel)
    # print('GLB_height: ', GLB_height)
    # print('GLB_width: ', GLB_width)
