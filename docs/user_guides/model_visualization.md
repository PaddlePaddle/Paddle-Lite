...
=======
打开 debug_log.txt 文件，将会看到多个由以下格式构成的拓扑图定义，由于 recognize_digits 模型在优化后仅存在一个 subgraph，所以在文本搜索`subgraphs`的关键词，即可得到子图拓扑如下：
>>>>>>> 39b3bbd... update model_visualization test=develop, test=document_fix
...
=======
之后继续在 debug_log.txt 文件中，搜索`subgraphs`关键字，可以得到所有子图的.dot格式内容如下：
>>>>>>> 39b3bbd... update model_visualization test=develop, test=document_fix
