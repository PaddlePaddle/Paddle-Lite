...
# 注意默认编译Windows X86平台，如果需要编译X64平台，需要加入build_x64选项
lite\tools\build_windows.bat build_x64
...
|   参数     |     介绍     |     值     |
|-----------|-------------|-------------|
|  with_log | 可选，是否编译带日志的预测库（默认为OFF）| `ON`、`OFF` |
...
|  build_x64 | 可选，是否编译X64平台预测库（默认为OFF）| `ON`、`OFF` |
