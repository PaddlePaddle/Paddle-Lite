...
Paddle Lite 支持在 Docker 或 Linux 环境编译 X86 预测库。环境搭建参考[编译环境准备](../source_compile/compile_env.rst)。
...
### 预测库编译
Windows 预测库编译参见[源码编译( Windows )](../source_compile/compile_windows.rst)。
