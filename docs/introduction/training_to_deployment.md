...
本文档将向您介绍如何使用 Paddle 新接口训练和推理一个模型, 保存训练的模型后，使用Lite的c++接口，在 andriod arm 上部署这个模型。
在这之前，可以参考[ Paddle安装指南 ](https://www.paddlepaddle.org.cn/install/quick?docurl=/documentation/docs/zh/install/pip/linux-pip.html)完成 Paddle 的安装。
...
注意：如果是以权重分离的方式保存模型的模型，config按如下方式设置：
