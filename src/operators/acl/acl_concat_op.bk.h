...
class AclConCtx : public acl::ACLOperator {
 public:
  AclConCtx(){
  ...
  ~AclConCtx() = default;
  AclConCtx(const AclConCtx&) = delete;
  AclConCtx& operator=(const AclConCtx&) = delete;
  AclConCtx(AclConCtx&&) = delete;
  AclConCtx& operator=(AclConCtx&&) = delete;
  ...
  void InitAclLayer(const framework::ExecutionContext& context) {
    ...
    const std::vector<const framework::Tensor*>* input_data = 
      (const std::vector<const framework::Tensor*>*)args.input_data;
...
private:
  void AclParametersByContext(const framework::ExecutionContext& context){
    auto input = context.MultiInput<framework::Tensor>("X");
    auto* output = context.Output<framework::Tensor>("Out");
    const std::vector<const framework::Tensor*>* input_data = &input;
...
template <typename DeviceContext, typename T>
class AclConcatKernel : public framework::OpKernel<T> {
 public:
  bool Bypass_acl(const framework::ExecutionContext& context) const {
      AclConCtx<DeviceContext, T>* acl_ctx = reinterpret_cast<AclConCtx<DeviceContext, T>*>(context.GetAclCtx());
      if (acl_ctx == nullptr) {
        acl_ctx = new AclConCtx<DeviceContext, T>();
  ...
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    for (int i = 0 ; i < input.size(); i++){
      std::cout << input[i]->dims()[i] <<std::endl;
    }
