...
void conv_depthwise_3x3p1(const float* din, float* dout, \
                          ...
                          int stride, bool flag_bias, bool flag_relu){
    if (stride == 1) {
        if (flag_relu) {
            if (w_in > 4) {
...
void conv_depthwise_3x3s1p1_bias(float* dout, const float* din, \
    ...
    float* zero_ptr = static_cast<float *>(framework::CPUContext::Context()->get_work_space(w_in * sizeof(float)));
        ...
        for (int c = 0; c < ch_in; c++) {
            ...
            for (int i = 0; i < h_in; i += 4){
                ...
                : [w0]"w"(wr0), [w1]"w"(wr1), [w2]"w"(wr2), [bias_val] "r" (vbias), \
                      [vmask] "r" (vmask), [rmask] "r" (rmask), [vzero] "w" (vzero)
