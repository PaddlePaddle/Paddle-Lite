...
bool ACLOperator::new_tensor(std::unique_ptr<ACLTensor> &tensor,
                             ...
                             void *mem, bool commit) {
    auto acl_tensor = new ACLTensor(arm_compute::TensorInfo(shape,
...
int BaseACLTensor::tensor_copy(arm_compute::ITensor* tensor, void * mem,
    ...
    if (toTensor) {  // mem->tensor
        arm_compute::execute_window_loop(window, [&]
          ...
          memcpy(it.ptr(), ((char*)mem) + ((id[3] * (width * height * deepth) +
                   id.z() * (width * height) + id.y() * width + id.x()) *
                   tensor->info()->element_size()), width *
        ...
        arm_compute::execute_window_loop(window, [&]
                     (const arm_compute::Coordinates & id) {
           memcpy(((char*)mem) + ((id[3] * (width * height * deepth) + id.z() *
                  (width * height) + id.y() * width) *
                  tensor->info()->element_size()), it.ptr(), width *
                  tensor->info()->element_size());
        },
        it);
    }
