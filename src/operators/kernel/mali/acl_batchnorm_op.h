...
template <typename DeviceType, typename T>
class AclBatchNormKernel
    : public framework::OpKernelBase<DeviceType, BatchNormParam> {
 public:
  bool Bypass_acl(const BatchNormParam& param) const {
    AclBatchNormOp<DeviceType, T>* acl_op =
        reinterpret_cast<AclBatchNormOp<DeviceType, T>*>(this->GetAclOp());
    if (acl_op == nullptr) {
      acl_op = new AclBatchNormOp<DeviceType, T>();
      this->SetAclOp((void*)acl_op, (void*)this);
    }
    return acl_op->Bypass_acl(param);
  }

  void Compute(const BatchNormParam& param) const override {
    AclBatchNormOp<DeviceType, T>* acl_op =
        reinterpret_cast<AclBatchNormOp<DeviceType, T>*>(this->GetAclOp());
    if (acl_op == nullptr) {
      return;
