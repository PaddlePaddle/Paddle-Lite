...
template <typename DeviceType, typename T>
class AclConvAddKernel
    : public framework::OpKernelBase<DeviceType, FushionConvAddParam> {
 public:
  bool Bypass_acl(const FushionConvAddParam& param) const {
    AclConvAddOp<DeviceType, T>* acl_op =
        reinterpret_cast<AclConvAddOp<DeviceType, T>*>(this->GetAclOp());
    if (acl_op == nullptr) {
