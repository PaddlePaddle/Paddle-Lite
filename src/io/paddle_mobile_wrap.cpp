...
void Net<Device>::SetThreadNum(int threads) {
    auto engine = (paddle_mobile::PaddleMobile<paddle_mobile::CPU> *)this->engine_;
    if (engine != nullptr) {
...
PMStatus Net<Device>::Load(const std::string &dirname, const bool optimize,
    ...
    auto engine = (paddle_mobile::PaddleMobile<paddle_mobile::CPU> *)this->engine_;
    if (engine != nullptr) {
        paddle_mobile::PMStatus status = engine->Load(dirname, false, false, 1, true);
...
template <typename Device>
PMStatus Net<Device>::Load(const std::string &model_path, const std::string &para_path,
        const bool optimize, const bool quantification,
        const int batch_size, const bool lod_mode) {
    auto engine = (paddle_mobile::PaddleMobile<paddle_mobile::CPU> *)this->engine_;
    if (engine != nullptr) {
        paddle_mobile::PMStatus status = engine->Load(model_path, para_path, false, false, 1, true);
        return status == paddle_mobile::PMSuccess ? PMSuccess : PMUnKownError;
    }
    return PMUnKownError;
}
...
bool Net<Device>::LoadCombinedMemory(size_t model_len, const uint8_t *model_buf,
    ...
    auto engine = (paddle_mobile::PaddleMobile<paddle_mobile::CPU> *)this->engine_;
    if (engine != nullptr) {
        bool status = engine->LoadCombinedMemory(model_len, model_buf, combined_params_len, combined_params_buf, false, false, 1, true);
        return status;
    }
    return false;
}
...
PMStatus Net<Device>::Predict(const Tensor &input) {
    ...
    if (engine != nullptr) {
        ...
        paddle_mobile::framework::Tensor input_inner(input_data, paddle_mobile::framework::make_ddim(input_dims_as_vector));
...
template <typename Device>
std::vector<float> Net<Device>::Predict(const std::vector<float> &input, const std::vector<int64_t> &dims) {
    auto engine = (paddle_mobile::PaddleMobile<paddle_mobile::CPU> *)this->engine_;
    if (engine != nullptr) {
