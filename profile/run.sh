...
if [ ${8} == '1' ]; then
    ...
    adb shell "cd $BIN_PATH && ./test_model_bin_v8 --model_dir=./tf_model/${2} --input_shape=${4} --out_txt=${3} --warmup=${5} --repeats=${6} --threads=${7} --use_optimize_nb=0 --power_mode=3 && exit" #
else 
    echo "v7"
    ...
    adb shell "cd $BIN_PATH && ./test_model_bin_v7 --model_dir=./tf_model/${2} --input_shape=${4} --out_txt=${3} --warmup=${5} --repeats=${6} --threads=${7} --use_optimize_nb=0 --power_mode=3 && exit" #
fi 
