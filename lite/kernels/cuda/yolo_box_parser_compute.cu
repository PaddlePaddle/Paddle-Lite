...
cudaError_t yolo_tensor_parse_cuda(
    ...
    int* bbox_count_alloc_ptr,  // [in/out] maximum bounding box number
...
void YoloBoxParserCompute::Run() {
  ...
  *bbox_count_alloc_ptr = 500;
  ...
  cudaMalloc((void**)bboxes_tensor_ptr,
             *bbox_count_alloc_ptr * (5 + class_num)sizeof(float));
  ...
  cudaMemcpyAsync(d_anchors0,
                  anchors0.data(),
                  anchors0.size() * sizeof(int),
                  cudaMemcpyHostToDevice,
                  stream);
  cudaMemcpyAsync(d_anchors1,
                  anchors1.data(),
                  anchors0.size() * sizeof(int),
                  cudaMemcpyHostToDevice,
                  stream);
  cudaMemcpyAsync(d_anchors2,
                  anchors2.data(),
                  anchors0.size() * sizeof(int),
                  cudaMemcpyHostToDevice,
                  stream);
  ...
  for (int batch_index = 0; batch_index < batch; batch_index++) {
    for (int input_index = 0; input_index < x.size(); input_index++) {
      ...
      const float* input =
          x[input_index]->data<float>() +
          batch_index * input_dims[1] * input_dims[2] * input_dims[3];
      const float* ImShape_data = ImShape->data<float>() + batch_index * 2;
      const float* ScaleFactor_data =
          ScaleFactor->data<float>() + batch_index * 2;
...
REGISTER_LITE_KERNEL(
    yolo_box_parser, kCUDA, kFloat, kNCHW, paddle::lite::kernels::cuda
    : YoloBoxParserCompute, def)
    .BindInput("X",
