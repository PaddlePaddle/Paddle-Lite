...
void Conv2dTransposeCompute<PRECISION(kFloat)>::Run() {
  ...
  float* max_filter_ptr = nullptr;
  float* max_image_ptr = nullptr;
  int filter_size = param.filter->numel();
  int image_size = param.x->numel();
  size_t cpy_lens = 4 * sizeof(float);
  int r = xpu_malloc(reinterpret_cast<void**>(&max_filter_ptr), cpy_lens);
  CHECK_EQ(r, 0);
  r = xpu_malloc(reinterpret_cast<void**>(&max_image_ptr), cpy_lens);
  CHECK_EQ(r, 0);
  ...
  xpu_free(max_filter_ptr);
  xpu_free(max_image_ptr);
}
