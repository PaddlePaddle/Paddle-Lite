...
void ElementwiseAddCompute::Run() {
  ...
  auto y_dims_after_trailing = trim_trailing_singular_dims(y_dims_untrimed);
  axis = (y_dims_after_trailing.size() == 0) ? x_dims.size() : axis;
  int pre, n, post;
  get_mid_dims(x_dims, y_dims_after_trailing, axis, &pre, &n, &post);
  int len = pre * n * post;
  ...
  if (pre != 1 || post != 1) {
    int r =
        xpu_malloc(reinterpret_cast<void**>(&y_broadcast), len * sizeof(float));
    CHECK_EQ(r, 0);
