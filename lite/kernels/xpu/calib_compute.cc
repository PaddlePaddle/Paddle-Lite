...
using xpu_calib_int64_to_float =
    paddle::lite::kernels::xpu::CalibCompute<int64_t, float>;
...
REGISTER_LITE_KERNEL(
    calib, kXPU, kFloat, kNCHW, xpu_calib_int64_to_float, int64_to_float)
    .BindInput("Input",
               {LiteType::GetTensorTy(TARGET(kXPU), PRECISION(kInt64))})
...
REGISTER_LITE_KERNEL(
    calibOnce, kXPU, kFloat, kNCHW, xpu_calib_int64_to_float, int64_to_float)
    .BindInput("Input",
               {LiteType::GetTensorTy(TARGET(kXPU), PRECISION(kInt64))})
    .BindOutput("Out", {LiteType::GetTensorTy(TARGET(kXPU), PRECISION(kFloat))})
    .Finalize();
