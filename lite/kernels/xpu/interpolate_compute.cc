...
template <typename T>
inline std::vector<T> GetNewDataFromTensor(const Tensor* new_data_tensor) {
...
void PrepareLayout(lite::Tensor* input,
  ...
  if (list_new_size_tensor.size() > 0) {
    auto new_size = GetNewShape(list_new_size_tensor);
    *out_h = new_size[0];
    *out_w = new_size[1];
  } else {
    if (scale_tensor != nullptr) {
      auto scale_data = GetNewDataFromTensor<float>(scale_tensor);
      scale = scale_data[0];
    }
    if (scale > 0) {
    ...
    if (out_size != nullptr) {
      auto out_size_data = GetNewDataFromTensor<int>(out_size);
