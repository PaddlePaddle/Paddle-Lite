...
void BilinearInterpCompute::Run() {
  ...
  if (param.OutSize != nullptr) {
    auto out_size_data = param.OutSize->data<int>();
    std::unique_ptr<int[]> out_size_data_host(new int[2]);
    xpu_memcpy(reinterpret_cast<void*>(&out_size_data_host),
               out_size_data,
               sizeof(int) * 2,
               XPU_DEVICE_TO_HOST);

    out_h = out_size_data_host[0];
    out_w = out_size_data_host[1];
  }
