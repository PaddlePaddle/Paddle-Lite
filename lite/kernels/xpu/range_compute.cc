...
void RangeCompute<T>::Run() {
  ...
  //int len = param.End->data<int>() - param.Start->data<int>();
  int len = param.Out->dims().production();
  T start = (param.Start->template data<T>()[0]);
  T step = (param.Step->template data<T>()[0]);
  int r = xdnn::range(ctx.GetRawContext(),
...
using range_float_f = paddle::lite::kernels::xpu::RangeCompute<float>;
REGISTER_LITE_KERNEL(range, kXPU, kFloat, kAny, range_float_f, def)
