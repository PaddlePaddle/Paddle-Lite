...
#include "lite/kernels/arm/pixel_unshuffle_compute.h"
    ...
    for (int c = 0; c < in_channels; ++c) {
      for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
          int out_c = c * downscale_factor * downscale_factor + (y % downscale_factor) * downscale_factor + (x % downscale_factor);
