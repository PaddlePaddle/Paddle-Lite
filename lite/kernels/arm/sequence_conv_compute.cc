...
void SequenceConvCompute::Run() {
  ...
  for(int i = 0; i < static_cast<int>(lod_level_0.size()) - 1; i++){
    ...
    if (input_row_begin < input_row_end) {
      ...
      auto * sub_col_data = col_data + input_row_begin * kernel_size * hidden_dim;
      tmp.Resize({kernel_size * hidden_dim, input_row_end - input_row_begin});
      auto * tmp_data =  tmp.mutable_data<float>();
      // Image Col: [input_channels, filter_height, filter_width, output_height, output_width]
      // sequence Col: [1, kernel_size, hidden_dim, sequence_len, 1]
      paddle::lite::arm::math::im2col(
        sub_in_data, 1, sequence_len, hidden_dim, // C H W -> 1, seq_len, hidden_dim
        kernel_size, hidden_dim, // kernel_h, kernel_w
        up_pad, down_pad, 0, 0, // pad_top, pad_bottom, pad_left, pad_right
        1, 1, 1, 1, // stride_h, stride_w, dilation_h, dilation_w
        tmp_data);
      my_naive_transpose(tmp_data, sub_col_data, kernel_size * hidden_dim, input_row_end - input_row_begin);
    }
  }

  // SGDMM C := alpha * A * B + beta * C
  // matmul: col * filter_data
  // [sequence_len, kernel_size * hidden_dim] * [kernel_size * hidden_dim, kernel_num]
  // = [sequence_len, kernel_num]
  paddle::lite::operators::ActivationParam act_param;
  paddle::lite::arm::math::sgemm(
    false,
    false, // is_transB
    sequence_len, // M
    kernel_num, // N
