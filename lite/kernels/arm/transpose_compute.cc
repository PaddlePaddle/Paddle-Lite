...
void trans_basic(const int count, const Dtype* din, const int* permute_order, \
...
void TransposeCompute::Run() {
  ...
  const std::vector<int> axis = param.axis;
    ...
    if (trans_mat) {
        ...
        trans_basic(output->numel(), din, param.axis.data(), \
        _old_steps.data(), _new_steps.data(), input->dims().size(), dout);
    }
