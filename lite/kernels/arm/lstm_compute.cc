...
void LSTMComputeRun(const operators::LstmParam& param,
  ...
  if (enable_int8) {
    ...
    auto weight_data = weight->data<int8_t>();
    LOG(INFO) << "weight data:" << static_cast<int>(weight_data[0]) << " "
              << static_cast<int>(weight_data[1]);
    LOG(INFO) << "weight scale:" << weight_scale[0];
    LOG(INFO) << "bit length:" << bit_length;

  } else {
    auto weight_data = weight->data<float>();
    LOG(INFO) << "weight data:" << weight_data[0] << " " << weight_data[1]
              << " " << weight_data[2];
  }
  ...
  for (size_t n = 0; n < num_batch; n++) {
    ...
    if (n == 1) {
      LOG(INFO) << "batch_num:" << n;
      LOG(INFO) << "input data:" << gate_t[0] << " " << gate_t[1] << " "
                << gate_t[2] << " " << gate_t[3];
    }
    ...
    if (n > 0) {
      ...
      if (n == 1) {
        LOG(INFO) << "Ht-1 data:" << pre_hidden_t[0] << " " << pre_hidden_t[1]
                  << " " << pre_hidden_t[2] << " " << pre_hidden_t[3];
      }
      ...
      if (enable_int8) {
        ...
        lite::arm::math::QuantizeActvation(pre_hidden_t,
                                           ...
                                           bit_length);
        if (n == 1) {
          LOG(INFO) << "pre_hidden_scale:" << pre_hidden_scale;
          LOG(INFO) << "Ht-1 int8 data:" << static_cast<int>(pre_hidden_int8[0])
                    << " " << static_cast<int>(pre_hidden_int8[1]) << " "
                    << static_cast<int>(pre_hidden_int8[2]) << " "
                    << static_cast<int>(pre_hidden_int8[3]);
          std::stringstream ss;
          for (int i = 0; i < K; i++) {
            ss << pre_hidden_t[i] << ", ";
          }
    ...
    if (n == 1) {
      LOG(INFO) << "gate value:" << gate_t[0] << " " << gate_t[1] << " "
                << gate_t[2] << " " << gate_t[3] << " " << gate_t[4];
      std::stringstream ss;
