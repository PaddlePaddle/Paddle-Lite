...
void matmul_add_n_scale_bias(float* o_data, float* scale_, int m_, int n_) {
...
void MatMulCompute<PRECISION(kInt8), PRECISION(kFloat)>::Run() {
  ...
  act_param.has_active = false;
  for (int i = 0; i < 10; i++) {
    // std::cout << "matmul x_data:" <<(float)x_data[i] *
  ...
  for (int i = 0; i < m_ * n_; i++) {
    // std::cout << "matmul o_scale:" <<param.output_scale<<std::endl;
    // std::cout << "matmul o_data:" <<o_data[i]<<std::endl;
  }
}
