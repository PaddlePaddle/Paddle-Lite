...
void do_broadcast_compute(std::function<T(T, T)> op) {
  ...
  // run on kernel
  lite::Tensor x, y, output;
  auto x_dim = DDim(std::vector<int64_t>(x_dim_cut.begin(), x_dim_cut.end()));
  auto y_dim = DDim(std::vector<int64_t>(y_dim_cut.begin(), y_dim_cut.end()));
  x.Resize(x_dim);
  y.Resize(y_dim);
  output.Resize(DDim(std::vector<int64_t>(out_dim.begin(), out_dim.end())));
  T* x_data = x.mutable_data<T>();
  T* y_data = y.mutable_data<T>();
  T* output_data = output.mutable_data<T>();
