...
void ConvCompute<PRECISION(kFloat), PRECISION(kFloat)>::PrepareForRun() {
  ...
  if (param.groups == ic && ic == oc && kps_equal && pads_equal &&
    ...
    SetProfilerKernelName("depthwise");
    ...
    SetProfilerKernelName("winograd");
    ...
    SetProfilerKernelName("direct");
    // VLOG(3) << "invoking direct conv";
  } else {
    impl_ = new GemmLikeConv<PRECISION(kFloat), PRECISION(kFloat)>;
    SetProfilerKernelName("gemm");
    // VLOG(3) << "invoking gemm like conv";
  }
  SetProfilerKernelInfo(std::to_string(kh) + "x" + std::to_string(kw) + "s" +
                        std::to_string(stride) + "p" + std::to_string(pad) +
                        "g" + std::to_string(param.groups));
  SetProfilerOPS(2.f * param.filter->dims()[2] * param.filter->dims()[3] *
                 param.x->dims()[0] * param.output->dims()[1] *
                 param.output->dims()[2] * param.output->dims()[3] *
                 param.x->dims()[1] / param.groups);
  SetProfilerInputShape(std::to_string(param.x->dims()[0]) + "x" +
                        std::to_string(param.x->dims()[1]) + "x" +
                        std::to_string(param.x->dims()[2]) + "x" +
                        std::to_string(param.x->dims()[3]));
  SetProfilerFilterShape(std::to_string(param.filter->dims()[0]) + "x" +
                         std::to_string(param.filter->dims()[1]) + "x" +
                         std::to_string(param.filter->dims()[2]) + "x" +
                         std::to_string(param.filter->dims()[3]));
...
void ConvCompute<PRECISION(kInt8), PRECISION(kFloat)>::PrepareForRun() {
  ...
  SetProfilerKernelInfo(std::to_string(kh) + "x" + std::to_string(kw) + "s" +
                        std::to_string(sh) + "p" + std::to_string(paddings[0]) +
                        "g" + std::to_string(param.groups));
  SetProfilerOPS(2.f * param.filter->dims()[2] * param.filter->dims()[3] *
                 param.x->dims()[0] * param.output->dims()[1] *
                 param.output->dims()[2] * param.output->dims()[3] *
