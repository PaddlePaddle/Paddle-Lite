        ...
        for (int oh = 0; oh < out_h; ++oh) {
          for (int ow = 0; ow < out_w; ++ow) {
            int out_idx = n * group * out_c_group * out_h * out_w +
                          g * out_c_group * out_h * out_w + oc * out_h * out_w + oh * out_w + ow;
            ...
            for (int ic = 0; ic < in_c_group; ++ic) {
              for (int kh = 0; kh < kernel_h; ++kh) {
                for (int kw = 0; kw < kernel_w; ++kw) {
                  ...
                  int iidx = n * in_channel * in_h * in_w + g * in_c_group * in_h * in_w +
                             ic * in_h * in_w + ih * in_w + iw;
                  int widx = g * out_c_group * in_c_group * kernel_h * kernel_w +
                             oc * in_c_group * kernel_h * kernel_w + ic * kernel_h * kernel_w +
                             kh * kernel_w + kw;
...
//  const std::string relu_flag = "None";
...
//  }
//  x_data_img->FromNCHWArray(x_data_cpu.data(), x_dim);
//
//
//  // preparam kernel
...
//      bias_cpu_data[i] = static_cast<float>(dist(engine));
//    }
//    bias_data->FromNCHWArray<float, float>(bias_cpu_data, bias_dim);
//  }
//
//  // run opencl kernel
//  kernel->Launch();
...
//  out_data->ToNCHWArray(out_data_cpu.data(), out_dim);
//#ifdef PRINT_RESULT
//  // a: filter_d ==> <m, k> <=> <oc, ic>
//                // b: x_d      ==> <k, n> <=> <ic, ih*iw>
//                // c: output_d ==> <m, n> <=> <oc, ih*iw>
...
//
//  for (int i = 0; i < 100; i++) {
//    EXPECT_NEAR(out_data_cpu.data()[i], out_ref_data[i], 1e-4);
//    if (abs(out_data_cpu.data()[i] - out_ref_data[i]) > 1e-4) {
//      LOG(INFO) << "error idx:" << i << "sub" << abs(out_data_cpu.data()[i] - out_ref_data[i]);
//    }
//  }
//  if (bias_cpu_data)
//    free(bias_cpu_data);
...
TEST(conv2d, compute_conv2d_gemm) {
  ...
  const std::string relu_flag = "None";  // "relu", "relu6", "None"
                ...
                int n = oc;
                LOG(INFO) << "bs=" << batch_size << " oc=" << oc << " ic=" << ic << " ih=" << ih
                          << " iw=" << iw << " oh=" << oh << " ow=" << ow
                          << " bias_flag=" << bias_flag << " relu_flag=" << relu_flag;
                LOG(INFO) << "m=oc=" << oc << " k=ic*ksize*ksize=" << ic * ksize * ksize
                          << " n=oc=" << oc;

                auto kernels = KernelRegistry::Global().Create(
                    "conv2d", TARGET(kMetal), PRECISION(kFloat), DATALAYOUT(kMetalTexture2DArray));
                ASSERT_FALSE(kernels.empty());
                auto kernel = std::move(kernels.front());
                ...
                const DDim x_dim = DDim(std::vector<DDim::value_type>{batch_size, ic, ih, iw});
                const DDim filter_dim = DDim(std::vector<DDim::value_type>{oc, ic, ksize, ksize});
                const DDim bias_dim = DDim(std::vector<DDim::value_type>{oc});
                const DDim out_dim = DDim(std::vector<DDim::value_type>{batch_size, oc, oh, ow});
                ...
                float* bias_cpu_data =
                    bias_flag
                        ? reinterpret_cast<float*>(calloc(sizeof(float), bias_dim.production()))
                        : nullptr;
                if (bias_flag) {
                  for (int i = 0; i < bias_dim.production(); ++i) {
                ...
                // b: x_d      ==> <k, n> <=> <ic, ih*iw>
                // c: output_d ==> <m, n> <=> <oc, ih*iw>
                PrintData("mapped_filter", static_cast<float*>(mapped_filter), m, k);
                PrintData("mapped_x", static_cast<float*>(mapped_x), k, n, batch_size);
                PrintData("mapped_bias", static_cast<float*>(mapped_bias), m, 1);
                std::cout << "mapped_bias[0]:" << mapped_bias[0] << std::endl;
                PrintData("out_ref_data", static_cast<float*>(out_ref_data), m, n, batch_size);
                PrintData("mapped_out", static_cast<float*>(mapped_out), m, n, batch_size);
#endif
