...
void exp_image_compute::PrepareForRun() {
  ...
  string function_name = "exp";
  kernel_ = mtl_ctx->get_kernel(*device, function_name);
}

void exp_image_compute::Run() {
...
void exp_image_compute_half::PrepareForRun() {
  auto& context = ctx_->As<MetalContext>();
  auto mtl_ctx = (metal_context*)context.context();
  auto device = mtl_ctx->get_default_device();
  ...
  string function_name = "exp_half";
  kernel_ = mtl_ctx->get_kernel(*device, function_name);
}

void exp_image_compute_half::Run() {
  auto output_width = output_buffer_->textureWidth_;
  auto output_height = output_buffer_->textureHeight_;
  auto output_array_length = output_buffer_->arrayLength_;
...
REGISTER_LITE_KERNEL(exp,
                     ...
                     paddle::lite::kernels::metal::exp_image_compute,
                     def)
...
REGISTER_LITE_KERNEL(exp,
                     ...
                     kFP16,
                     kMetalTexture2DArray,
                     paddle::lite::kernels::metal::exp_image_compute_half,
                     def)
        .BindInput("X", {LiteType::GetTensorTy(TARGET(kMetal),
                                               PRECISION(kFP16),
                                               DATALAYOUT(kMetalTexture2DArray))})
