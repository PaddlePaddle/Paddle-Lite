...
void Conv2dImageCompute::SetupWithMPS() {
...
void Conv2dImageComputeHalf::PrepareForRun() {
  auto& context = ctx_->As<ContextMetal>();
  metal_context_ = (MetalContext*)context.context();
  auto device = metal_context_->GetDefaultDevice();
  ...
  if (param.activation_param.has_active) {
    if (lite_api::ActivationType::kRelu == param.activation_param.active_type)
      ...
      throw std::logic_error("ERROR: cannot support the activate type fuse");
    }
  }
  ...
  DDim blank_dim = DDimLite({output_dims[1]});
  blank_tensor_.Resize(blank_dim);
  auto p = blank_tensor_.mutable_data<MetalHalf, MetalImage>(blank_dim);
...
void Conv2dImageComputeHalf::Run() {
  const auto& param = this->Param<param_t>();
  auto output_width = output_buffer_->texture_width_;
  auto output_height = output_buffer_->texture_height_;
  auto output_array_length = output_buffer_->array_length_;
...
void Conv2dImageComputeHalf::SetupWithMPS() {
  // TODO: (lzy) add MPS support
}

void Conv2dImageComputeHalf::SetupWithoutMPS() {
  const auto& param = this->Param<param_t>();
  auto padLeft = (*param.paddings)[2];
  auto padTop = (*param.paddings)[0];
  assert((*param.paddings)[0] == (*param.paddings)[1]);
...
REGISTER_LITE_KERNEL(conv2d,
                     ...
                     kFP16,
                     kMetalTexture2DArray,
                     paddle::lite::kernels::metal::Conv2dImageComputeHalf,
                     def)
    .BindInput("Input",
               {LiteType::GetTensorTy(TARGET(kMetal),
                                      PRECISION(kFP16),
