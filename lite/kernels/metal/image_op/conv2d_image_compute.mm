...
void Conv2dImageCompute::SaveOutput() { MetalDebug::SaveOutput(function_name_, output_buffer_); };
...
void Conv2dImageCompute::setup_without_mps() {
  ...
  params_buffer_ = std::make_shared<MetalBuffer>(metal_context_, sizeof(conv_params), &conv_params);
...
void Conv2dImageCompute::setup_with_mps() {
  ...
  if (@available(iOS 11.3, *)) {
    ...
    if (is_depthwise_) {
      description =
          [MPSCNNDepthWiseConvolutionDescriptor cnnConvolutionDescriptorWithKernelWidth:filter_w
...
bool Conv2dImageCompute::canAddUseMPS() { return canMPSAddByChannel() || canMPSAddByElement(); }
