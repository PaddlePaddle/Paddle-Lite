...
void SoftmaxImageCompute::setup_without_mps() {
  ...
  params_buffer_ = std::make_shared<MetalBuffer>(metal_context_, sizeof(metal_param), &metal_param);
}
...
void SoftmaxImageCompute::setup_with_mps() {
  auto backend = (__bridge MetalContextImp *)metal_context_->backend();
  //
  mps_softmax_op_ = (__bridge_retained void *)[[MPSCNNSoftMax alloc] initWithDevice:backend.device];
