 ...
 public:
  void PrepareForRun() override {
    ...
    if ((input_dims.size() == 4 && input_dims[1] <= 4) ||
      ...
      output_buffer_ = param.y->template mutable_data<MetalHalf, MetalImage>(metal_context_, param.y->dims());
    }
    // scene: op params, resident memory can be initialized once
    else {
      output_buffer_ =
          param.y->template mutable_data<MetalHalf, MetalImage>(metal_context_, param.y->dims(), {0, 1, 2, 3});
