...
#include "lite/kernels/metal/image_op/expand_image_compute.h"
...
template class paddle::lite::kernels::metal::
    ExpandImageCompute<float, PRECISION(kFloat)>;
template class paddle::lite::kernels::metal::
    ExpandImageCompute<MetalHalf, PRECISION(kFP16)>;
typedef paddle::lite::kernels::metal::ExpandImageCompute<float,
                                                         PRECISION(kFloat)>
    MetalExpandFp32;
typedef paddle::lite::kernels::metal::ExpandImageCompute<MetalHalf,
                                                         PRECISION(kFP16)>
    MetalExpandFp16;

REGISTER_LITE_KERNEL(
    expand, kMetal, kFloat, kMetalTexture2DArray, MetalExpandFp32, def)
    .BindInput("X",
...
REGISTER_LITE_KERNEL(
    expand, kMetal, kFP16, kMetalTexture2DArray, MetalExpandFp16, def)
    .BindInput("X",
               {LiteType::GetTensorTy(TARGET(kMetal),
                                      PRECISION(kFP16),
                                      DATALAYOUT(kMetalTexture2DArray))})
