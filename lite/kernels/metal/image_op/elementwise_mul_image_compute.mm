...
void ElementwiseMulImageCompute<P, PTYPE>::PrepareForRun() {
  ...
  input_x_mul_dim_ = DDim(
      {input_buffer_y_->tensor_dim_[0], input_buffer_y_->tensor_dim_[1], 1, 1});
  auto valid = true;
  int by_channel = 0;
  if (input_buffer_x_->tensor_dim_.size() == 4) {
    if (input_buffer_y_->tensor_dim_.size() == 4) {
      if (input_buffer_y_->tensor_dim_[0] == 1 &&
        ...
        for (int i = 0; i < 4; i++) {
          if (input_buffer_x_->tensor_dim_[i] !=
              input_buffer_y_->tensor_dim_[i]) {
            valid = false;
            break;
          }
        }
  ...
  if (!valid) {
    throw std::logic_error(
        "ERROR: elementwise_mul only supports : 1. input shapes are the same. "
...
template class paddle::lite::kernels::metal::
    ElementwiseMulImageCompute<float, PRECISION(kFloat)>;
template class paddle::lite::kernels::metal::
    ElementwiseMulImageCompute<MetalHalf, PRECISION(kFP16)>;
