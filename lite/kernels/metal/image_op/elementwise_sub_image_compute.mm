...
void ElementwiseSubImageCompute::setup_without_mps() {
    ...
    if (!valid) {
        LOG(FATAL) << "elementwise_sub: only supports : 1.same shapes 2.by channel.";
    }
    ...
    function_name_ = "elementwise_sub";
    // pipline
    auto backend = (__bridge MetalContextImp*)metal_context_->backend();
    pipline_ = [backend pipline:function_name_];
}

ElementwiseSubImageCompute::~ElementwiseSubImageCompute() {
