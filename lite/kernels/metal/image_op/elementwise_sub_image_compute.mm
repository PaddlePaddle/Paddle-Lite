...
void elementwise_sub_image_compute::PrepareForRun() {
...
void elementwise_sub_image_compute::Run() {
  const auto& param = this->Param<param_t>();
...
void elementwise_sub_image_compute_half::PrepareForRun() {
  auto& context = ctx_->As<MetalContext>();
  auto mtl_ctx = (metal_context*)context.context();
  auto device = mtl_ctx->get_default_device();
...
void elementwise_sub_image_compute_half::Run() {
  const auto& param = this->Param<param_t>();
  auto output_width = output_buffer_->textureWidth_;
  auto output_height = output_buffer_->textureHeight_;
  auto output_array_length = output_buffer_->arrayLength_;
...
REGISTER_LITE_KERNEL(elementwise_sub,
                     ...
                     paddle::lite::kernels::metal::elementwise_sub_image_compute,
                     def)
...
REGISTER_LITE_KERNEL(elementwise_sub,
                     ...
                     kFP16,
                     kMetalTexture2DArray,
                     paddle::lite::kernels::metal::elementwise_sub_image_compute_half,
                     def)
        .BindInput("X", {LiteType::GetTensorTy(TARGET(kMetal),
                                               PRECISION(kFP16),
                                               DATALAYOUT(kMetalTexture2DArray))})
