...
int PriorBoxConverter(void* ctx, OpLite* op, KernelBase* kernel)
{
    ...
    float step_w = op_info->HasAttr("step_w") ? op_info->GetAttr<float>("step_w") : 0;
    float step_h = op_info->HasAttr("step_h") ? op_info->GetAttr<float>("step_h") : 0;
    float offset = op_info->HasAttr("offset") ? op_info->GetAttr<float>("offset") : 0;
