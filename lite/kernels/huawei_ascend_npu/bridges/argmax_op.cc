...
int ArgMaxConverter(void* ctx, OpLite* op, KernelBase* kernel) {
  ...
  std::cout<<"argmax axis:"<<axis<<std::endl;
  std::cout<<"argmax x_dims size:"<<x_dims.size()<<std::endl;
  std::cout<<"argmax x_dims:"<<x_dims.repr()<<std::endl;
  ...
  auto ge_dtype = CvtPrecisionType(PrecisionType(dtype));
  if (ge_dtype != ge::DT_INT64 || ge_dtype != ge::DT_INT32) {
    LOG(WARNING) << "[HUAWEI_ASCEND_NPU] Attr[dtype] is used to define the output type, \
                    only support DT_INT64 DT_INT32";
    return FAILED;
  }*/
