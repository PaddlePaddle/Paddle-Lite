...
TEST(softmax_arm_trustzone, init) {
  SoftmaxCompute<PRECISION(kFloat), PRECISION(kFloat)> softmax;
  ASSERT_EQ(softmax.precision(), PRECISION(kFloat));
  ASSERT_EQ(softmax.target(), TARGET(kARMTrustZone));
}

TEST(softmax_arm_trustzone, compute) {
  SoftmaxCompute<PRECISION(kFloat), PRECISION(kFloat)> softmax;
...
USE_LITE_KERNEL(softmax, kARMTrustZone, kFloat, kNCHW, def);
