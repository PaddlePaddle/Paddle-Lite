 ...
 protected:
  ...
  int LaunchDeviceProgram() override {
    ...
    if (GetBoolFromEnv("BATCH_SIZE_CHANGEBLE")) {
      ...
      //const std::vector<std::vector<int64_t>> new_output_size = graph->InferOutputsShape(graph_in);
      ...
      for (size_t i = 0; i < origin_otensors_.size(); ++i) {
        ...
        void* p_data = static_cast<void*>(origin_otensors_[i]->mutable_data<typename paddle::lite::subgraph::mlu::FPTypeTraits<Precision>::T>(TARGET(kMLU)));
