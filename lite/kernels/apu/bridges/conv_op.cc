...
int ConvConverter(void* ctx, OpLite* op, KernelBase* kernel) {
  ...
  LOAD_FUNCTIONS(libHandle, NeuronModel_setOperandValue, neuron_model_setOperandValue)
  LOAD_FUNCTIONS(libHandle, NeuronModel_addOperation, neuron_model_addOperation)
  LOAD_FUNCTIONS(libHandle, NeuronModel_setOperandSymmPerChannelQuantParams,
  ...
  CHECK_EQ(paddings.size(), 4L)
      << "[APU] Paddings size should be the same or twice as the input size." << paddings.size();
  ...
  VLOG(3) << "strides.size(): " << strides.size() << " ,groups: " << groups << " ,dilations: " << dilations[0] << ":" << dilations[1];
  VLOG(3) << "with_act: " << with_act << " ,act_type:" << act_type ;
  VLOG(3) << "input_dims: " << input_dims << " ,output_dims: " << output_dims << " ,weight_scale size: " << weight_scale.size();
  VLOG(3) << "filter_dims: "<< filter_dims << " ,memory_size: " << filter->memory_size() <<" ,data_size: " << filter->data_size();
  ...
  VLOG(3) << "input node idx" << input_node->index() << ": input_scale: " << input_scale << ", inType: " << inType.dimensions[0] << ":" << inType.dimensions[1] << ":" << inType.dimensions[2] << ":"  << inType.dimensions[3];
  ...
  if (is_depthwise_mode) {
    transpose_filter.Resize({1, (uint32_t)filter_dims[2],
                          (uint32_t)filter_dims[3], (uint32_t)filter_dims[0]});
    dims_filter = {1, (uint32_t)filter_dims[0],
                   (uint32_t)filter_dims[2], (uint32_t)filter_dims[3]};
    transpose(filter->data<int8_t>(),
              transpose_filter.mutable_data<uint8_t>(),
              dims_filter,
              {0,2,3,1});

    dims_filter = {(uint32_t)filter_dims[1], (uint32_t)filter_dims[2],
                   (uint32_t)filter_dims[3], (uint32_t)filter_dims[0]};
  } else {
    transpose_filter.Resize({(uint32_t)filter_dims[0], (uint32_t)filter_dims[2],
                          (uint32_t)filter_dims[3], (uint32_t)filter_dims[1]});
    dims_filter = {(uint32_t)filter_dims[0], (uint32_t)filter_dims[1],
                   (uint32_t)filter_dims[2], (uint32_t)filter_dims[3]};
    transpose(filter->data<int8_t>(),
              transpose_filter.mutable_data<uint8_t>(),
              dims_filter,
    ...
    dims_filter = {(uint32_t)filter_dims[0], (uint32_t)filter_dims[2],
                   (uint32_t)filter_dims[3], (uint32_t)filter_dims[1]};
  }
  ...
  if (1 == weight_scale.size()) {
    ...
    VLOG(3) << "filter node idx: " << filter_node->index() << "w_scale[0]" << weight_scale[0] << ": filterType: " << filterType.dimensions[0] << ":" << filterType.dimensions[1] << ":" << filterType.dimensions[2] << ":"  << filterType.dimensions[3];
    memcpy(filter->mutable_data<int8_t>(), transpose_filter.mutable_data<uint8_t>(), filter->memory_size());
    neuron_errCode = (*neuron_model_setOperandValue)(model, filter_node->index(), filter->raw_data(), filter->memory_size());
    ...
    filter_node = graph->Add(filter_name, dims_filter);
    VLOG(3) << "chennel filter node idx: " << filter_node->index() << " ,scale_count:" << weight_scale.size()
    ...
    neuron_errCode = (*neuron_model_setOperandSymmPerChannelQuantParams)(model, filter_node->index(), &symmPerChannelQuantParams);
    if (NEURON_NO_ERROR != neuron_errCode) {
  ...
  if (HasInputArg(op_info, scope, "Bias")) {
    ...
    VLOG(3) << "node idx" << bias_node->index() << ": Bias name: " << bias_name << " ,bias scale: " << biasType.scale << " ,dimensions: " << bias_dims;
  } else {
    ...
    VLOG(3) << "node idx" << bias_node->index() << ": Bias name: default_bias " << " ,bias scale: " << biasType.scale << " ,dimensions: " << dims_bias.size();
  }
  ...
  std::vector<uint32_t>dims_out = {(uint32_t)output_dims[0], (uint32_t)output_dims[2],
                                   (uint32_t)output_dims[3], (uint32_t)output_dims[1]};
  ...
  VLOG(3) << "output node idx: " << output_node->index() << ": output_scale: " << outType.scale << ", outType: " << outType.dimensions[0] << ":" << outType.dimensions[1] << ":" << outType.dimensions[2] << ":"  << outType.dimensions[3];

  // Add bias value
  if (HasInputArg(op_info, scope, "Bias")) {
    auto bias_name = op_info->Input("Bias").front();
    auto bias = scope->FindMutableTensor(bias_name);
    int32_t* int32_bias_data = (int32_t*)bias->mutable_data<float>();
    float2int32(bias->data<float>(),
                input_scale,
                weight_scale,
                int32_bias_data);

    VLOG(3) << "int32_bias_data: " << int32_bias_data[0] << " : " << int32_bias_data[1] << " : " << int32_bias_data[2] << " : " << int32_bias_data[3];
    neuron_errCode = (*neuron_model_setOperandValue)(model, bias_node->index(), bias->raw_data(), bias->memory_size());
  } else {
  ...
  VLOG(3) << "paddings: " << paddings[0] << ":" << paddings[1] << ":" << paddings[2] << ":" << paddings[3];
  // Add padding value
  int32_t padding_val[1];
  padding_val[0] = paddings[2];
  (*neuron_model_setOperandValue)(model, paddingL_node->index(), padding_val, sizeof(int32_t) * 1);
  padding_val[0] = paddings[3];
  (*neuron_model_setOperandValue)(model, paddingR_node->index(), padding_val, sizeof(int32_t) * 1);
  padding_val[0] = paddings[0];
  (*neuron_model_setOperandValue)(model, paddingT_node->index(), padding_val, sizeof(int32_t) * 1);
  padding_val[0] = paddings[1];
  (*neuron_model_setOperandValue)(model, paddingB_node->index(), padding_val, sizeof(int32_t) * 1);
  ...
  (*neuron_model_setOperandValue)(model, strideW_node->index(), stride_val, sizeof(int32_t) * 1);
  stride_val[0] = strides[0]; //height
  (*neuron_model_setOperandValue)(model, strideH_node->index(), stride_val, sizeof(int32_t) * 1);
  ...
  if (is_depthwise_mode) {
    int32_t dm = oc/ic;
    (*neuron_model_setOperandValue)(model, dm_node->index(), &dm, sizeof(int32_t) * 1);
    VLOG(3) << "depthwise multiplier:" << dm;

    // Depthwise conv
    (*neuron_model_setOperandValue)(model, fuse_node->index(), fuse_val, sizeof(int32_t) * 1);
    ...
    neuron_errCode = (*neuron_model_addOperation)(model, NEURON_DEPTHWISE_CONV_2D,
    ...
    std::vector<uint32_t>addOutIndex = {output_node->index()};
    neuron_errCode = (*neuron_model_addOperation)(model, NEURON_CONV_2D,
                           addInIndex.size(), &addInIndex[0],
                           addOutIndex.size(), &addOutIndex[0]);
  }
