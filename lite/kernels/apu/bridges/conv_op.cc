...
int ConvConverter(void* ctx, OpLite* op, KernelBase* kernel) {
  ...
  VLOG(3) << "strides.size(): " << strides.size() << " ,groups: " << groups << " ,dilations.size: " << dilations.size();
  VLOG(3) << "with_act: " << with_act << " ,act_type:" << act_type ;
  VLOG(3) << "input_dims: " << input_dims.size() << " ,output_dims: " << output_dims.size() << " ,weight_scale size: " << weight_scale.size();
  VLOG(3) << "filter_dims: "<< filter_dims.size() << " ,memory_size: " << filter->memory_size() <<" ,data_size: " << filter->data_size();
  VLOG(3) << "filter_dims[0]: "<< filter_dims[0] << " : " << filter_dims[1] << " : " << filter_dims[2] << " : " << filter_dims[3];
  ...
  VLOG(3) << input_node->index() << ": input_scale: " << input_scale << ", inType: " << inType.dimensions[0] << ":" << inType.dimensions[1] << ":" << inType.dimensions[2] << ":"  << inType.dimensions[3];
  ...
  VLOG(3) << output_node->index() << ": output_scale: " << output_scale << ", outType: " << outType.dimensions[0] << ":" << outType.dimensions[1] << ":" << outType.dimensions[2] << ":"  << outType.dimensions[3];
  ...
  // Add filter type
  std::vector<uint32_t> dims_filter = {(uint32_t)filter_dims[0], (uint32_t)filter_dims[2],
  ...
  if (1 == weight_scale.size()) {
    ...
    VLOG(3) << filter_node->index() << "w_scale[0]" << weight_scale[0] << ": filterType: " << filterType.dimensions[0] << ":" << filterType.dimensions[1] << ":" << filterType.dimensions[2] << ":"  << filterType.dimensions[3];
    neuron_errCode = NeuronModel_setOperandValue(model, filter_node->index(), filter->raw_data(), filter->memory_size());
    ...
    filter_node = graph->Add(filter_name, dims_filter);
    VLOG(3) << filter_node->index() << "w_scale[0]" << weight_scale[0] << ": channelFilterType: " << channelFilterType.dimensions[0]
    ...
    neuron_errCode = NeuronModel_setOperandSymmPerChannelQuantParams(model, filter_node->index(), &symmPerChannelQuantParams);
    if (NEURON_NO_ERROR != neuron_errCode) {
  ...
  if (HasInputArg(op_info, scope, "Bias")) {
    ...
    std::vector<uint32_t>dims_bias = {(uint32_t)bias_dims[0], (uint32_t)bias_dims[2],
                                      (uint32_t)bias_dims[3], (uint32_t)bias_dims[1]};
    biasType.dimensions = &dims_bias[0];
    NeuronModel_addOperand(model, &biasType);  //2: bias
    bias_node = graph->Add(bias_name, dims_bias);
    VLOG(3) << bias_node->index() << ": Bias name: " << bias_name << " ,bias scale: " << biasType.scale << " ,dimensions: " << bias_dims.size();
  } else {
    biasType.dimensionCount = 1;
    std::vector<uint32_t>dims_bias = {(uint32_t)output_dims[1]};
    biasType.dimensions = &dims_bias[0];
    NeuronModel_addOperand(model, &biasType);  //2: bias

    bias_node = graph->Add(filter_name + "_default_bias", dims_bias);
    VLOG(3) << bias_node->index() << ": Bias name: default_bias " << " ,bias scale: " << biasType.scale << " ,dimensions: " << dims_bias.size();
  }

  NeuronOperandType int32Type;
  int32Type.type = NEURON_INT32;
  int32Type.scale = 0.0f;
  int32Type.zeroPoint = 0;
  int32Type.dimensionCount = 1;
  std::vector<uint32_t> dims_int32 = {1};
  int32Type.dimensions = &dims_int32[0];
  ...
  if (HasInputArg(op_info, scope, "Bias")) {
    auto bias_name = op_info->Input("Bias").front();
    auto bias = scope->FindMutableTensor(bias_name);
    neuron_errCode = NeuronModel_setOperandValue(model, bias_node->index(), bias->raw_data(), bias->memory_size());
  } else {
    Tensor default_bias;
    default_bias.Resize({output_dims[1]});
    default_bias.mutable_data<int32_t>();
    VLOG(3) << "bais_default: " << default_bias.memory_size();
    neuron_errCode = NeuronModel_setOperandValue(model, bias_node->index(), default_bias.raw_data(), default_bias.memory_size());
  }
  if (NEURON_NO_ERROR != neuron_errCode) {
  ...
  NeuronModel_setOperandValue(model, paddingL_node->index(), padding_val, sizeof(int32_t) * 1);
  padding_val[0] = paddings[3];
  NeuronModel_setOperandValue(model, paddingR_node->index(), padding_val, sizeof(int32_t) * 1);
  padding_val[0] = paddings[0];
  NeuronModel_setOperandValue(model, paddingT_node->index(), padding_val, sizeof(int32_t) * 1);
  padding_val[0] = paddings[1];
  NeuronModel_setOperandValue(model, paddingB_node->index(), padding_val, sizeof(int32_t) * 1);

  // Add Stride
  int32_t stride_val[1];
  stride_val[0] = strides[1]; //width
  NeuronModel_setOperandValue(model, strideW_node->index(), stride_val, sizeof(int32_t) * 1);
  stride_val[0] = strides[0]; //height
  NeuronModel_setOperandValue(model, strideH_node->index(), stride_val, sizeof(int32_t) * 1);
  ...
  if (is_depthwise_mode) {
    int32_t dm = oc/ic;
    NeuronModel_setOperandValue(model, dm_node->index(), &dm, sizeof(int32_t) * 1);
    VLOG(3) << "depthwise multiplier:" << dm;

    // Depthwise conv
    NeuronModel_setOperandValue(model, fuse_node->index(), fuse_val, sizeof(int32_t) * 1);
    ...
    neuron_errCode = NeuronModel_addOperation(model, NEURON_DEPTHWISE_CONV_2D,
    ...
    std::vector<uint32_t>addOutIndex = {output_node->index()};
    neuron_errCode = NeuronModel_addOperation(model, NEURON_CONV_2D,
                           addInIndex.size(), &addInIndex[0],
                           addOutIndex.size(), &addOutIndex[0]);
  }
