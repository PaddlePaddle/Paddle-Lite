...
int FCConverter(void* ctx, OpLite* op, KernelBase* kernel) {
  ...
  transposeAsym(w->data<int8_t>(),
            ...
            {0,1,3,2});
  memcpy(w->mutable_data<int8_t>(), transpose_filter.mutable_data<uint8_t>(), w->memory_size());
  int neuron_errCode = (*neuron_model_setOperandValue)(model, w_node->index(), w->raw_data(), w->memory_size());
  ...
  if (HasInputArg(op_info, scope, "Bias")) {
    ...
    VLOG(3) <<  int32_bias_data[0] << ":" << int32_bias_data[1] << ":" << int32_bias_data[2] << ":" << int32_bias_data[3];
    neuron_errCode = (*neuron_model_setOperandValue)(model, bias_node->index(), bias->raw_data(), bias->memory_size()); //2: bias
  } else {
    ...
    neuron_errCode = (*neuron_model_setOperandValue)(model, bias_node->index(), int32_bias->raw_data(), int32_bias->memory_size()); //2: bias
    bias_node->set_data(int32_bias);
  }
  // Add fuse value
  int32_t fuse_val[1] = {0};
  (*neuron_model_setOperandValue)(model, fuse_node->index(), fuse_val, sizeof(int32_t) * 1); //3: fuse
  ...
  neuron_errCode = (*neuron_model_addOperation)(model, NEURON_FULLY_CONNECTED,
