...
//  neuron_errCode = NeuronModel_addOperand(model, &permsType); //perm
  ...
  VLOG(3) << "axis :" << axis[0] << ":" << axis[1] << ":" << axis[2] << ":" << axis[3];
//  neuron_errCode = NeuronModel_setOperandValue(model, perms_node->index(), &axis[0], sizeof(int32_t) * axis.size());
  neuron_errCode = (*neuron_model_setOperandValue)(model, perms_node->index(), &axis[0], sizeof(int32_t) * axis.size());
  if (NEURON_NO_ERROR != neuron_errCode) {
    LOG(WARNING) << "Insert transpose op fail!";
    return;
  }
...
//                                            addOutIndex.size(), &addOutIndex[0]);
  neuron_errCode = (*neuron_model_addOperation)(model, NEURON_TRANSPOSE,
                                            addInIndex.size(), &addInIndex[0],
                                            addOutIndex.size(), &addOutIndex[0]);
...
void transpose(const int8_t* input_data,
  ...
  VLOG(3) << input_shape[0] <<":"<<input_shape[1]<<":"<< input_shape[2] <<":" << input_shape[3];
    ...
    for (dim[1] = 0; dim[1] < input_shape[1]; dim[1]++) {
      for (dim[2] = 0; dim[2] < input_shape[2]; dim[2]++) {
        for (dim[3] = 0; dim[3] < input_shape[3]; dim[3]++) {
          ...
          new_index = dim[axis[0]] * shape[axis[1]] * shape[axis[2]] * shape[axis[3]] +
