...
int PoolConverter(void* ctx, OpLite* op, KernelBase* kernel) {
  ...
  NeuronModel_addOperand(model, &int32Type); //8: fuse
  ...
  NeuronModel_setOperandValue(model, filterW_node->index(), filter_val, sizeof(int32_t) * 1);
  filter_val[0] = global_pooling? x_dims[2] : ksize[0]; // height
  NeuronModel_setOperandValue(model, filterH_node->index(), filter_val, sizeof(int32_t) * 1);
  ...
  int neuron_errCode;
  if (pooling_type == "max") {
    neuron_errCode = NeuronModel_addOperation(model, NEURON_MAX_POOL_2D, addInIndex.size(),
                                 &addInIndex[0], addOutIndex.size(), &addOutIndex[0]);
  } else {
    neuron_errCode = NeuronModel_addOperation(model, NEURON_AVERAGE_POOL_2D, addInIndex.size(),
                                 &addInIndex[0], addOutIndex.size(), &addOutIndex[0]);
  }
