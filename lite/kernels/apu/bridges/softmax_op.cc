...
int SoftmaxConverter(void* ctx, OpLite* op, KernelBase* kernel) {
  ...
  if (graph->Has(x_name)) {
    ...
    // add input operand
    NeuronModel_addOperand(model, &xType); //input
  ...
  VLOG(3) << "output_scale size: " << out_scale;

  float beta_val[] = {1.0f};
  NeuronModel_setOperandValue(model, beta_node->index(), beta_val, sizeof(float) * 1);

  int32_t axis_val[1];
  axis_val[0] = axis;
  NeuronModel_setOperandValue(model, axis_node->index(), axis_val, sizeof(int32_t) * 1);
  ...
  int neuron_errCode = NeuronModel_addOperation(model, NEURON_SOFTMAX,
