...
int SoftmaxConverter(void* ctx, OpLite* op, KernelBase* kernel) {
  ...
  VLOG(3) << "input_scale size: " << input_scale << " ,x_dims size: " << x_dims.size() << " ,x_dims: " << x_dims;
  ...
  float beta_val[] = {1.0f};
  (*neuron_model_setOperandValue)(model, beta_node->index(), beta_val, sizeof(float) * 1);

  int32_t axis_val[1];
  axis_val[0] = axis;
  (*neuron_model_setOperandValue)(model, axis_node->index(), axis_val, sizeof(int32_t) * 1);
  std::vector<uint32_t>addInIndex = {x_node->index(), beta_node->index(), axis_node->index()};
  std::vector<uint32_t>addOutIndex = {out_node->index()};
  int neuron_errCode = (*neuron_model_addOperation)(model, NEURON_SOFTMAX,
