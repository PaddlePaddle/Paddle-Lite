...
inline void* LoadFunc(void* libHandle, const char* name) {
  ...
  if (fn == nullptr) {
    LOG(WARNING) << "Unable to open Neuron Runtime function [" << name << "] Because " << dlerror();
  }
  return fn;
}

#define LOAD_FUNCTIONS(libHandle, FUNC_NAME, VARIABLE_NAME) \
    FUNC_NAME VARIABLE_NAME = reinterpret_cast<FUNC_NAME>(LoadFunc(libHandle, #FUNC_NAME));
...
int SubgraphEngine::BuildDeviceProgram() {
  ...
  LOAD_FUNCTIONS(libHandle_, NeuronModel_identifyInputsAndOutputs, neuron_model_identifyInputsAndOutputs)
  ...
  (*neuron_model_identifyInputsAndOutputs)(model_, ins.size(), &ins[0], outs.size(), &outs[0]);
...
int SubgraphEngine::LaunchDeviceProgram() {
  ...
  LOAD_FUNCTIONS(libHandle_, NeuronExecution_setInput, neuron_execution_setInput)
  LOAD_FUNCTIONS(libHandle_, NeuronExecution_setOutput, neuron_execution_setOutput)
  ...
  for (size_t i = 0; i < origin_otensors_.size(); i++) {
    (*neuron_execution_setOutput)(run1, i, NULL,
