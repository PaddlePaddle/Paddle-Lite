...
int MulConverter(void* ctx, OpLite* op) {
  ...
  VLOG(3) << "x_var_name:" << x_var_name
          << ", is data: " << graph_ctx->HasNode(x_var_name);
  VLOG(3) << "y_var_name:" << y_var_name
          << ", is data: " << graph_ctx->HasNode(y_var_name);
  CHECK(graph_ctx->HasNode(x_var_name))
      << "[NPU] MatMul in HiAI DDK only support X is data, Y is const yet.";

  auto mul_node = graph_ctx->AddNode<ge::op::MatMul>(out_var_name);
  ...
  if (graph_ctx->HasNode(x_var_name)) {
    auto reshaped_x_node =
        graph_ctx->AddNode<ge::op::Reshape>(x_var_name + "/reshape");
    reshaped_x_node->set_input_tensor(*graph_ctx->GetNode(x_var_name));
    reshaped_x_node->set_attr_shape({m, k});
    reshaped_x_node->set_attr_axis(0);
    mul_node->set_input_x1(*reshaped_x_node);
  } else {
    auto x_const_node = graph_ctx->AddNode(x_var_name, *x, {m, k});
  ...
  if (graph_ctx->HasNode(y_var_name)) {
    auto reshaped_y_node =
        graph_ctx->AddNode<ge::op::Reshape>(y_var_name + "/reshape");
    reshaped_y_node->set_input_tensor(*graph_ctx->GetNode(y_var_name));
    reshaped_y_node->set_attr_shape({k, n});
    reshaped_y_node->set_attr_axis(0);
    mul_node->set_input_x2(*reshaped_y_node);
  } else {
    auto y_const_node = graph_ctx->AddNode(y_var_name, *y, {k, n});
