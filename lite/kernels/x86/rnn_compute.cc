...
static void reset_parameter_vector(const std::vector<Tensor*>& raw_params_vec,
...
*******************************************************/
static void preprocess(X86Context* ctx,
                       const Tensor* input,
                       const Tensor& weight,
  ...
  if("GRU" == mode) {
    ...
    std::memset(bias_ptr + bias_offt, 0, (bias_hh.numel() - bias_offt) * sizeof(float));
    lite::x86::math::fill_bias_fc(o_data, bias_tmp_hh.data<float>(), m, n);
  } else {
...
output:
    ...
    last_c:(2D)
*******************************************************/
static void postprocess(X86Context* ctx, 
                        Tensor* output,
                        const Tensor* init_h, 
                        const Tensor* init_c, 
                        Tensor* last_h,
  ...
  if ("LSTM" == mode) {
    ...
    for(int i = 0; i < output->dims()[0]; i++) {
      for(int j = 0; j < output->dims()[1]; j++) {
        offset = i * output->dims()[1] + j;
        cur_c_ptr[offset] = cur_c_ptr[offset] * mask_ptr[i] + 
                            pre_c_ptr[offset] * mask_ptr_1[i];
      }
    }
  }
}
...
static void create_mask_matrix(const Tensor* sequence_length, 
  ...
  std::memcpy(&seq_len_vec[0], sequence_length->data<int>(),
    sequence_length->numel() * sizeof(int));

  const int& table_width = mask_matrix->dims()[0];
  Tensor temp;
  DDimLite dims(std::vector<int64_t>{mask_matrix->dims()[1], mask_matrix->dims()[0]});
  temp.Resize(dims);
...
static void lstm_cell(X86Context* ctx,
                      Tensor* input,
                      Tensor* weight_hh,
                      Tensor* init_h,
                      Tensor* init_c,
...
static void gru_cell(X86Context* ctx,
                     Tensor* input,
                     Tensor* weight_hh,
                     Tensor* init_h,
                     Tensor* init_c,
  ...
  lite::x86::math::GRUMetaValue<float> gru_value;
  gru_value.gate_weight = weight_hh->data<float>();
  gru_value.state_weight = weight_hh->data<float>() + 2 * frame_size * frame_size;
  ...
  lite::x86::math::RnnGruUnitFunctorV2<float>::compute(ctx, 
                                                       gru_value, 
                                                       frame_size, 
                                                       batch_size, 
                                                       cand_act, 
...
static void RunRnnLayer(X86Context* ctx,
                       const Tensor* input,
  ...
  if(has_sequence_length) {
    ...
    create_mask_matrix(sequence_length, 
                       &mask_matrix, 
                       is_reverse,
                       &mask_min_length);
  ...
  if ("GRU" == mode) {
    ...
    std::memset(weight_hh_tmp.mutable_data<float>() + size * 2, 0, size * sizeof(float));
  }
...
void RnnCompute::Run() {
  ...
  std::vector<Tensor> init_h_unbind, init_c_unbind, 
        last_h_unbind, last_c_unbind;
  std::vector<Tensor *> init_h_unbind_t, init_c_unbind_t, 
        last_h_unbind_t, last_c_unbind_t;
  ...
  if("LSTM" == mode) {
    ...
    lite::host::math::split(state[1]->data<float>(), last_c_unbind_t, 0, stride2);
  }
  ...
  for (int i = 0; i < num_layers; i++) {
    ...
    if (is_bidirec) {
      ...
      lite::x86::math::ConcatFunctor<lite::TargetType::kX86, float>
          concat_x86;
      concat_x86(ctx, output_vec, 2, output_holder);
    } else {
  ...
  std::cout<<"\n-------------sts----------------\n"
  <<tmp<<" "
  <<pre_state[0]->dims()[0]<<" "<<pre_state[0]->dims()[1]<<" "<<pre_state[0]->dims()[2]<<" "
  <<"\n-----------------------------\n";
  tmp = 0.0;
