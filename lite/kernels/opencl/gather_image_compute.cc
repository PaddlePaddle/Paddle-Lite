//
// Created by chenyaohuang on 2021/9/16.
//
...
void GatherImageCompute::PrepareForRun() {
  ...
  DDim axis_image_shape =
      default_convertor.InitImageDimInfoWith(axis->dims());

  default_convertor.ImageToNCHW(axis_image_v.data(),
                                axis_v.data(),
                                axis_image_shape,
                                axis_dims);
...
//    std::cout<<"试试看，你猜是啥axis？     "<<(axis_v[i])<<std::endl;
  if(axis_v[0]!=axis_)
    axis_=axis_v[0],axis_change=true;
  // choose kernel
...
void GatherImageCompute::ReInitWhenNeeded() {
  ...
  if ((!first_epoch_for_reinit_ && ((x_dims != last_x_dims_) || (axis_change))) ||
    ...
    index_img_shape_=default_convertor.InitImageDimInfoWith(index->dims());  // w, h
...
void GatherImageCompute::GetGlobalWorkSize() {
  ...
  if(axis_==0){
    global_work_size_ =
        cl::NDRange{static_cast<cl::size_type>(index_img_shape_[0]),static_cast<cl::size_type>(x_img_shape_[0])};
  }
  else{
    global_work_size_ =
        cl::NDRange{static_cast<cl::size_type>(index_img_shape_[0]),static_cast<cl::size_type>(x_img_shape_[1])};
  }
  if(axis_==0){
...
//#define LITE_test_x_index_LOG
  ...
  DDim x_image_shape =
      default_converter->InitImageDimInfoWith(x_dims);
   DDim index_image_shape =
      default_converter->InitImageDimInfoWith(index_dims);
  ...
  TargetWrapperCL::ImgcpySync(index_image_data.data(),
                              ...
                              cl_image2d_slice_pitch,
                              IoDirection::DtoH);
  default_converter->ImageToNCHW(x_image_data.data(),
                                 x_v.data(),
                                 x_image_shape,
                                 x_dims);
  default_converter->ImageToNCHW(index_image_data.data(),
                                 index_v.data(),
                                 index_image_shape,
                                 index_dims);
 ...
 //   VLOG(1) << i<<":  "<<x_dims_data[i];
  ...
  DDim out_image_shape =
      default_converter->InitImageDimInfoWith(out_dims);
