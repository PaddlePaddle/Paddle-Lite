...
template <typename T>
void gemm_bias_trans(const T* a,
                     const int M,
                     const int K,
                     const T* b,
                     const int K_,
...
TEST(fc, compute) {
  ...
  const int n = 2048;  // 2048
        ...
        lite::Tensor x, w, bias, out, out_ref, x_h, out_h, bias_h;
        ...
        DDim w_dim;
        if (param.trans_weights == true) {
          w_dim = DDim(std::vector<DDim::value_type>{n, k});
        } else {
          w_dim = DDim(std::vector<DDim::value_type>{k, n});
        }
        ...
        auto* bias_data_h = bias_h.mutable_data<half_t>();
        ...
        for (size_t i = 0; i < bias_dim.production(); ++i) {
          ...
          bias_data[i] = 10;
          bias_data_h[i] = Float2Half(bias_data[i]);
        }
        if (fp16_flag) {
...
// }
        ...
        if (param.trans_weights == true) {
          gemm_bias_trans<float>(x_source.data(),
                                 m,
                                 k,
                                 w_source.data(),
                                 k,
        ...
        if (param.trans_weights == true) {
          PrintData("w", static_cast<float*>(w_source.data()), n, k);
        } else {
