...
void act_compute_ref(const dtype *x_data,
  ...
  for (int i = 0; i < x_dim.production(); i++){
    ...
    case 5: // sigmoid
      ...
      break;
    case 6: // tanh
      out_data[i] = (expf(x_data[i]) - expf(-x_data[i])) / (expf(x_data[i]) + expf(-x_data[i]));
      break;
    default:
      break;
    }
    ...
    for (auto c : {1, 3, 8, 23, 32}) {
      for (int h = 12; h <= 100; h += 13) {
        for (int w = 12; w <= 100; w += 25) {
          ...
          actParam.active_type = (paddle::lite_api::ActivationType)act_type;
          ...
          auto *act_out_data =
              act_out.mutable_data<half_t, cl::Image2D>(
                  act_image2d_shape["width"],
                  act_image2d_shape["height"]);
          ...
          std::unique_ptr<KernelContext> act_img_context(new KernelContext);
          context->As<OpenCLContext>().CopySharedTo(
          ...
          // compute ref cpu
          act_compute_ref<float>(mapped_x, x_dim, y_data_ref, act_type, threshold, scale);
// result
