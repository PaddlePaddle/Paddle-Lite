...
TEST(opencl_abs_buffer, compute) {
  // prepare data
  ...
  auto kernels = KernelRegistry::Global().Create(
      "abs", TARGET(kOpenCL), PRECISION(kFloat), DATALAYOUT(kNCHW));
  ASSERT_FALSE(kernels.empty());
  auto kernel = std::move(kernels.front());
  kernel->SetParam(param);
  std::unique_ptr<KernelContext> abs_context(new KernelContext);
  context->As<OpenCLContext>().CopySharedTo(
      &(abs_context->As<OpenCLContext>()));
  kernel->SetContext(std::move(abs_context));
  ...
  abs_compute_ref<float>(mapped_x, x_dim, out_ref.get());
...
// abs buffer
USE_LITE_KERNEL(abs, kOpenCL, kFloat, kNCHW, def);
