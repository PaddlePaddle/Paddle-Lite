...
void ConcatCompute<PRECISION(kFloat), DATALAYOUT(kImageDefault)>::Run() {
  ...
  const auto& x_dims = param.Out->dims();
...
void ConcatCompute<PRECISION(kFloat), DATALAYOUT(kNCHW)>::Run() {
  ...
  if (inputs.size() == 2) {
    ...
    for (i = 0; i < inputs.size(); i++) {
      arg_idx = 0;
      auto size = inputs[i]->dims()[axis_] auto* x_buf =
          inputs[i]->data<float, cl::Buffer>();
      global_work_size = cl::NDRange{size};
...
REGISTER_LITE_KERNEL(
    ...
    kFloat,
    kImageDefault,
    paddle::lite::kernels::opencl::ConcatComputeFloatImageDefault,
    ImageDefault)
    .BindInput("X",
...
REGISTER_LITE_KERNEL(
    ...
    kFloat,
    kNCHW,
    paddle::lite::kernels::opencl::ConcatComputeFloat,
    kNCHW)
    .BindInput("X",
               {LiteType::GetTensorTy(TARGET(kOpenCL),
                                      PRECISION(kFloat),
