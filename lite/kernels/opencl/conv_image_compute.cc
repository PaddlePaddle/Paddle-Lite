  ...
  if (UseFcReplaceConv()) {
    ...
    if (filter_tensor_h_ == 1 && filter_tensor_w_ == 1) {
      ...
      MUTABLE_DATA_GPU(filter_gpu_image0_,
                       ...
                       filter_image_data);
      NCHW2IMG4(
          filter_cpu, filter_image_data, filter_dims[0], filter_dims[1], 1);
      MUTABLE_DATA_GPU(filter_gpu_image1_,
                       filter_image_w_,
                       filter_image_h_,
                       filter_image_data);
      NCHW2IMG4(
          filter_cpu, filter_image_data, filter_dims[0], filter_dims[1], 2);
      MUTABLE_DATA_GPU(filter_gpu_image2_,
                       filter_image_w_,
                       filter_image_h_,
                       filter_image_data);
      NCHW2IMG4(
          filter_cpu, filter_image_data, filter_dims[0], filter_dims[1], 3);
...
#undef DEPTH_CONV_USE_SPL
      ...
      if (stride_h_ == 1 && input_tensor_c_ >= 32 && output_tensor_c_ >= 32 &&
        ...
        if (kernel_func_names_.back() != "conv2d_3x3_multi_batch") {
          kernel_func_paths_.push_back("image/conv2d_3x3_default_kernel.cl");
        } else {
...
#undef CONV_7x7_OPT
  ...
  // relu options
  VLOG(3) << "relu_fused_:" << relu_fused_ <<" sigmoid_fused_:"<<sigmoid_fused_
  ...
  if (conv_param_->activation_param.has_active) {
    ...
    }else if(conv_param_->activation_param.active_type==lite_api::ActivationType::kSigmoid){
...
void ConvImageCompute::SetGlobalWorkSize() {
  ...
  if (kernel_func_names_[0] == "fc") {
    ...
    global_work_size_ = cl::NDRange{static_cast<size_t>(c_blk_),
                                    static_cast<size_t>(w_blk_),
                                    static_cast<size_t>(nh_blk_)};
  } else if (kernel_func_names_[0] == "conv2d_3x3_opt_mali") {
    int w_blk_size = 2;
    int w_blk = (default_w_blk_ + w_blk_size - 1) / w_blk_size;
