  ...
  const std::string relu_flag = "leaky_relu";
              ...
              for (const auto p :
                ...
                for (int i = 0; i < out_dim.production(); i++) {
                  ...
                  auto abs_diff =
                      COMPUTE_ABS_DIFF(output_v[i], out_ref_data[i]);
...
const int stride = 2;
  ...
  const std::string relu_flag = "leaky_relu";
              ...
              for (const auto p :
                ...
                std::unique_ptr<KernelContext> conv_3x3_context(
                    new KernelContext);
                context->As<OpenCLContext>().CopySharedTo(
                ...
                VLOG(2) << "input_dim:" << input_dim
                        ...
                        << " out_dim:" << out_dim << " bias_flag:" << bias_flag
                        << " bias_dim:" << bias_dim << " group:" << group
                ...
                std::vector<float> input_v(batch_size * ic * ih * iw);
                std::vector<float> filter_v(oc * filter_channel * ksize *
                                            ksize);
...
//  int loop_cnt = 0;
  ...
  const std::string relu_flag = "leaky_relu";
              ...
              for (const auto p :
                ...
                std::unique_ptr<KernelContext> conv_5x5_context(
                    new KernelContext);
                context->As<OpenCLContext>().CopySharedTo(
                    &(conv_5x5_context->As<OpenCLContext>()));
                kernel->SetContext(std::move(conv_5x5_context));
...
//  int loop_cnt = 0;
  ...
  const std::string relu_flag = "leaky_relu";
              ...
              for (const auto p :
                ...
                std::unique_ptr<KernelContext> conv_7x7_context(
                    new KernelContext);
                context->As<OpenCLContext>().CopySharedTo(
                    &(conv_7x7_context->As<OpenCLContext>()));
                kernel->SetContext(std::move(conv_7x7_context));
...
// #define USE_TUNE
                ...
                std::vector<int> paddings = {
                    pad_up, pad_up, pad_left, pad_left};
                std::vector<int> dilations = {dilation, dilation};
                ...
                const DDim& filter_dim = lite::DDim{std::vector<int64_t>(
                    {oc, filter_channel, ksize_y, ksize_x})};
                const DDim& out_dim =
                    lite::DDim{std::vector<int64_t>({batch_size, oc, oh, ow})};
                // element wise bias
                const DDim& bias_dim = lite::DDim{std::vector<int64_t>({oc})};
