...
class GatherBufferCompute
 ...
 public:
  ...
  void Run() override {
    ...
    auto* out_buf =
        gather_param_->Out->template mutable_data<float, cl::Buffer>(
            TARGET(kOpenCL));
    ...
    if (gather_param_->Axis != nullptr) {
      ...
      if (x_dims.size() == 4) {  // n,c,h,w
        ...
        kernel_key << kernel_func_names_[0] << build_options_ << time_stamp_;
        auto kernel = context.cl_context()->GetKernel(kernel_key.str());
        global_work_size_ = cl::NDRange{static_cast<cl::size_type>(x_dims[2]),
        ...
        kernel_key << kernel_func_names_[0] << build_options_ << time_stamp_;
        auto kernel = context.cl_context()->GetKernel(kernel_key.str());
        global_work_size_ = cl::NDRange{static_cast<cl::size_type>(x_dims[1]),
        ...
        kernel_key << kernel_func_names_[0] << build_options_ << time_stamp_;
        auto kernel = context.cl_context()->GetKernel(kernel_key.str());
        global_work_size_ = cl::NDRange{static_cast<cl::size_type>(batch_size),
                                        static_cast<cl::size_type>(x_dims[1])};
...
REGISTER_LITE_KERNEL(gather,
                     ...
                     def)
    .BindInput("X", {LiteType::GetTensorTy(TARGET(kOpenCL), PRECISION(kFloat))})
    .BindInput("Index",
