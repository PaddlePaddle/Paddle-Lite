...
TEST(sin_image2d_fp16, compute) {
  ...
  auto kernels = KernelRegistry::Global().Create(
      "sin", TARGET(kOpenCL), PRECISION(kFP16), DATALAYOUT(kImageDefault));
  ...
  std::unique_ptr<KernelContext> sin_context(new KernelContext);
  context->As<OpenCLContext>().CopySharedTo(
      &(sin_context->As<OpenCLContext>()));
  kernel->SetContext(std::move(sin_context));
