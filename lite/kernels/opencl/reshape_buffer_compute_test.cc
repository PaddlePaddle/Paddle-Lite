...
TEST(reshape_opencl, compute) {
  ...
  lite::Tensor input, output, input_h, output_h;
  ...
  auto* output_data_h =
      output_h.mutable_data<half_t, cl::Buffer>(TARGET(kOpenCL));
  auto* input_data = input.mutable_data<float, cl::Buffer>(TARGET(kOpenCL));
  auto* output_data = output.mutable_data<float, cl::Buffer>(TARGET(kOpenCL));
