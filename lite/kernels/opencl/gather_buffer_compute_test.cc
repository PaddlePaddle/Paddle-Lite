...
template <typename Dtype>
void gather_compute_without_axis(Dtype* in,
                                 Dtype* out,
                                 Dtype* index,
                                 int index_size,
                                 int slice_size)
    ...
    for (int i = 0; i < inner_dim_size; i++) {
        for (int j = 0; j < index_size; j++) {
            for (int k = 0; k < outer_dim_size; k++) {
                ...
                int in_index = k + index_ * outer_dim_size +
    ...
    for (int i = 0; i < axis_dim_v; ++i) {
        for (int j = 0; j < x_dim_v; ++j) {
            for (int k = 0; k < index_dim_v; ++k) {
                ...
                auto *x_data = gather_x.mutable_data<float, cl::Buffer>(TARGET(kOpenCL));
                ...
                auto *index_data = gather_index.mutable_data<float, cl::Buffer>(TARGET(kOpenCL));
                auto *mapped_index = static_cast<float *>(
                    TargetWrapperCL::Map(index_data, 0, sizeof(float) * index_dim.production()));
                ...
                if (axis_dim.production() != 0) {
                    ...
                    fill_index_rand(mapped_index, index_dim.production(), x_dim[axis_index]);
                    ...
                    context->As<OpenCLContext>().CopySharedTo(&(gather_buf_context->As<OpenCLContext>()));
                    ...
                    gather_compute_without_axis(mapped_x,
                                                out_ref.get(),
                                                mapped_index,
                                                batch_size,
                                                slice_size);
