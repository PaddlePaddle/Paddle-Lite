...
void test(const lite_api::CLPrecisionType p,
  ...
  folder_converter->NCHWToImage(x_source.data(), x_image_data.data(), x_dim);
  ...
  basic_gemm<float>(
      x_source.data(), m, k, y_source.data(), k, n, out_ref.data());
...
TEST(matmul, compute_basic) {
  for (const auto precision_type :
       {ite_api::CLPrecisionType::CL_PRECISION_FP32,
