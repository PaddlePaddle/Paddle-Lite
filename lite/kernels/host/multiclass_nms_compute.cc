...
using nmsfp32 = paddle::lite::kernels::host::
    MulticlassNmsCompute<float, int, TARGET(kHost), PRECISION(kFloat)>;

using nmsfp32_int64 = paddle::lite::kernels::host::
    MulticlassNmsCompute<float, int64_t, TARGET(kHost), PRECISION(kFloat)>;
...
REGISTER_LITE_KERNEL(
    multiclass_nms2, kHost, kFloat, kNCHW, nmsfp32_int64, index_int64)
    .BindInput("BBoxes", {LiteType::GetTensorTy(TARGET(kHost))})
    .BindInput("Scores", {LiteType::GetTensorTy(TARGET(kHost))})
    .BindOutput("Out", {LiteType::GetTensorTy(TARGET(kHost))})
    .BindOutput("Index",
...
REGISTER_LITE_KERNEL(
    multiclass_nms3, kHost, kFloat, kNCHW, nmsfp32_int64, index_int64)
    .BindInput("BBoxes", {LiteType::GetTensorTy(TARGET(kHost))})
    .BindInput("Scores", {LiteType::GetTensorTy(TARGET(kHost))})
    .BindInput("RoisNum",
               {LiteType::GetTensorTy(TARGET(kHost), PRECISION(kInt32))})
