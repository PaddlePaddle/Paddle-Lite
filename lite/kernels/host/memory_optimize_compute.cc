 ...
 public:
  ...
  void Run() override {
    ...
    for (size_t i = 0; i < param.memory_reuse_table.size() - 1; i++) {
      ...
      var->GetMutable<lite::Tensor>()->ShareDataWith(
          *(substitute_var->GetMutable<lite::Tensor>()));
    }
  }
};
...
REGISTER_LITE_KERNEL(memory_optimize,
                     kHost,
                     kAny,
                     kAny,
                     paddle::lite::kernels::host::MemoryOptimizeCompute,
                     def)
    .Finalize();
