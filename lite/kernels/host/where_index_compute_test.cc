...
TEST(where_index_int32, init) {
  ...
  ASSERT_EQ(where_index_int32.target(), TARGET(kHost));
}

TEST(where_index_int32, retrive_op) {
  auto where_index_int32 =
      KernelRegistry::Global().Create<TARGET(kHost), PRECISION(kInt32)>(
          "where_index");
...
TEST(where_index_int64, init) {
  ...
  ASSERT_EQ(where_index_int64.target(), TARGET(kHost));
}

TEST(where_index_int64, retrive_op) {
  auto where_index_int64 =
      KernelRegistry::Global().Create<TARGET(kHost), PRECISION(kInt64)>(
          "where_index");
...
USE_LITE_KERNEL(where_index, kHost, kInt32, kAny, def);
USE_LITE_KERNEL(where_index, kHost, kInt64, kAny, def);
