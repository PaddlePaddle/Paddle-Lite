...
void detect_object(Tensor* tout, const float thresh, int orih, int oriw) {
  ...
  auto dims = tout->dims();
  for (int iw = 0; iw < dims[2]; iw++) {
    Object object;
    const float* values = dout + iw * tout.dims[3];
    object.class_id = static_cast<int>(values[0]);
    object.prob = static_cast<int>(values[1]);
    object.rec.x = static_cast<int>(values[2] * oriw));
  ...
  for (int i = 0; i < objects.size(); ++i) {
    ...
    if (object.prob > thresh) {
      ...
      pro_str << object.prob;
      std::string label =
          std::string(class_names[object.class_id]) + ": " + pro_str.str();
      LOG(INFO) << "detection in batch: " << object.batch_id
                << ", image size: " << image.cols << ", " << image.rows
                << ", detect object: " << class_names[object.class_id]
