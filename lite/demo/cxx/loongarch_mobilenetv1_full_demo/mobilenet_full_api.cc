...
void RunModel(std::string model_dir,
  ...
  std::string optimized_model_dir = "mobilenet_v1_loongarch";
#ifdef METAL
  ...
  config.set_valid_places(
      {Place{
       ...
       Place{TARGET(kLOONGARCH), PRECISION(kFloat)},
       Place{TARGET(kLOONGARCH), PRECISION(kInt64)},
       Place{TARGET(kLOONGARCH), PRECISION(kAny)}});
  ...
  if (!is_opencl_backend_valid) {
    config.set_valid_places({Place{TARGET(kLOONGARCH), PRECISION(kFloat)},
    ...
    // opencl precision option. Most loongarch devices only support fp32, so set
