...
void DumpLog(const T* ptr,
  ...
  VLOG(3) << "******" << tensor_name << ",len=" << len << ",sum=" << sum
          << ",mean=" << sum / len << "******";
...
void DumpTensor(const Tensor* tensor,
  ...
  if (tensor->template data<T>() == nullptr) {
    VLOG(3) << "Current tensor:" << tensor_name
  ...
  try {
    ...
    VLOG(3) << "Current tensor dump error:" << tensor_name;
  }
}
...
void DumpOpoutTensor(Instruction* inst,
  ...
  for (auto tensor_name : output_names) {
    ...
    switch (pout->precision()) {
      ...
      default: {
        VLOG(3) << "unsupport Tensor Type ,tensor name:" << tensor_name;
        break;
      }
    }
  }
