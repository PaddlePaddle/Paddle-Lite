...
//      }
 ...
 for(int i = 0; i < 100; i++) {
  ...
  std::unique_ptr<Tensor> cloned_input_tensor(std::move(cloned_predictor->GetInput(0)));
 ...
 for(int i = 0; i < 1; i++) {
  ...
  std::unique_ptr<const Tensor> cloned_output_tensor(std::move(cloned_predictor->GetOutput(0)));
  std::cout << "cloned_Output shape " << cloned_output_tensor->shape()[1] << std::endl;
  for (int i = 0; i < ShapeProduction(cloned_output_tensor->shape()); i += 100) {
    std::cout << "cloned_Output[" << i << "]: " << cloned_output_tensor->data<float>()[i]<< std::endl;
  }
