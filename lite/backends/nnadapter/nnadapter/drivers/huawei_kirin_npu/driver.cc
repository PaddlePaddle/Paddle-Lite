...
int createModelFromGraph(void* context, driver::Graph* graph, void** model) {
  NNADAPTER_LOG(INFO) << "Create model from graph for huawei_kirin_npu.";
...
int createModelFromCache(void* context,
  ...
  NNADAPTER_LOG(INFO) << "Create model from cache for huawei_kirin_npu.";
  *model = nullptr;
  auto m = new Model();
  if (!m) {
    return NNADAPTER_OUT_OF_MEMORY;
...
void destroyModel(void* context, void* model) {
  if (context && model) {
    NNADAPTER_LOG(INFO) << "Destroy model for huawei_kirin_npu.";
...
nnadapter::driver::Driver NNADAPTER_EXPORT
    NNADAPTER_AS_SYM2(NNADAPTER_DRIVER_TARGET) = {
        ...
        .createModelFromGraph =
            nnadapter::driver::huawei_kirin_npu::createModelFromGraph,
        .createModelFromCache =
            nnadapter::driver::huawei_kirin_npu::createModelFromCache,
        .destroyModel = nnadapter::driver::huawei_kirin_npu::destroyModel,
        .runModelSync = nnadapter::driver::huawei_kirin_npu::runModelSync,
        .runModelAsync = nnadapter::driver::huawei_kirin_npu::runModelAsync,
};
