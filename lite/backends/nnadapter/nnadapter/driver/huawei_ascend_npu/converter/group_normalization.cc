...
#include "core/operation/group_normalization.h"
...
void ConvertGroupNormalizationWithGroups1(Converter* converter,
  ...
  layer_norm_op->set_attr_begin_norm_axis(1);
  layer_norm_op->set_attr_begin_params_axis(1);
...
int ConvertGroupNormalization(Converter* converter, hal::Operation* operation) {
  GROUP_NORMALIZATION_OPERATION_EXTRACT_INPUTS_OUTPUTS
  if (groups == 1) {
    ConvertGroupNormalizationWithGroups1(converter, operation);
