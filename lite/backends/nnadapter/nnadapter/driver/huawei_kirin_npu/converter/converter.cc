...
#include "driver/huawei_kirin_npu/converter/all.h"  // NOLINT
#undef __NNADAPTER_DRIVER_HUAWEI_KIRIN_NPU_CONVERTER_ALL_H__
...
std::shared_ptr<Operator> Converter::GetMappedOperator(hal::Operand* operand) {
...
std::shared_ptr<Operator> Converter::AddConstantOperator(
  ...
  auto op = AddOperator<hiai::op::Const>();
...
std::shared_ptr<Operator> Converter::ConvertOperand(
  ...
  if (IsConstantOperand(operand)) {
    auto op = AddOperator<hiai::op::Const>(operand);
    auto tensor = std::make_shared<ge::Tensor>();
    tensor->SetTensorDesc(*tensor_desc);
    ...
    auto op = AddOperator<hiai::op::Data>(operand);
