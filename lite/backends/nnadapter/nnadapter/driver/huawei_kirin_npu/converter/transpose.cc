...
int Program::ConvertTranspose(hal::Operation* operation) {
  ...
  auto permute_name = GetOperatorName(output_operand);
  auto permute_op = std::make_shared<hiai::op::Permute>(permute_name);
  std::vector<int32_t> perm(perm_data, perm_data + perm_count);
  permute_op->set_attr_order(ge::AttrValue::LIST_INT(perm.begin(), perm.end()));
  SET_INPUT(permute_op, x, input_operator);
  MAP_OUTPUT(permute_op, y, output_operand);
