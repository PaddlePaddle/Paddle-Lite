...
uint32_t Converter::GetMappedIndex(hal::Operand* operand) {
...
uint32_t Converter::UpdateIndexMap(hal::Operand* operand, uint32_t index) {
  auto it = operand_indexes_->find(operand);
  if (it == operand_indexes_->end()) {
...
uint32_t Converter::ConvertOperand(hal::Operand* operand,
