...
#define YOLO_BOX_PARSER_OPERATION_EXTRACT_INPUTS_OUTPUTS                     \
  ...
  NNADAPTER_VLOG(5) << "image_shape: "                                       \
                    << OperandToString(image_shape_operand);                 \
  ...
  auto downsample_ratio2 =                                                   \
      *reinterpret_cast<int*>(input_operands[12]->buffer);                   \
  ...
  auto nms_thresh = *reinterpret_cast<float*>(input_operands[15]->buffer);   \
  /* Output */                                                               \
  auto boxes_scores_operand = output_operands[0];                            \
  NNADAPTER_VLOG(5) << "boxes_scores: "                                      \
                    << OperandToString(boxes_scores_operand);                \
  auto boxes_num_operand = output_operands[1];                               \
  NNADAPTER_VLOG(5) << "boxes_num: " << OperandToString(boxes_scores_operand);
}  // namespace operation
}  // namespace nnadapter
