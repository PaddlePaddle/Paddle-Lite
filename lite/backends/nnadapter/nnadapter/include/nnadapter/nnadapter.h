...
typedef enum {
  ...
  NNADAPTER_XOR,
  /**
   * yolo_box introduction:
   * https://www.paddlepaddle.org.cn/documentation/docs/zh/api/paddle/vision/ops/yolo_box_cn.html#yolo-box
   *
   * Inputs:
   * * 0: x, a NNADAPTER_FP32 tensor, [N, C, H, W].
   * * 1: image_size, a NNADAPTER_FP32 tensor, [N, 2].
   * * 2: anchors, a list, [?, 2].
   * * 3: class_num, int.
   * * 4: conf_thresh, float.
   * * 5: downsample_ratio, int, value=[32, 16, 8].
   * * 6: clip_bbox, bool, optional.
   * * 7: scale_x_y, int, optional, default=1.
   *
   * Outputs:
   * * 0: box, a NNADAPTER_FP32 tensor, [N, M, 4].
   * * 1: score, a NNADAPTER_FP32 tensor, [N, M, class_num].
   */
  NNADAPTER_YOLO_BOX,
  /**
   ...
   *
   * Inputs:
   * * 0: box, a NNADAPTER_FP32 tensor, [N, M, 4/8/16/32] or [M, C, 4]
   * Lod_tensor.
   * * 1: scores, a NNADAPTER_FP32 tensor, [N, C, M] or [M, C] Lod_tensor.
   * * 2: background_label, int, default=1.
   * * 3: score_threshold, float.
   * * 4: nms_top_k, int.
   * * 5: nms_threshold, float.
   * * 6: nms_eta, float, default=1.
   * * 7: keep_top_k, int.
   * * 8: normalized, bool,  default=true.
   *
   * Outputs:
   * * 0: box_res, a NNADAPTER_FP32 tensor, [?, 6].
   * * 1: index, a NNADAPTER_INT32 tensor, [?], optional.
   * * 2: nms_rois_num, a NNADAPTER_INT32 tensor, [?].
   */
