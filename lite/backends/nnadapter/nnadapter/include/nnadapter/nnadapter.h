...
typedef enum {
   ...
   *   1. A 3-D Tensor with shape [N, M, 4/8/16/24/32] represents the predicted
   * locations of M bounding bboxes, N is the batch size. 
   *     Each bounding box has four coordinate values and the layout is [xmin,
   * ymin, xmax, ymax], when box size equals to 4.
   *   2. A 3-D Tensor with shape [M, C, 4]. M is the number of bounding boxes,
   * C is the class number
   * * 1: scores,scores, a NNADAPTER_FLOAT32 tensor.
   *    Two types of scores are supported:
   *    1. A 3-D Tensor with shape [N, C, M] represents the predicted confidence
   ...
   * bounding boxes. In this case, input bboxes should be the first case with
   * shape [N, M, 4/8/16/24/32].
   *    2. A 2-D LoDTensor with shape [M, C]. M is the number of bbox, C is the
   * class number. In this case, input bboxes should be the second case with
   * shape [M, C, 4].
   ...
   * of background label. If set to 0, the background label will be ignored.
   ...
   * bboxes to be kept per image after NMS step. "-1" means keeping all bboxes
   * after NMS step.
