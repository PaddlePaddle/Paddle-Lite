...
int OpenDevice(void** device) {
  ...
  if (!d) {
    *device = nullptr;
    NNADAPTER_LOG(FATAL) << "Failed to open device for nvidia_tensorrt.";
...
int CreateContext(void* device,
  ...
  auto d = reinterpret_cast<Device*>(device);
  auto c = new Context(d, properties, runtime_parameters_function);
  if (!c) {
    *context = nullptr;
...
int CreateProgram(void* context,
                  ...
                  core::Cache* cache,
                  void** program) {
  NNADAPTER_LOG(INFO) << "Create program for nvidia_tensorrt.";
...
void DestroyProgram(void* program) {
  if (program) {
    NNADAPTER_LOG(INFO) << "Destroy program for nvidia_tensorrt.";
...
NNADAPTER_EXPORT nnadapter::driver::Device NNADAPTER_AS_SYM2(DEVICE_NAME) = {
    ...
    .version = 1,
    .open_device = nnadapter::nvidia_tensorrt::OpenDevice,
