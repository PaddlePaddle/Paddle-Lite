...
void FuseYoloBox(core::Model* model) {
  ...
  for (auto operation : operations) {
    ...
    if (operation->type == NNADAPTER_DIV) {
      ...
      /*
        // yolo_box_0
        auto yolo_box_operation_0_box_operand0 =
        yolo_box_operation_0->output_operands[0];
        auto yolo_box_operation_0_score_operand0 =
        yolo_box_operation_0->output_operands[1];
        auto box_0_consumers = GetOperandConsumers(model,
        yolo_box_operation_0_box_operand0);
        auto score_0_consumers = GetOperandConsumers(model,
        yolo_box_operation_0_score_operand0);
        if (box_0_consumers.size() != 1 || score_0_consumers.size() != 1) {
          break;
        }
        auto concat_0_operation_0 = box_0_consumers[0];
        auto transpose2_0_operation = score_0_consumers[0];
        if (concat_0_operation_0->type != NNADAPTER_CONCAT &&
        transpose2_0_operation != NNADAPTER_TRANSPOSE) {
          NNADAPTER_VLOG(5) << "Converting " <<
        OperationTypeToString(concat_0_operation_0->type)
                        << " ...";
          NNADAPTER_VLOG(5) << "Converting " <<
        ...
        NNADAPTER_CHECK_EQ(concat_0_operation_0->input_operands.size(), 4);
        auto transpose2_0_output_operand =
        transpose2_0_operation->output_operands[0];
        auto transpose2_0_consumers = GetOperandConsumers(model,
        transpose2_0_output_operand);
        if (transpose2_0_consumers.size() != 1) {
          break;
        }
        auto concat_0_operation_1 = transpose2_0_consumers[0];
        ...
        NNADAPTER_CHECK_EQ(concat_0_operation_1->input_operands.size(), 4);
        auto concat_0_output_operand_1 =
        concat_0_operation_1->output_operands[0];
        auto concat_0_consumers = GetOperandConsumers(model,
        concat_0_output_operand_1);
        if (concat_0_consumers.size() != 1) {
          break;
        }
        auto nms_0_operation_1 = concat_0_consumers[0];
        ...
        NNADAPTER_CHECK_EQ(nms_0_operation_1->input_operands.size(), 2);
        NNADAPTER_CHECK_EQ(nms_0_operation_1->output_operands.size(), 2);
        auto nms_0_output_operand_0 = nms_0_operation_1->output_operands[0];
        auto nms_1_output_operand_0 = nms_0_operation_1->output_operands[1];
        // yolo_box_1
        auto yolo_box_operation_1_box_operand0 =
        yolo_box_operation_1->output_operands[0];
        auto yolo_box_operation_1_score_operand0 =
        yolo_box_operation_1->output_operands[1];
        auto box_1_consumers = GetOperandConsumers(model,
        yolo_box_operation_1_box_operand0);
        auto score_1_consumers = GetOperandConsumers(model,
        yolo_box_operation_1_score_operand0);
        if (box_1_consumers.size() != 1 || score_1_consumers.size() != 1) {
          break;
        }
        auto concat_1_operation_0 = box_1_consumers[0];
        auto transpose2_1_operation = score_1_consumers[0];
        if (concat_1_operation_0->type != NNADAPTER_CONCAT &&
        transpose2_1_operation != NNADAPTER_TRANSPOSE) {
          NNADAPTER_VLOG(5) << "Converting " <<
        OperationTypeToString(concat_1_operation_0->type)
                        << " ...";
          NNADAPTER_VLOG(5) << "Converting " <<
        OperationTypeToString(transpose2_1_operation->type)
                        << " ...";
          break;
        }
        auto transpose2_1_output_operand =
        transpose2_1_operation->output_operands[0];
        auto transpose2_1_consumers = GetOperandConsumers(model,
        transpose2_1_output_operand);
        if (transpose2_1_consumers.size() != 1) {
          break;
        }
        auto concat_1_operation_1 = transpose2_1_consumers[0];
        if (concat_1_operation_1->type != NNADAPTER_CONCAT) {
          NNADAPTER_VLOG(5) << "Converting " <<
        OperationTypeToString(concat_1_operation_1->type)
                        << " ...";
          break;
        }
        auto concat_1_output_operand_1 =
        concat_1_operation_1->output_operands[0];
        auto concat_1_consumers = GetOperandConsumers(model,
        concat_1_output_operand_1);
        if (concat_1_consumers.size() != 1) {
          break;
        }
        auto nms_1_operation_1 = concat_1_consumers[0];
        if (nms_1_operation_1->type != NNADAPTER_MULTICLASS_NMS) {
          NNADAPTER_VLOG(5) << "Converting " <<
        OperationTypeToString(nms_1_operation_1->type)
                        << " ...";
          break;
        }
        // yolo_box_2
        auto yolo_box_operation_2_box_operand0 =
        yolo_box_operation_2->output_operands[0];
        auto yolo_box_operation_2_score_operand0 =
        yolo_box_operation_2->output_operands[1];
        auto box_2_consumers = GetOperandConsumers(model,
        yolo_box_operation_2_box_operand0);
        auto score_2_consumers = GetOperandConsumers(model,
        yolo_box_operation_2_score_operand0);
        if (box_2_consumers.size() != 1 || score_2_consumers.size() != 1) {
          break;
        }
        auto concat_2_operation_0 = box_2_consumers[0];
        auto transpose2_2_operation = score_2_consumers[0];
        if (concat_2_operation_0->type != NNADAPTER_CONCAT &&
        transpose2_2_operation != NNADAPTER_TRANSPOSE) {
          NNADAPTER_VLOG(5) << "Converting " <<
        OperationTypeToString(concat_2_operation_0->type)
                        << " ...";
          NNADAPTER_VLOG(5) << "Converting " <<
        OperationTypeToString(transpose2_2_operation->type)
                        << " ...";
          break;
        }
        auto transpose2_2_output_operand =
        transpose2_2_operation->output_operands[0];
        auto transpose2_2_consumers = GetOperandConsumers(model,
        transpose2_2_output_operand);
        if (transpose2_2_consumers.size() != 1) {
          break;
        }
        auto concat_2_operation_1 = transpose2_2_consumers[0];
        if (concat_2_operation_1->type != NNADAPTER_CONCAT) {
          NNADAPTER_VLOG(5) << "Converting " <<
        OperationTypeToString(concat_2_operation_1->type)
                        << " ...";
          break;
        }
        auto concat_2_output_operand_1 =
        concat_2_operation_1->output_operands[0];
        auto concat_2_consumers = GetOperandConsumers(model,
        concat_2_output_operand_1);
        if (concat_2_consumers.size() != 1) {
          break;
        }
        auto nms_2_operation_1 = concat_2_consumers[0];
        if (nms_2_operation_1->type != NNADAPTER_MULTICLASS_NMS) {
          NNADAPTER_VLOG(5) << "Converting " <<
        OperationTypeToString(nms_2_operation_1->type)
                        << " ...";
          break;
        }
        // fuse
        operation_map[nms_0_operation_1] =
        {concat_0_operation_0->output_operands[0],
        concat_0_operation_1->output_operands[0], nms_0_output_operand_0,
        nms_0_output_operand_1};
        // add new operation
        auto yolo_box_head0 = AddOperation(model);
        auto yolo_box_head1 = AddOperation(model);
        auto yolo_box_head2 = AddOperation(model);
        ...
        auto yolo_box_nms = AddOperation(model);
        yolo_box_head0->type = NNADAPTER_YOLO_BOX_HEAD;
        yolo_box_head0->input_operands =
        {yolo_box_operation_0->input_operands[0]}
        auto yolo_box_0_dims =
        yolo_box_operation_0_box_operand0->type.dimensions.count;
        auto yolo_box_0_dims_ptr =
        reinterpret_cast<int32_t*>(yolo_box_operation_0_box_operand0->buffer)
        auto yolo_score_0_dims =
        yolo_box_operation_0_score_operand0->type.dimensions.count;
        auto yolo_score_0_dims_ptr =
        reinterpret_cast<int32_t*>(yolo_box_operation_0_score_operand0->buffer)
        NNADAPTER_CHECK_EQ(yolo_box_0_dims, yolo_score_0_dims);
        std::vector<int32_t> yolo_box_0_output_dims;
        for (int i = 0; i < yolo_box_0_dims - 1; i++) {
          NNADAPTER_CHECK_EQ(yolo_box_0_dims_ptr[i], yolo_score_0_dims_ptr[i]);
          yolo_box_0_output_dims.push_back(yolo_box_0_dims_ptr[i]);
        }
        yolo_box_0_output_dims.push_back(yolo_box_0_dims_ptr[yolo_box_0_dims -
        1] + yolo_score_0_dims[yolo_box_0_dims-1]);
        auto yolo_box_head0_output_operand = AddOperand(model,
        yolo_box_0_output_dims, NNADAPTER_FLOAT32);
        yolo_box_head0->output_operands = {yolo_box_head0_output_operand};

        yolo_box_head1->type = NNADAPTER_YOLO_BOX_HEAD;
        yolo_box_head1->input_operands =
        {yolo_box_operation_1->input_operands[0]};
        auto yolo_box_1_dims =
        yolo_box_operation_1_box_operand0->type.dimensions.count;
        auto yolo_box_1_dims_ptr =
        reinterpret_cast<int32_t*>(yolo_box_operation_1_box_operand0->buffer)
        auto yolo_score_1_dims =
        yolo_box_operation_1_score_operand0->type.dimensions.count;
        auto yolo_score_1_dims_ptr =
        reinterpret_cast<int32_t*>(yolo_box_operation_1_score_operand0->buffer)
        NNADAPTER_CHECK_EQ(yolo_box_1_dims, yolo_score_1_dims);
        std::vector<int32_t> yolo_box_1_output_dims;
        for (int i = 0; i < yolo_box_1_dims - 1; i++) {
          NNADAPTER_CHECK_EQ(yolo_box_1_dims_ptr[i], yolo_score_1_dims_ptr[i]);
          yolo_box_1_output_dims.push_back(yolo_box_1_dims_ptr[i]);
        }
        yolo_box_0_output_dims.push_back(yolo_box_0_dims_ptr[yolo_box_0_dims -
        1] + yolo_score_0_dims[yolo_box_0_dims-1]);
        auto yolo_box_head0_output_operand = AddOperand(model,
        yolo_box_0_output_dims, NNADAPTER_FLOAT32);
        yolo_box_head1->output_operands = {yolo_box_head1_output_operand};

        yolo_box_head2->type = NNADAPTER_YOLO_BOX_HEAD;
        yolo_box_head2->input_operands =
        {yolo_box_operation_2->input_operands[0]}
        yolo_box_head2->output_operands =
        {yolo_box_operation_2->output_operands[0]}
        yolo_box_parser->type = NNADAPTER_YOLO_BOX_PARSER;
        yolo_box_parser->input_operands = {yolo_box_head0->output_operands[0],
        yolo_box_head0->output_operands[1], yolo_box_head0->output_operands[2]}
        yolo_box_parser->output_operands = {}
        yolo_box_nms->type = NNADAPTER_YOLO_BOX_NMS;
        yolo_box_nms->input_operands = {}
        yolo_box_nms->output_operands = nms_0_operation_1->output_operands;
        */
    }
  }
}
