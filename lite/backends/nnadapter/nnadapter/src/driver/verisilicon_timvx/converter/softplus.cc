...
int ConvertSoftplus(Converter* converter, core::Operation* operation) {
  ...
  NNADAPTER_CHECK(std::fabs(beta - 1.0f) <= 1e-5f)
      << "Only supports beta = 1.0f!";
  ...
  auto softplus_op =
      converter->graph()->CreateOperation<tim::vx::ops::SoftRelu>();
  softplus_op->BindInputs({input_tensor});
  softplus_op->BindOutputs({output_tensor});
