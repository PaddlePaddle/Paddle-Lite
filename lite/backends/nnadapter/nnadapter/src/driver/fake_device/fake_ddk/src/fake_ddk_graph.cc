...
static fakedevice_nn_node_t *FakeDeviceAddNode(fakedevice_nn_graph_t *graph,
  ...
  fprintf(stderr, "fake_ddk: fakedevice_nn_AddNode\n");
  fakedevice_nn_node_t *node_ = new fakedevice_nn_node_t();
  if (NULL != node_) {
    graph->node_table.push_back(node_);
    node_->op = optype;
    fprintf(stderr, "fake_ddk: fakedevice_nn_AddNode finished\n");
    return node_;
  } else {
    fprintf(stderr, "fake_ddk: AddNode FAIL !\n");
    return NULL;
  }
}
...
std::shared_ptr<fakedevice_nn_tensor_t> Graph::CreateTensor(
  ...
  fprintf(stderr, "fake_ddk: CreateTensor\n");
  std::shared_ptr<fakedevice_nn_tensor_t> tensor_ =
      std::make_shared<fakedevice_nn_tensor_t>();
  if (NULL != tensor_) {
    tensor_->attr = attr;
    uint32_t tensor_size = 1;
    for (uint32_t i = 0; i < attr.get()->dims.size(); i++) {
      tensor_size *= attr.get()->dims[i];
    }
    if (attr.get()->precision == fake_ddk::nn::PrecisionType::INT32) {
      tensor_size *= sizeof(int32_t);
    } else {
      tensor_size *= sizeof(uint8_t);
    }
    tensor_->data = malloc(tensor_size);
    if (data) {
      memcpy(tensor_->data,
             data,
             tensor_size);  // If need re-arrange, process here
    }
  } else {
    fprintf(stderr, "fake_ddk: CreateTensor FAIL !\n");
    return NULL;
  }
...
static int AddOperatorConv2d(
    ...
    void *attrs) {
  fprintf(stderr, "fake_ddk: addOperator_CONV2D\n");
  ...
  if (node == nullptr) {
    fprintf(stderr, "fake_ddk: add node is fail!\n");
    return -1;
  }
  ...
  fprintf(stderr, "fake_ddk: addOperator_CONV2D finished\n");
...
int Graph::AddOperator(OperatorType type,
                       ...
                       void *attrs,
                       std::string name) {
  fprintf(stderr, "fake_ddk: AddOperator\n");
  switch (type) {
...
int Graph::SetInputsOutputs(
  ...
  fprintf(stderr, "fake_ddk: SetInputsOutputs\n");
  int i;
...
Graph::Graph() {
  fprintf(stderr, "fake_ddk: Graph()\n");
  fakedevice_graph_ = new fakedevice_nn_graph_t();
  if ((nullptr == fakedevice_graph_)) {
    fprintf(stderr, "fake_ddk: create graph is fail!\n");
  }
