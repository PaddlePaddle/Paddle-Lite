...
static void UnpackActivations(core::Model* model,
  ...
  NNADAPTER_VLOG(0) << "fuse_code: " << *fuse_code;
  if (*fuse_code == NNADAPTER_FUSED_RELU ||
      *fuse_code == NNADAPTER_FUSED_RELU6) {
    *fuse_code = NNADAPTER_FUSED_NONE;
    NNAdapterOperationType relu_type = NNADAPTER_RELU;
    if (*fuse_code == NNADAPTER_FUSED_RELU6) relu_type = NNADAPTER_RELU6;
    // Insert a relu6 operation
    auto act_output_operand =
        InsertUnaryOperation(model, output_operand, relu_type);
