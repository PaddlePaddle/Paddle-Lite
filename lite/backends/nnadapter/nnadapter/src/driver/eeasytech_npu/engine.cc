...
int Program::BuildFromCache(core::Cache* cache) {
  ...
  NNADAPTER_LOG(INFO) << "lombo BuildFromCache.";
...
int Program::Execute(uint32_t input_count,
  ...
  for (uint32_t i = 0; i < output_count; i++) {
    ...
    // TODO(hong19860320) Get the dimensions of the outputs from eeasynpu_ddk
  ...
  NNADAPTER_CHECK_EQ(execution_->GetOutputs(output_info), eeasy::nn::EZ_SUCCESS);
