...
int ConvertPool2D(Converter* converter, core::Operation* operation) {
  ...
  attr.round_type = ceil_mode ? eeasy::nn::RoundType::ROUND_CEIL
                              : eeasy::nn::RoundType::ROUND_FLOOR;
  std::vector<std::shared_ptr<eeasy::nn::Tensor>> input_tensors = {input_tensor};
  std::vector<std::shared_ptr<eeasy::nn::Tensor>> output_tensors = {output_tensor};
  if (operation->type == NNADAPTER_AVERAGE_POOL_2D) {
    attr.pool_type = eeasy::nn::PoolType::POOLING_AVG;
  } else if (operation->type == NNADAPTER_MAX_POOL_2D) {
    attr.pool_type = eeasy::nn::PoolType::POOLING_MAX;
  } else {
  ...
  converter->AddOperator(
      eeasy::nn::OperatorType::POOL, input_tensors, output_tensors, &attr);
