...
int OpenDevice(void** device) {
  ...
  if (!d) {
    *device = nullptr;
    NNADAPTER_LOG(FATAL) << "Failed to open device for cambricon_mlu.";
...
int CreateContext(void* device,
  ...
  if (!c) {
    *context = nullptr;
    NNADAPTER_LOG(FATAL) << "Failed to create context for cambricon_mlu.";
    return NNADAPTER_OUT_OF_MEMORY;
  }
...
int CreateProgram(void* context,
                  ...
                  core::Cache* cache,
                  void** program) {
  NNADAPTER_LOG(INFO) << "Create program for cambricon_mlu.";
...
void DestroyProgram(void* program) {
  if (program) {
    NNADAPTER_LOG(INFO) << "Destroy program for cambricon_mlu.";
...
NNADAPTER_EXPORT nnadapter::driver::Device NNADAPTER_AS_SYM2(DEVICE_NAME) = {
    ...
    .version = 1,
    .open_device = nnadapter::cambricon_mlu::OpenDevice,
