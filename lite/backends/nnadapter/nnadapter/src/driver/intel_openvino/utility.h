...
#define INTEL_OPENVINO_SELECT_DEVICE_NAMES \
  "INTEL_OPENVINO_SELECT_DEVICE_NAMES"
...
template<typename T>
std::shared_ptr<OutputNode> AddConstOutputNode(std::vector<size_t> dimensions, std::vector<T> values) {
  NNADAPTER_LOG(FATAL) << "Unable to add const output node with type " << typeid(T).name();
  return nullptr;
}
