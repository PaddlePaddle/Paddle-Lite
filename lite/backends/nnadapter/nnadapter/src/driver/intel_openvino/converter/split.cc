...
int ConvertSplit(Converter* converter, core::Operation* operation) {
  ...
  std::shared_ptr<Operator> split_op;
  if (split_count == 1) {
    split_op = std::make_shared<default_opset::Split>(
        *input_tensor, *axis_tensor, split[0]);
  } else {
    auto sections_tensor =
        converter->AddConstantTensor<int32_t>({split_count}, split);
    split_op = std::make_shared<default_opset::VariadicSplit>(
        *input_tensor, *axis_tensor, *sections_tensor);
  }
  NNADAPTER_CHECK(split_op);
