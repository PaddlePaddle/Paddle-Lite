...
int OpenDevice(void** device) {
  ...
  if (!d) {
    *device = nullptr;
    NNADAPTER_LOG(FATAL) << "Failed to open device for qualcomm_qnn.";
...
int CreateContext(void* device,
  ...
  if (!c) {
    *context = nullptr;
    NNADAPTER_LOG(FATAL) << "Failed to create context for qualcomm_qnn.";
    return NNADAPTER_OUT_OF_MEMORY;
  }
...
int CreateProgram(void* context,
                  ...
                  core::Cache* cache,
                  void** program) {
  NNADAPTER_LOG(INFO) << "Create program for qualcomm_qnn.";
...
void DestroyProgram(void* program) {
  if (program) {
    NNADAPTER_LOG(INFO) << "Destroy program for qualcomm_qnn.";
...
NNADAPTER_EXPORT nnadapter::driver::Device NNADAPTER_AS_SYM2(DEVICE_NAME) = {
    ...
    .version = 1,
    .open_device = nnadapter::qualcomm_qnn::OpenDevice,
    .close_device = nnadapter::qualcomm_qnn::CloseDevice,
    .create_context = nnadapter::qualcomm_qnn::CreateContext,
    .destroy_context = nnadapter::qualcomm_qnn::DestroyContext,
    .validate_program = 0,
    .create_program = nnadapter::qualcomm_qnn::CreateProgram,
    .destroy_program = nnadapter::qualcomm_qnn::DestroyProgram,
    .execute_program = nnadapter::qualcomm_qnn::ExecuteProgram,
};
