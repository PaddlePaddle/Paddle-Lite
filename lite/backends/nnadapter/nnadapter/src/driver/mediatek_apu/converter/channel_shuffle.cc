...
int ConvertChannelShuffle(Converter* converter, core::Operation* operation) {
  ...
  auto group_index = converter->AddInt32ConstantOperand(group);
  auto dim_index = converter->AddInt32ConstantOperand(3);  // NHWC
  auto output_index = converter->ConvertOperand(output_operand);
  NNADAPTER_CHECK_EQ(
      converter->AddOperation(NEURON_CHANNEL_SHUFFLE,
                              {input_index, group_index, dim_index},
                              {output_index}),
      NEURON_NO_ERROR);
