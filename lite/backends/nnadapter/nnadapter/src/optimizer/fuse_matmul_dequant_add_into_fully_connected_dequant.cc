...
class MatMulDequantAddFuser : public PatternMatcher {
 public:
  explicit MatMulDequantAddFuser(bool restrict_2d_input)
...
void MatMulDequantAddFuser::BuildPattern() {
  // Operation patterns
  auto matmul_pattern =
  ...
  auto dequant_pattern = CreatePattern("dequant", NNADAPTER_DEQUANTIZE);
  // Operand patterns
  ...
  auto matmul_transpose_y_pattern =
      CreatePattern("matmul_transpose_y")
          ...
          ->IsConstantOperand()
          ->IsIntermediate();
  auto dequant_input_pattern =
      CreatePattern("dequant_input")
          ->IsOperationInputOperand(NNADAPTER_DEQUANTIZE, 0)
          ->IsIntermediate();
  auto dequant_output_pattern =
      CreatePattern("dequant_output")->IsIntermediate();
  ...
  std::vector<Pattern*> add_input_patterns{
      dequant_output_pattern, add_input_pattern, add_fuse_code_pattern};

  matmul_input_patterns >> *matmul_pattern >> *dequant_input_pattern;
  *dequant_input_pattern >> *dequant_pattern >> *dequant_output_pattern;
  add_input_patterns >> *add_pattern >> *add_output_pattern;
}

bool MatMulDequantAddFuser::HandleMatchedResults(
  ...
  fully_connected_operation->output_operands = {dequant_input_operand};
  dequant_operation->output_operands = {add_output_operand};
  // The matched intermediate operands and operations will be deleted only when
  // it returns true.
  return true;
}

NNADAPTER_EXPORT void FuseMatMulDequantAddIntoFullyConnectedDequant(
    core::Model* model, bool restrict_2d_input) {
  NNADAPTER_VLOG(5) << "Apply MatMulDequantAddFuser";
  bool stop;
  do {
    MatMulDequantAddFuser mat_mul_add_fuser(restrict_2d_input);
