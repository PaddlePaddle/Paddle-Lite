...
TEST(cl_test, target_wrapper_image_test) {
  ...
  auto *d_image = static_cast<cl::Image2D *>(
      TargetWrapperCL::MallocImage<float>(image_shape, image_pitch));
  // Map/Unmap test
...
TEST(cl_test, tensor_image2d_test) {
  const size_t img_h = 32;
  const size_t img_w = 28;
  const std::array<size_t, 2> image_shape{img_w, img_h};
  std::array<size_t, 2> image_pitch;
  auto *d_image = static_cast<cl::Image2D *>(
      TargetWrapperCL::MallocImage<float>(image_shape, image_pitch));
  LOG(INFO) << "image_pitch[0]:" << image_pitch[0];
  LOG(INFO) << "image_pitch[1]:" << image_pitch[1];
  // Map/Unmap test
  auto *h_image = static_cast<float *>(
      TargetWrapperCL::MapImage(d_image, image_shape, &image_pitch));
  // row_pitch = 448 = 28 * 4 (RGBA: 4 floats) * 4 (float in bytes)
