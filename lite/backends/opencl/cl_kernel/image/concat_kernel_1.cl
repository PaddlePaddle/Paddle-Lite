...
#define CHECK_IDX_UNALIGN                                                     \
  int c_blk_idx = get_global_id(0);                                           \
  int w_idx = get_global_id(0);                                               \
  int nh_idx = get_global_id(1);                                              \
  if (c_blk_idx >= output_shape.y ||                                          \
      w_idx >= output_shape.w ||                                              \
      nh_idx >= output_shape.x * output_shape.z) {                            \
    return;                                                                   \
...
__kernel void Concat2InputAxis1UnAligned(__read_only image2d_t input0,
                                         ...
                                         int4 output_shape) {
  CHECK_IDX_UNALIGN
  int start_c_reamin0  = 0;
  int start_c_blk_idx0 = 0;
  int start_c_reamin1  = input_shape0.y % 4;
  int start_c_blk_idx1 = input_shape0.y / 4;
...
__kernel void Concat3InputAxis1UnAligned(__read_only image2d_t input0,
  ...
  int start_c_reamin1  = input_shape0.y % 4;
  int start_c_blk_idx1 = input_shape0.y / 4;
  int start_c_reamin2  = start_c_reamin1 + input_shape1.y % 4 - 4;
  int start_c_blk_idx2 = (input_shape0.y + input_shape1.y) / 4;
...
__kernel void Concat4InputAxis1UnAligned(__read_only image2d_t input0,
                                         __read_only image2d_t input1,
                                         __read_only image2d_t input2,
                                         __read_only image2d_t input3,
                                         __read_write image2d_t output,
  ...
  int start_c_reamin2  = start_c_reamin1 + input_shape1.y % 4 - 4;
  int start_c_blk_idx2 = (input_shape0.y + input_shape1.y) / 4;
  int start_c_reamin3  = start_c_reamin2 + input_shape2.y % 4 - 4;
  int start_c_blk_idx3 = (input_shape0.y + input_shape1.y + input_shape2.y) / 4;
