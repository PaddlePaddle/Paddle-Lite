...
int SoftmaxConverter(cvt_ctx_type* ctx, lite::OpLite* op) {
  auto scope = op->scope();
  ...
  auto softmax_node = ctx->AddNode<ge::op::Softmax>(out_var_name);
  CHECK(ctx->HasNode(x_var_name));
  softmax_node->set_input_x(*ctx->GetNode(x_var_name));
...
REGISTER_NPU_BRIDGE(softmax, paddle::lite::npu::bridges::SoftmaxConverter);
