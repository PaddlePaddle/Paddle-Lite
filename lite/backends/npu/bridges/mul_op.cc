...
int MulConverter(cvt_ctx_type* ctx, lite::OpLite* op) {
  auto scope = op->scope();
  ...
  if (ctx->HasNode(x_var_name)) {
    ...
    x_const_node->set_attr_value(CvtTensor(x, {m, k}));
  ...
  if (ctx->HasNode(y_var_name)) {
    ...
    y_const_node->set_attr_value(CvtTensor(y, {k, n}));
...
REGISTER_NPU_BRIDGE(mul, paddle::lite::npu::bridges::MulConverter);
