...
int ShuffleChannelConverter(cvt_ctx_type* ctx, lite::OpLite* op) {
  auto scope = op->scope();
  ...
  auto shuffle_channel_node =
      ctx->AddNode<ge::op::ShuffleChannel>(out_var_name);

  CHECK(ctx->HasNode(x_var_name));
  shuffle_channel_node->set_input_x(*ctx->GetNode(x_var_name));
...
REGISTER_NPU_BRIDGE(shuffle_channel,
                    paddle::lite::npu::bridges::ShuffleChannelConverter);
