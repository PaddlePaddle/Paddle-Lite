  ...
  [desc_ setDepth:1];
	...
	switch (tensor_dim_.size()) {
		...
		case 1: {
			...
			//GPU上的arraylength方向使用3549大小为(3549+3)/4=888 width方向使用36大小为36 height方向使用1大小为1
			//eg12:tensor={1,24}转换为dim={1,1,1,24}
	...
	if (transpose_ == transpose_nhwc) {
		...
		//使用场景：OP运算时使用的参数 tensor传入转为texture使用 目前tensor.size都是1或2 其他情况还未遇到
		switch (tensor_dim_.size()) {
			case 4: {
			...
			for (int i0 = 0; i0 < n; i0++) {
				for (int i1 = 0; i1 < p; i1++) {
					for (int i2 = 0; i2 < h; i2++) {
						for (int i3 = 0; i3 < w; i3++) {
							for (int k = 0; k < 4; k++) {
								auto dx = i0 * p * h * w * 4 + i1 * h * w * 4 + i2 * w * 4 + i3 * 4 + k;
