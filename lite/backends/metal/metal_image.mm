...
void MetalImage::InitTexture() {
    ...
    switch (tensor_dim_.size()) {
        case 4: {
            ...
            for_each(transpose.begin(), transpose.end(),
                     [&](int i) -> void { dim.emplace_back(pad_to_four_dim_[i]); });
        } break;
        case 3:
        case 2:
        case 1: {
            ...
            // GPU上的arraylength方向使用24 大小为(24+3)/4=6  width方向使用1大小为1
            // height方向使用1大小为1
...
void MetalImage::CopyToNCHW(P* dst) const {
    ...
    if (precision_type_ == METAL_PRECISION_TYPE::FLOAT && std::is_same<P, float>::value) {
        ...
        for (int i = 0; i < array_length_; ++i) {
            ...
            [image_ getBytes:(p)
                 bytesPerRow:(bytes_per_row)bytesPerImage:(bytes_per_image)fromRegion
                            :(region)mipmapLevel:(0)slice:static_cast<NSUInteger>(i)];
        }
                ...
                for (int i1 = 0; i1 < C; ++i1) {
                    for (int i2 = 0; i2 < H; ++i2) {
                        for (int i3 = 0; i3 < W; ++i3) {
                            ...
                            std::vector<int> jg = {ig[transpose_[0]], ig[transpose_[1]],
                                                   ig[transpose_[2]], ig[transpose_[3]]};
