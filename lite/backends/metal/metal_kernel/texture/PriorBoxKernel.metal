...
kernel void prior_box(texture2d_array<ftype, access::read> inTexture
                      [[texture(0)]],
                      texture2d_array<ftype, access::write> outBoxTexture
                      [[texture(1)]],
                      texture2d_array<ftype, access::write> varianceTexture
                      [[texture(2)]],
  ...
  if (gid.z < param.aspecRatiosSize) {
    ...
    outBoxTexture.write(ftype4(res), gid.xy, gid.z);
    ...
    if (param.maxSizeSize > 0) {
      ...
      outBoxTexture.write(ftype4(max_box), gid.xy, gid.z);
    }
  }

  if (gid.z < param.numPriors) {
    varianceTexture.write(ftype4(variances[0]), gid.xy, gid.z);
  }
}

kernel void prior_box_MinMaxAspectRatiosOrder(
    texture2d_array<ftype, access::read> inTexture [[texture(0)]],
    texture2d_array<ftype, access::write> outBoxTexture [[texture(1)]],
    texture2d_array<ftype, access::write> varianceTexture [[texture(2)]],
    const device float *aspect_ratios [[buffer(0)]],
    constant PriorBoxMetalParam &param [[buffer(1)]],
    const device float4 *variances [[buffer(2)]],
    uint3 gid [[thread_position_in_grid]]) {
  ...
  if (gid.z < param.numPriors) {
    ...
    varianceTexture.write(ftype4(variances_output), gid.xy, gid.z);
  }
}
