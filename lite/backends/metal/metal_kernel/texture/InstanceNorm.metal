...
kernel void instance_norm(texture2d_array<float, access::read> in
                          [[texture(0)]],
...
kernel void instance_norm_half(texture2d_array<half, access::read> in
                               [[texture(0)]],
  ...
  for (ushort xIndex = gid.x; xIndex < width; xIndex += tg_size.x) {
    for (ushort yIndex = gid.y; yIndex < height; yIndex += tg_size.y) {
      ...
      out.write(
          half4((val - mean) * multiplier + 0), uint2(xIndex, yIndex), gid.z);
    }
  }
}
