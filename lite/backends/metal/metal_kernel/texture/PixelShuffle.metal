...
kernel void pixel_shuffle(texture2d_array<float, access::sample> inTexture
                          [[texture(0)]],
  ...
  for (int i = 0; i < 4; i++) {
    int c = gid.z * 4 + i;
    int inC = c * upscale_factor * upscale_factor +
              (gid.y % upscale_factor) * upscale_factor +
...
kernel void pixel_shuffle_half(texture2d_array<half, access::sample> inTexture
                               [[texture(0)]],
