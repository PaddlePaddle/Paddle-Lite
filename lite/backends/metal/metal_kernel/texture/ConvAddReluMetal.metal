...
inline ftype4 get_bias(uint3 gid, constant ElementwiseAddParam &addParam, texture2d_array<ftype, access::sample> biasTexture) {
...
kernel void conv_3x3(texture2d_array<ftype, access::sample> inTexture [[texture(0)]],
		...
		for (uint i = 0; i < input_arr_size; ++i) {
				...
				for (int j = 0; j < 9; ++j) {
						ftype4 weight_x = weights[weithTo + 0 * kernelHXW * input_arr_size + j * input_arr_size + i];
						output.x += dot(input[j], weight_x);

						ftype4 weight_y = weights[weithTo + 1 * kernelHXW * input_arr_size + j * input_arr_size + i];
						output.y += dot(input[j], weight_y);

						ftype4 weight_z = weights[weithTo + 2 * kernelHXW * input_arr_size + j * input_arr_size + i];
						output.z += dot(input[j], weight_z);

						ftype4 weight_w = weights[weithTo + 3 * kernelHXW * input_arr_size + j * input_arr_size + i];
...
kernel void depthwise_conv_5x5(texture2d_array<ftype, access::sample> inTexture [[texture(0)]],
		...
		for (int i = 0; i < 5; i++) {
				for (int j = 0; j < 5; j++) {
						ftype4 input = inTexture.sample(sample, float2(posInInput.x + j - 2,    posInInput.y + i - 2), output_slice);
...
kernel void depthwise_conv_3x3_winograd(texture2d_array<half, access::read> inTexture [[texture(0)]],
																				texture2d_array<half, access::sample> biasTexture [[texture(1)]],
																				texture2d_array<half, access::write> outTexture [[texture(2)]],
																				constant MetalConvParam &param [[buffer(0)]],
																				const device half4x4 *weights [[buffer(1)]],
