...
inline ftype4 get_yolo_box(const device ftype *x_data,
    ...
    box[2] = exp(x_data[box_idx + x_stride * 2]) * anchors_data[2 * anchor_idx] * img_width / x_size;
    box[3] = exp(x_data[box_idx + x_stride * 3]) * anchors_data[2 * anchor_idx + 1] * img_height / x_size;
    return box;
}
...
kernel void yolo_box(device ftype *x_data[[buffer(0)]],
    ...
    for (int imgsize_idx = 0; imgsize_idx < imgsize_num; imgsize_idx++) {
        const int obj_idx = (imgsize_idx * anchor_num + anchor_idx) * anchor_stride + 4 * x_stride + hw_idx;
        ...
        // get yolo box
        int box_idx = (imgsize_idx * anchor_num + anchor_idx) * anchor_stride + 0 * x_stride + hw_idx;
        ftype4 box = get_yolo_box(x_data, anchors_data, l, k, anchor_idx,
                                  x_h, x_size, box_idx, x_stride,
                                  img_height, img_width, scale, bias_value);
        ...
        const int label_idx = (imgsize_idx * anchor_num + anchor_idx) * anchor_stride + 5 * x_stride + hw_idx;
