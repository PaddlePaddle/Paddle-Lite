...
kernel void grid_sampler(texture2d_array<float, access::sample> inTexture
                         [[texture(0)]],
                         texture2d_array<float, access::write> outTexture
                         [[texture(1)]],
                         texture2d_array<float, access::read> gridTexture
                         [[texture(2)]],
  ...
  float4 output =
      input0 * (x0 + 1 - x) * (y0 + 1 - y) + input1 * (x - x0) * (y0 + 1 - y) +
      input2 * (x0 + 1 - x) * (y - y0) + input3 * (x - x0) * (y - y0);

  outTexture.write(output, gid.xy, gid.z);
}

kernel void grid_sampler_half(texture2d_array<half, access::sample> inTexture
                              [[texture(0)]],
  ...
  float4 input3 =
      float4(inTexture.sample(sample, float2(x0 + 1, y0 + 1), gid.z));
