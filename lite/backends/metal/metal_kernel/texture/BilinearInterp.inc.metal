...
kernel void FUNC(bilinear_interp, P)(texture2d_array<P, access::read> input [[texture(0)]],
    ...
    if ((input.get_width() == output.get_width()) && (input.get_height() == output.get_height())) {
        ...
        // 中心像素，align_delta在CPU中计算后传入Kernel，可选值（0.0：不中心对齐，0.5：中心对齐）
        P w = (gid.x + pm.align_delta) * pm.ratio_w - pm.align_delta;
        P h = (gid.y + pm.align_delta) * pm.ratio_h - pm.align_delta;
        uint w0 = w, h0 = h;            // 中心左侧x坐标，下侧y坐标
        uint w1 = w0 + 1, h1 = h0 + 1;  // 中心右侧x坐标，上侧y坐标
        // 分别获取X方向、Y方向两个位置与中心位置的距离权重
        ...
        // 根据四个位置与中心位置的距离权重，对四个数值加权获取插值结果
