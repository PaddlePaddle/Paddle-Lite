...
kernel void instance_norm_relu(texture2d_array<float, access::read> in
                               [[texture(0)]],
                               texture2d_array<float, access::write> out
                               [[texture(1)]],
                               constant MetalInstanceNormReluParam &param
                               [[buffer(0)]],
  ...
  for (ushort xIndex = gid.x; xIndex < width; xIndex += tg_size.x) {
    for (ushort yIndex = gid.y; yIndex < height; yIndex += tg_size.y) {
      ...
      out.write(param.hasReluOp == 1 ? fmax(val, 0) : val,
                uint2(xIndex, yIndex),
                gid.z);
    }
  }
...
kernel void instance_norm_relu_half(
  ...
  for (ushort xIndex = gid.x; xIndex < width; xIndex += tg_size.x) {
    for (ushort yIndex = gid.y; yIndex < height; yIndex += tg_size.y) {
      ...
      val = (val - mean) * multiplier + 0;
      out.write(half4(param.hasReluOp == 1 ? fmax(val, 0) : val),
                uint2(xIndex, yIndex),
                gid.z);
    }
  }
