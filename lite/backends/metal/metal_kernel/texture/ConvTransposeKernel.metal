...
kernel void conv_transpose3x3_stride2x2_half(texture2d_array<half, access::sample> inTexture [[texture(0)]],
...
kernel void depthwise_conv_transpose3x3_stride2x2_half(texture2d_array<half, access::sample> inTexture [[texture(0)]],
                                             texture2d_array<half, access::sample> biasTexture [[texture(1)]],
                                             texture2d_array<half, access::write> outTexture [[texture(2)]],
                                             constant MetalConvTransposeParam &param [[buffer(0)]],
                                             const device half4 *weights [[buffer(1)]],
...
kernel void conv_transpose3x3_stride2x2_quadruple_half(texture2d_array<half, access::sample> inTexture [[texture(0)]],
                                             texture2d_array<half, access::sample> biasTexture [[texture(1)]],
                                             texture2d_array<half, access::write> outTexture [[texture(2)]],
                                             constant MetalConvTransposeParam &param [[buffer(0)]],
                                             const device half4 *weights [[buffer(1)]],
    ...
    if (tx >= inTexture.get_width() ||
        ty >= inTexture.get_height() ||
        gid.z >= outTexture.get_array_size()) {
        return;
    }
    constexpr sampler sample(coord::pixel, filter::nearest, address::clamp_to_zero);
    ...
    half4 output1, output2, output3, output4, output5, output6, output7, output8, output9, output10, output11, output12, output13, output14, output15, output16;
...
kernel void conv_transpose3x3_caculate_half(texture2d_array<half, access::read> inTexture [[texture(0)]],
                                         texture2d_array<half, access::write> outTexture [[texture(1)]],
...
kernel void conv_transpose3x3_stride2_shift_left_half(texture2d_array<half, access::read> inTexture [[texture(0)]],
                                         texture2d_array<half, access::write> outTexture [[texture(1)]],
                                         ushort3 gid [[thread_position_in_grid]]) {
    ushort in_x = 3 * gid.x, out_x = 2 * gid.x, y = 3 * gid.y;
    if (out_x >= outTexture.get_width() ||
        y >= outTexture.get_height() ||
        gid.z >= outTexture.get_array_size()) {
        return;
    }
    //crop right
//    for (ushort h = 0; h < 3; ++h) {
//        ushort in_out_y = y + h;
//        half4 add = in_x > 0 ? inTexture.read(uint2(in_x - 1, in_out_y), gid.z) : half4(0);
//        half4 pix1 = inTexture.read(uint2(in_x, in_out_y), gid.z);
//        half4 pix2 = inTexture.read(uint2(in_x + 1, in_out_y), gid.z);
//        outTexture.write(pix1 + add, uint2(out_x, in_out_y), gid.z);
//        outTexture.write(pix2, uint2(out_x + 1, in_out_y), gid.z);
//    }
    
    //crop left
    for (ushort h = 0; h < 3; ++h) {
        ushort in_out_y = y + h;
        half4 add = (in_x + 3 >= inTexture.get_width()) ? half4(0) : inTexture.read(uint2(in_x + 3, in_out_y), gid.z);
...
kernel void conv_transpose3x3_stride2_shift_top_half(texture2d_array<half, access::read> inTexture [[texture(0)]],
                                                 texture2d_array<half, access::write> outTexture [[texture(1)]],
                                                 ushort3 gid [[thread_position_in_grid]]) {
    ushort in_y = 3 * gid.y, out_y = 2 * gid.y, x = 2 * gid.x;
    if (out_y >= outTexture.get_height() ||
        x >= outTexture.get_width() ||
        gid.z >= outTexture.get_array_size()) {
        return;
    }
...
//        ushort in_out_x = x + w;
//        half4 add = in_y > 0 ? inTexture.read(uint2(in_out_x, in_y - 1), gid.z) : half4(0);
//        half4 pix1 = inTexture.read(uint2(in_out_x, in_y), gid.z);
//        half4 pix2 = inTexture.read(uint2(in_out_x, in_y + 1), gid.z);
//        outTexture.write(pix1 + add, uint2(in_out_x, out_y), gid.z);
//        outTexture.write(pix2, uint2(in_out_x, out_y + 1), gid.z);
//    }
    
    //crop top
    for (ushort w = 0; w < 2; ++w) {
        ushort in_out_x = x + w;
        half4 add = (in_y + 3 >= inTexture.get_height()) ? half4(0) : inTexture.read(uint2(in_out_x, in_y + 3), gid.z);
...
kernel void conv_transpose4x4_caculate_half(texture2d_array<half, access::read> inTexture [[texture(0)]],
                                            texture2d_array<half, access::write> outTexture [[texture(1)]],
                                            const device half4 *weights [[buffer(0)]],
                                            uint3 gid [[thread_position_in_grid]]) {
    if (gid.x >= inTexture.get_width() ||
...
kernel void conv_transpose4x4_stride2_shift_left_half(texture2d_array<half, access::read> inTexture [[texture(0)]],
                                                      texture2d_array<half, access::write> outTexture [[texture(1)]],
                                                      ushort3 gid [[thread_position_in_grid]]) {
...
kernel void conv_transpose4x4_stride2_shift_top_half(texture2d_array<half, access::read> inTexture [[texture(0)]],
                                                     texture2d_array<half, access::write> outTexture [[texture(1)]],
                                                     ushort3 gid [[thread_position_in_grid]]) {
