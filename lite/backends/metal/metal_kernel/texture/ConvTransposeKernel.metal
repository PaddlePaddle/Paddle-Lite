...
kernel void conv_transpose3x3_stride2x2_quadruple_half(
  ...
  half4 output1, output2, output3, output4, output5, output6, output7, output8,
  ...
  if (param.hasAddOp == 1) {
    ...
    if (addParam.addByChannel == 1) {
      ...
      output3 = getBiasHalf(uint3(ox, oy + 1, gid.z), addParam, biasTexture);
      output4 =
          getBiasHalf(uint3(ox + 1, oy + 1, gid.z), addParam, biasTexture);

      output5 = getBiasHalf(uint3(ox + 2, oy, gid.z), addParam, biasTexture);
      output6 = getBiasHalf(uint3(ox + 3, oy, gid.z), addParam, biasTexture);
      output7 =
          getBiasHalf(uint3(ox + 2, oy + 1, gid.z), addParam, biasTexture);
      output8 =
          getBiasHalf(uint3(ox + 3, oy + 1, gid.z), addParam, biasTexture);

      output9 = getBiasHalf(uint3(ox, oy + 2, gid.z), addParam, biasTexture);
      output10 =
          getBiasHalf(uint3(ox + 1, oy + 2, gid.z), addParam, biasTexture);
      output11 = getBiasHalf(uint3(ox, oy + 3, gid.z), addParam, biasTexture);
      output12 =
          getBiasHalf(uint3(ox + 1, oy + 3, gid.z), addParam, biasTexture);

      output13 =
          getBiasHalf(uint3(ox + 2, oy + 2, gid.z), addParam, biasTexture);
      output14 =
          getBiasHalf(uint3(ox + 3, oy + 2, gid.z), addParam, biasTexture);
      output15 =
          getBiasHalf(uint3(ox + 2, oy + 3, gid.z), addParam, biasTexture);
      output16 =
          getBiasHalf(uint3(ox + 3, oy + 3, gid.z), addParam, biasTexture);
    }
  } else {
    output1 = half4(0);
    output2 = half4(0);
...
kernel void conv_transpose3x3_caculate_half(
  ...
  for (ushort i = 0; i < input_array_size; ++i) {
    ...
    for (ushort j = 0; j < 9; ++j) {
      w = weights[weightTo + 0 * 9 * input_array_size + j * input_array_size +
                  i];
      results[j].x += dot(float4(input), float4(w));
      w = weights[weightTo + 1 * 9 * input_array_size + j * input_array_size +
                  i];
      results[j].y += dot(float4(input), float4(w));
      w = weights[weightTo + 2 * 9 * input_array_size + j * input_array_size +
                  i];
      results[j].z += dot(float4(input), float4(w));
      w = weights[weightTo + 3 * 9 * input_array_size + j * input_array_size +
                  i];
...
kernel void conv_transpose4x4_caculate_half(
  ...
  for (ushort i = 0; i < input_array_size; ++i) {
    ...
    for (ushort j = 0; j < 16; ++j) {
      w = weights[weightTo + 0 * 16 * input_array_size + j * input_array_size +
                  i];
      results[j].x += dot(float4(input), float4(w));
      w = weights[weightTo + 1 * 16 * input_array_size + j * input_array_size +
                  i];
      results[j].y += dot(float4(input), float4(w));
      w = weights[weightTo + 2 * 16 * input_array_size + j * input_array_size +
                  i];
      results[j].z += dot(float4(input), float4(w));
      w = weights[weightTo + 3 * 16 * input_array_size + j * input_array_size +
                  i];
      results[j].w += dot(float4(input), float4(w));
    }
  }
...
kernel void conv_transpose4x4_stride2_shift_left_half(
  ...
  for (ushort h = 0; h < 4; ++h) {
    ...
    if (in_x == 0) {
      ...
      half4 pix2 =
          inTexture.read(uint2(inTexture.get_width() - 2, in_out_y), gid.z);
      outTexture.write(pix1, uint2(0, in_out_y), gid.z);
      outTexture.write(
          pix2, uint2(outTexture.get_width() - 1, in_out_y), gid.z);
    } else {
...
kernel void conv_transpose4x4_stride2_shift_top_half(
  ...
  for (ushort w = 0; w < 2; ++w) {
    ...
    if (in_y == 0) {
      ...
      half4 pix2 =
          inTexture.read(uint2(in_out_x, inTexture.get_height() - 2), gid.z);
      outTexture.write(pix1, uint2(in_out_x, 0), gid.z);
      outTexture.write(
          pix2, uint2(in_out_x, outTexture.get_height() - 1), gid.z);
    } else {
