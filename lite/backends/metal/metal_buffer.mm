  ...
  if (dim_.size() == 4) {
    ...
    // padding
      ...
      if (data_layout_ == DataLayout::kNCHW) {
        ...
        int padNCount = static_cast<int>((((dim_[0] + 3) / 4) * 4 * paddedC) * dim_[2] * dim_[3]);
        ...
        buffer_ = [backend newDeviceBuffer:new_length access:METAL_ACCESS_FLAG::CPUReadWrite];
        ...
        int padNCount = static_cast<int>((((dim_[0] + 3) / 4) * 4 * paddedC) * dim_[1] * dim_[2]);
        void *convertedPointer = TargetWrapperMetal::Malloc(padNCount * precision_size_);
        // pad 0.0f
        if (precision_ == METAL_PRECISION_TYPE::FLOAT) {
          auto data_src = (float *)rawdata_;
...
void MetalBuffer::Convert2NHWC() {
  ...
  void *new_pointer = TargetWrapperMetal::Malloc(static_cast<size_t>(precision_size_ * tensor_dim_.production()));
