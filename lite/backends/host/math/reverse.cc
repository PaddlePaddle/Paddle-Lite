...
void reverse_func(const lite::Tensor *input,
  ...
  /*
      for (int i = 0; i < input_ddim.count(0, input_ddim.size()); i++)
        out_ptr[i] = in_ptr[i];

      for (int i = axis.size() - 1; i >= 0; i--)
      {
        int a = input_ddim.count(axis[i], input_ddim.size());
        int b = input_ddim.count(axis[i] + 1, input_ddim.size());
        ReverseAB(out_ptr ,input_ddim.count(0, input_ddim.size()) ,a, b);
      }
  */
