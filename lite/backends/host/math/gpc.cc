...
static void add_left(polygon_node *p, double x, double y) {
  CHECK(p) << "Input polygon node is nullptr.";
...
void gpc_vertex_create(edge_node *e, int p, int s, double x, double y) {
  CHECK(e) << "Input edge node is nullptr.";
...
static bbox *create_contour_bboxes(gpc_polygon *p) {
  ...
  CHECK(box) << "Failed to malloc box memory.";
...
void gpc_polygon_clip(gpc_op op,
  ...
  CHECK(sbt) << "Failed to malloc scanbeam table memory.";
  ...
  while (scanbeam < sbt_entries) {
    ...
    CHECK(aet) << "Edge node AET is nullptr.";
