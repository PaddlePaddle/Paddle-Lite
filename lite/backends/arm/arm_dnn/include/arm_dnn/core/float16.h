...
// Use ARM_DNN_ALIGNED(2) to ensure that each float16 will be allocated
// and aligned at least on a 2-byte boundary, which leads to efficient
// memory access of float16 struct and also makes float16 compatible
// with CUDA half, ARM float16_t data types.
struct ARM_DNN_ALIGN(2) float16 {
 public:
  uint16_t x;
...
#if defined(__F16C__) && (ARM_DNN_ARCH_X86 || ARM_DNN_ARCH_X86_64)
    x = _cvtss_sh(val, 0);
...
template <>
struct is_pod<arm_dnn::float16> {
  static const bool value = is_trivial<arm_dnn::float16>::value &&
                            is_standard_layout<arm_dnn::float16>::value;
};

template <>
struct is_floating_point<arm_dnn::float16>
    : std::integral_constant<bool,
                             std::is_same<arm_dnn::float16,
                                          typename std::remove_cv<
                                              arm_dnn::float16>::type>::value> {
};
template <>
struct is_signed<arm_dnn::float16> {
  static const bool value = true;
};

template <>
struct is_unsigned<arm_dnn::float16> {
  static const bool value = false;
};

inline bool isnan(const arm_dnn::float16& a) { return arm_dnn::isnan(a); }

inline bool isinf(const arm_dnn::float16& a) { return arm_dnn::isinf(a); }

inline bool isfinite(const arm_dnn::float16& a) { return arm_dnn::isfinite(a); }

template <>
struct numeric_limits<arm_dnn::float16> {
  ...
  static arm_dnn::float16(min)() {
    return arm_dnn::raw_uint16_to_float16(0x400);
  }
  static arm_dnn::float16 lowest() {
    return arm_dnn::raw_uint16_to_float16(0xfbff);
  }
  static arm_dnn::float16(max)() {
    return arm_dnn::raw_uint16_to_float16(0x7bff);
  }
  static arm_dnn::float16 epsilon() {
    return arm_dnn::raw_uint16_to_float16(0x1400);
  }
  static arm_dnn::float16 round_error() { return arm_dnn::float16(0.5); }
  static arm_dnn::float16 infinity() {
    return arm_dnn::raw_uint16_to_float16(0x7c00);
  }
  static arm_dnn::float16 quiet_NaN() {
    return arm_dnn::raw_uint16_to_float16(0x7e00);
  }
  static arm_dnn::float16 signaling_NaN() {
    return arm_dnn::raw_uint16_to_float16(0x7e00);
  }
  static arm_dnn::float16 denorm_min() {
    return arm_dnn::raw_uint16_to_float16(0x1);
  }
};

inline arm_dnn::float16 abs(const arm_dnn::float16& a) {
  return arm_dnn::abs(a);
}
