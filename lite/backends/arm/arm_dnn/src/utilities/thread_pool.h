...
#ifdef ARM_DNN_WITH_THREAD_POOL
/* Support simple task
 * for (int i = 0; i < work_size; ++i)
 */
#define ARM_DNN_THREAD_POOL_SIMPLE_TASK_BEGIN(index, tid, work_size) \
  {                                                                  \
    std::pair<std::function<void(int, int)>, int> task;              \
    task.second = work_size;                                         \
  task.first = [&](int index, int tid) {
#define ARM_DNN_THREAD_POOL_SIMPLE_TASK_END()    \
  }                                              \
  ;                                              \
  arm_dnn::ThreadPool::Enqueue(std::move(task)); \
  }

/* Support common task
 * for (int i = start; i < end; i += step)
 */
#define ARM_DNN_THREAD_POOL_COMMON_TASK_BEGIN(index, tid, end, start, step) \
  {                                                                         \
...
#define ARM_DNN_THREAD_POOL_COMMON_TASK_END()    \
  }                                              \
  ;                                              \
  arm_dnn::ThreadPool::Enqueue(std::move(task)); \
  }
#elif defined(ARM_DNN_WITH_OMP)
#include <omp.h>
#define ARM_DNN_THREAD_POOL_SIMPLE_TASK_BEGIN(index, tid, work_size)   \
