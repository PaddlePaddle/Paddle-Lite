...
#include "arm_dnn/operators/nn_ops.h"
...
template <>
ARM_DNN_DLL_EXPORT Status relu<float16>(void* context,
                                        const float16* x_data,
                                        float16* y_data,
                                        size_t size) {
  auto ctx = reinterpret_cast<Context*>(context);
  ARM_DNN_CHECK(ctx);
#if ARM_DNN_ARM_WITH_FP16
  if (ctx->enable_arm_fp16())
    return kernels::relu_fp16_neon_x32(ctx, x_data, y_data, size);
#endif
  ARM_DNN_VLOG(5) << "relu<float16>() is not accelerated on the current "
                     "architecture! Only accelerated on ARM and ARM64 !";
  return kernels::relu<float16>(ctx, x_data, y_data, size);
}

ARM_DNN_DLL_EXPORT Status relu_qs8(void* context,
                                   const int8_t* x_data,
                                   float x_scale,
                                   int8_t* y_data,
                                   float y_scale,
  ...
  auto ctx = reinterpret_cast<Context*>(context);
  ARM_DNN_CHECK(ctx);
  ARM_DNN_CHECK_EQ(x_scale, y_scale);
  ARM_DNN_VLOG(5) << "relu_qs8() is not accelerated on the architecture!";
