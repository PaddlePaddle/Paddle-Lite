	...
	for (int j = 0; j < n - 1; j++)
	{
		...
		for (int temp = j+1; temp < n; temp++)
			if (std::abs(U[temp * n + j]) > std::abs(U[j_swap * n + j]))
				j_swap = temp;
		CHECK_GT(std::abs(U[j_swap * n + j]), 0) << "the input matrix is not invertible";
		if (j_swap != j)
		{
...
{
	T* temp_array = (T*)malloc(sizeof(T) * n * n);
	for (int i=0;i<n;i++)
		for (int j = 0; j < n; j++)
		{
			T temp = 0;
			for (int k = 0; k < n; k++)
				temp += U_1[i * n + k] * L_1[k * n + j];
...
void inverse_func(const lite::Tensor *input,
                 lite::Tensor *output) {
  ...
  InType *L = (InType *)malloc(sizeof(InType) * n *n);
  InType *U = (InType *)malloc(sizeof(InType) * n *n);
  InType *P = (InType *)malloc(sizeof(InType) * n *n);
