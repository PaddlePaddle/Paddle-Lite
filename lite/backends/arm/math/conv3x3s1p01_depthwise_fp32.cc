...
#define PARAM2                                                        \
  [w0] "w"(wr0), [w1] "w"(wr1), [w2] "w"(wr2), [bias_val] "r"(vbias), \
      [vmask] "r"(vmask), [right_pad_num] "r"(right_pad_num)
...
#define RIGHT_COMPUTE_S1                                                      \
  ...
  "vld1.32  {d18}, [%[din0_ptr]]    \n"                                       \
  "vld1.32  {d20-d21}, [%[din1_ptr]]!   \n"                                   \
  "vld1.32  {d22}, [%[din1_ptr]]    \n"                                       \
  "vld1.32  {d24-d25}, [%[din2_ptr]]!    \n"                                  \
  "vld1.32  {d26}, [%[din2_ptr]]    \n"                                       \
  "vld1.32  {d28-d29}, [%[din3_ptr]]!  \n"                                    \
  "vld1.32  {d30}, [%[din3_ptr]]    \n"                                       \
                                                                              \
    ...
    LITE_PARALLEL_BEGIN(c, tid, ch_in) {
      ...
      for (int i = 0; i < h_out; i += 4) {
        ...
        asm volatile(
            ...
            : PARAM1
            : PARAM2, [vsix] "w"(vsix), [vzero] "w"(vzero)
            : ASM_PARAM);
    ...
    LITE_PARALLEL_BEGIN(c, tid, ch_in) {
      ...
      for (int i = 0; i < h_out; i += 4) {
        ...
        asm volatile(INIT_S1 LEFT_COMPUTE_S1 LEFT_RESULT_S1_LEAKY_RELU
                     ...
                     : PARAM1
                     : PARAM2, [vscale] "w"(vscale), [vzero] "w"(vzero)
                     : ASM_PARAM);
        dout_ptr = dout_ptr + 4 * w_out;
      }
#else
    ...
    LITE_PARALLEL_BEGIN(c, tid, ch_in) {
      ...
      for (int i = 0; i < h_out; i += 4) {
        ...
        asm volatile(
            ...
            : PARAM1
            : PARAM2, [vsix] "w"(vsix), [remain] "r"(remain)
            : ASM_PARAM);
        dout_ptr = dout_ptr + 4 * w_out;
      }
#else
      for (int i = 0; i < h_out; i += 2) {
        ...
        asm volatile(INIT_S1
                     ...
                     "blt 0f                         \n" RIGHT_COMPUTE_S1
                         RIGHT_RESULT_S1_RELU6 "0:                         \n"
                     : PARAM1
                     : PARAM2, [six_ptr] "r"(six), [remain] "r"(remain)
                     : ASM_PARAM);
        dout_ptr += 2 * w_out;
      }  //! end of processing mid rows
#endif
    ...
    LITE_PARALLEL_BEGIN(c, tid, ch_in) {
      ...
      for (int i = 0; i < h_out; i += 4) {
        ...
        asm volatile(
            ...
            : PARAM1
            : PARAM2, [vscale] "w"(vscale), [remain] "r"(remain)
            : ASM_PARAM);
        dout_ptr = dout_ptr + 4 * w_out;
      }
#else
      for (int i = 0; i < h_out; i += 2) {
        ...
        asm volatile(INIT_S1
                     ...
                     "0:                         \n"
                     : PARAM1
                     : PARAM2, [scale_ptr] "r"(scale), [remain] "r"(remain)
                     : ASM_PARAM);
        dout_ptr += 2 * w_out;
      }  //! end of processing mid rows
#endif
