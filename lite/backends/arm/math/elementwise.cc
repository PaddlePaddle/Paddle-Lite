...
#pragma omp parallel for collapse(2)
  for (int i = 0; i < batch; ++i) {
    for (int j = 0; j < channels; ++j) {
      ...
      if (remain > 0) {
        for (int p = 0; p < remain; p++) {
          *dout_ptr = pow(*din_ptr, diny_data);
          dout_ptr++;
          din_ptr++;
        }
      }
...
#pragma omp parallel for
  for (int i = 0; i < cnt; i++) {
    ...
    dinx0 = pow_ps(dinx0, diny0);
    dinx1 = pow_ps(dinx1, diny1);
    dinx2 = pow_ps(dinx2, diny2);
    dinx3 = pow_ps(dinx3, diny3);
  ...
  if (remain > 0) {
    ...
    float* dout_ptr = dout + (cnt << 4);
    for (int i = 0; i < remain; i++) {
      *dout_ptr = pow(*dinx_ptr, *diny_ptr);
      dout_ptr++;
      dinx_ptr++;
      diny_ptr++;
    }
