...
void conv_depthwise_3x3s2p1_bias_rlu6(float* dout,
                                      const float* din,
                                      const float* weights,
                                      const float* bias,
                                      const float* six,
...
#pragma omp parallel for
    for (int i = 0; i < ch_in; ++i) {
      ...
      for (int i = 0; i < h_out; i++) {
        ...
        asm volatile(INIT_S2 LEFT_COMPUTE_S2 LEFT_RESULT_S2_RELU6 MID_COMPUTE_S2
                     MID_RESULT_S2_RELU6 RIGHT_COMPUTE_S2_RELU6 RIGHT_RESULT_S2
                     : [din0_ptr] "+r"(din0_ptr),
                       [din1_ptr] "+r"(din1_ptr),
                       [din2_ptr] "+r"(din2_ptr),
                       [outptr] "+r"(doutr0_ptr),
...
#pragma omp parallel for
    for (int i = 0; i < ch_in; ++i) {
      ...
      for (int i = 0; i < h_out; i++) {
        ...
        unsigned int* mask_ptr = dmask;
        asm volatile(INIT_S2 LEFT_COMPUTE_S2 LEFT_RESULT_S2_LEAKY_RELU MID_COMPUTE_S2
                     MID_RESULT_S2_LEAKY_RELU RIGHT_COMPUTE_S2_LEAKY_RELU RIGHT_RESULT_S2
                     : [din0_ptr] "+r"(din0_ptr),
                       [din1_ptr] "+r"(din1_ptr),
                       [din2_ptr] "+r"(din2_ptr),
                       [outptr] "+r"(doutr0_ptr),
