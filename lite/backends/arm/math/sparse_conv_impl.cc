...
void sparse_conv_fp32_pipelined(const float* A,
  ...
  if
    SPARSE_UNLIKELY(mc != 0) {
      ...
      if SPARSE_UNLIKELY(mc != 0 && mc < 4 * sizeof(float)) {
        const float* w = A;
        const int32_t* dmap = widx_dmap;
        const uint32_t* nnzmap = nidx_nnzmap;
        const float* bs = bias;
        ...
        for (size_t i = 0; i < nc; i++) {
          ...
          if (flag_act == 1) {
              ...
              for (size_t k = 0; k < mindex; k++) {
                 *(output + k) = *(output + k) >= 0 ? *(output + k) : *(output + k) * val;
              }
          }
          output = (float*) ((uintptr_t) output + output_stride);
        }
...
void sparse_conv_int8_fp32_pipelined(const int8_t* A,
  ...
  if
    SPARSE_UNLIKELY(mc != 0) {
      ...
      if SPARSE_UNLIKELY(mc != 0 && mc < 4 * sizeof(int8_t)) {
        const int8_t* w = A;
        const int32_t* dmap = widx_dmap;
        const uint32_t* nnzmap = nidx_nnzmap;
        const float* bs = bias;
  ...
  while
    SPARSE_LIKELY(mc >= 48 * sizeof(int8_t)) {
      ...
      for (size_t i = 0; i < nc; i++) {
          ...
          output = (int8_t*) ((uintptr_t) output + output_stride);
      }
      output = (int8_t*) ((uintptr_t) output - output_decrement);
      B += 48;
      mc -= 48 * sizeof(int8_t);
    }
  if
