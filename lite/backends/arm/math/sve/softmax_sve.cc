...
void softmax_inner1_sve(const Dtype* din,
  ...
  LITE_PARALLEL_COMMON_BEGIN(i, tid, outer_size - 3, 0, 4) {
    ...
    auto pg = svwhilelt<Dtype>(i, outer_size);
    ...
    Dtype* din_max_ptr0 = din_ptr0;
    Dtype* din_max_ptr1 = din_ptr1;
    Dtype* din_max_ptr2 = din_ptr2;
    Dtype* din_max_ptr3 = din_ptr3;
    int x = 0;
    auto pg0 = svwhilelt<Dtype>(x, axis_size);
    auto vec_max0 = svdup_n(support::cpp11::lowest<Dtype>());
    auto vec_max1 = svdup_n(support::cpp11::lowest<Dtype>());
    auto vec_max2 = svdup_n(support::cpp11::lowest<Dtype>());
    auto vec_max3 = svdup_n(support::cpp11::lowest<Dtype>());
    do {
      auto vdata0 = svld1(pg, din_max_ptr0);
      auto vdata1 = svld1(pg, din_max_ptr1);
      auto vdata2 = svld1(pg, din_max_ptr2);
      auto vdata3 = svld1(pg, din_max_ptr3);
      // get max
      auto vmax0 = svmax_m(pg, vec_max0, vdata0);
      auto vmax1 = svmax_m(pg, vec_max1, vdata1);
      auto vmax0 = svmax_m(pg, vec_max2, vdata2);
      auto vmax1 = svmax_m(pg, vec_max3, vdata3);
      din_max_ptr0 += svcnt<Dtype>();
      din_max_ptr1 += svcnt<Dtype>();
      din_max_ptr2 += svcnt<Dtype>();
      din_max_ptr3 += svcnt<Dtype>();
      x += svcnt<Dtype>();
      pg = svwhilelt<Dtype>(x, axis_size);
    } while (svptest_any(all_true_pg, pg0));
    Dtype vmax_0 = svmaxv(vec_max0);
    Dtype vmax_1 = svmaxv(vec_max1);
    Dtype vmax_2 = svmaxv(vec_max2);
    Dtype vmax_3 = svmaxv(vec_max3);
    // sub, exp and sum
    x = 0;
    ...
    for (int j = 0; j < axis_size; j += svcnt<Dtype>()) {
      ...
      auto vsub_exp0 = svexp_z(svsub_z(pg0, svld1(pg0, din_max_ptr0), vmax0));
      auto vsub_exp1 = svexp_z(svsub_z(pg0, svld1(pg0, din_max_ptr1), vmax1));
      auto vsub_exp2 = svexp_z(svsub_z(pg0, svld1(pg0, din_max_ptr2), vmax2));
      auto vsub_exp3 = svexp_z(svsub_z(pg0, svld1(pg0, din_max_ptr3), vmax3));
    ...
    auto vsum_0 = svaddv(vsum0);
    auto vsum_1 = svaddv(vsum1);
    auto vsum_2 = svaddv(vsum2);
    auto vsum_3 = svaddv(vsum3);
    auto vinf0 = svmul_z(pg, svdup_n(vsum_0), vinf);
    auto vinf1 = svmul_z(pg, svdup_n(vsum_1), vinf);
    auto vinf2 = svmul_z(pg, svdup_n(vsum_2), vinf);
    auto vinf3 = svmul_z(pg, svdup_n(vsum_3), vinf);
...
template <>
softmax_basic_sve<float>(const float* din,
                         float* dout,
                         const int axis_size,
                         const int inner_num,
                         const int outer_num);
...
template <>
softmax_axis4_sve<float>(const float* din,
                         float* dout,
                         const int axis_size,
                         const int inner_num,
                         const int outer_num);
...
template <>
softmax_inner1_sve<float>(const float* din,
                          float* dout,
                          const int axis_size,
                          const int inner_num,
                          const int outer_num);
...
template <>
softmax_basic_sve<float16_t>(const float16_t* din,
                             float16_t* dout,
                             const int axis_size,
                             const int inner_num,
                             const int outer_num);
...
template <>
softmax_axis4_sve<float16_t>(const float16_t* din,
                             float* dout,
                             const int axis_size,
                             const int inner_num,
                             const int outer_num);
...
template <>
softmax_inner1_sve<float16_t>(const float16_t* din,
                              float16_t* dout,
                              const int axis_size,
                              const int inner_num,
                              const int outer_num);
