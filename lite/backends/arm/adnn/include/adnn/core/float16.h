...
// Use ADNN_ALIGNED(2) to ensure that each float16 will be allocated
// and aligned at least on a 2-byte boundary, which leads to efficient
// memory access of float16 struct and also makes float16 compatible
// with CUDA half, ARM float16_t data types.
struct ADNN_ALIGN(2) float16 {
 public:
  uint16_t x;
...
#if defined(__F16C__) && (ADNN_ARCH_X86 || ADNN_ARCH_X86_64)
    x = _cvtss_sh(val, 0);
...
template <>
struct is_pod<adnn::float16> {
  static const bool value = is_trivial<adnn::float16>::value &&
                            is_standard_layout<adnn::float16>::value;
};

template <>
struct is_floating_point<adnn::float16>
    : std::integral_constant<
          bool,
          std::is_same<adnn::float16,
                       typename std::remove_cv<adnn::float16>::type>::value> {};
template <>
struct is_signed<adnn::float16> {
  static const bool value = true;
};

template <>
struct is_unsigned<adnn::float16> {
  static const bool value = false;
};

inline bool isnan(const adnn::float16& a) { return adnn::isnan(a); }

inline bool isinf(const adnn::float16& a) { return adnn::isinf(a); }

inline bool isfinite(const adnn::float16& a) { return adnn::isfinite(a); }

template <>
struct numeric_limits<adnn::float16> {
  ...
  static adnn::float16(min)() { return adnn::raw_uint16_to_float16(0x400); }
  static adnn::float16 lowest() { return adnn::raw_uint16_to_float16(0xfbff); }
  static adnn::float16(max)() { return adnn::raw_uint16_to_float16(0x7bff); }
  static adnn::float16 epsilon() { return adnn::raw_uint16_to_float16(0x1400); }
  static adnn::float16 round_error() { return adnn::float16(0.5); }
  static adnn::float16 infinity() {
    return adnn::raw_uint16_to_float16(0x7c00);
  }
  static adnn::float16 quiet_NaN() {
    return adnn::raw_uint16_to_float16(0x7e00);
  }
  static adnn::float16 signaling_NaN() {
    return adnn::raw_uint16_to_float16(0x7e00);
  }
  static adnn::float16 denorm_min() { return adnn::raw_uint16_to_float16(0x1); }
};

inline adnn::float16 abs(const adnn::float16& a) { return adnn::abs(a); }
