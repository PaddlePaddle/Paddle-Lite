...
#ifdef ADNN_WITH_THREAD_POOL
/* Support simple task
 * for (int i = 0; i < work_size; ++i)
 */
#define ADNN_THREAD_POOL_SIMPLE_TASK_BEGIN(index, tid, work_size) \
  {                                                               \
    std::pair<std::function<void(int, int)>, int> task;           \
    task.second = work_size;                                      \
  task.first = [&](int index, int tid) {
#define ADNN_THREAD_POOL_SIMPLE_TASK_END()    \
  }                                           \
  ;                                           \
  adnn::ThreadPool::Enqueue(std::move(task)); \
  }

/* Support common task
 * for (int i = start; i < end; i += step)
 */
#define ADNN_THREAD_POOL_COMMON_TASK_BEGIN(index, tid, end, start, step) \
  {                                                                      \
...
#define ADNN_THREAD_POOL_COMMON_TASK_END()    \
  }                                           \
  ;                                           \
  adnn::ThreadPool::Enqueue(std::move(task)); \
  }
#elif defined(ADNN_WITH_OMP)
#include <omp.h>
#define ADNN_THREAD_POOL_SIMPLE_TASK_BEGIN(index, tid, work_size)      \
