...
void conv_depthwise_3x3s2_p1_direct(const float* din,
  ...
  float *write_ptr = static_cast<float*>(
        TargetMalloc(TARGET(kX86), w_out * sizeof(float)));  

  //! prepare for processing right result
  int rmask_o[4] = {0};
  float rmask_i[8] = {1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f};          
  int r = w_out % 3;
  int col = w_out / 3;
  if (r > 0) col++;
  if (r > 0) {
      ...
      for (int i = 0; i < h_in; i += 4) {
        ...
        for (int j = 0; j < col; j+=1) {
          ...
          if (j == 0) {
            ...
            din_ptr3+=5;
            din_ptr4+=5;
            __m256 lmaski_ = _mm256_set_ps(0.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f);     
            i0 = _mm256_mul_ps (i0, lmaski_);  
            i0 = _mm256_permutevar8x32_ps (i0, shift_3);
            i1 = _mm256_mul_ps (i1, lmaski_);
            i1 = _mm256_permutevar8x32_ps (i1, shift_3); 
            i2 = _mm256_mul_ps (i2, lmaski_); 
            i2 = _mm256_permutevar8x32_ps (i2, shift_3);
            i3 = _mm256_mul_ps (i3, lmaski_); 
            i3 = _mm256_permutevar8x32_ps (i3, shift_3);
            i4 = _mm256_mul_ps (i4, lmaski_); 
          ...
          if (j+1 == col) {
            ...
            i1 = _mm256_mul_ps (i1, rmaski_);  
            i2 = _mm256_mul_ps (i2, rmaski_);  
            i3 = _mm256_mul_ps (i3, rmaski_);                          
            i4 = _mm256_mul_ps (i4, rmaski_);                
            dout_ptr = dout_ptr + 2 * w_out;    
            if (right) {
              mask = _mm_setr_epi32(rmask_o[0], rmask_o[1], rmask_o[2], rmask_o[3]);
            }    
          }                 
          ...
          if (has_active) { // process activation
            if (act_type == lite_api::ActivationType::kRelu) {
              ...
              r0_128 =  _mm_add_ps(_mm_and_ps(_mm_cmple_ps(zero, r0_128), r0_128),
                        _mm_mul_ps(_mm_and_ps(_mm_cmplt_ps(r0_128, zero), r0_128), negative_slope));
              r1_128 =  _mm_add_ps(_mm_and_ps(_mm_cmple_ps(zero, r1_128), r1_128),
                        _mm_mul_ps(_mm_and_ps(_mm_cmplt_ps(r1_128, zero), r1_128), negative_slope));
            } else {
  ...
  float *zero_ptr = static_cast<float*>(
        TargetMalloc(TARGET(kX86), Max(w_in * sizeof(float), 12 * sizeof(float))));
  memset(zero_ptr, 0, Max(w_in * sizeof(float), 12 * sizeof(float)));  
  float *write_ptr = static_cast<float*>(
        TargetMalloc(TARGET(kX86), w_out * sizeof(float)));  

  //! prepare for processing right result
  float rmask_ro[4] = {0.f, 0.f, 0.f, 0.f};
  float rmask_rm[4] = {1.f, 1.f, 1.f, 1.f};
  float rmask_i[12] = {1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f};            
  int r = w_out % 4;
  int col = w_out / 4;
  if (r > 0) col++;
  if (r > 0) {
    ...
    for (int c = 0; c < ch_in; c++) {
      ...
      for (int i = 0; i < h_in; i += 2) {
        ...
        for (int j = 0 ; j < col ; j++) {
          ...
          if (j + 1 == col) {
            doutr0 = doutr0 + w_out;      
            __m128 mask = _mm_loadu_ps(rmask_i);
            i0_0 = _mm_mul_ps(i0_0, mask);
            i1_0 = _mm_mul_ps(i1_0, mask); 
            i2_0 = _mm_mul_ps(i2_0, mask);  

            mask = _mm_loadu_ps(rmask_i+4);
            i0_1 = _mm_mul_ps(i0_1, mask);
            i1_1 = _mm_mul_ps(i1_1, mask); 
            i2_1 = _mm_mul_ps(i2_1, mask);  

            mask = _mm_loadu_ps(rmask_i+8);
            i0_2 = _mm_mul_ps(i0_2, mask);
            i1_2 = _mm_mul_ps(i1_2, mask); 
            i2_2 = _mm_mul_ps(i2_2, mask);              
          }           
          //！ shift input
          ...
          if (has_active) { // process activation
            if (act_type == lite_api::ActivationType::kRelu) {
              ...
              r0 = _mm_add_ps(_mm_and_ps(_mm_cmple_ps(zero, r0), r0),
                    _mm_mul_ps(_mm_and_ps(_mm_cmplt_ps(r0, zero), r0), negative_slope));
            } else {
              LOG(FATAL) << "[X86] activation type not supported";               
            }         
          }           
...
void conv_depthwise_3x3s1_p1_direct(const float* din,
  ...
  float rmask_i[8] = {0, 0, 0, 0, 0, 0, 0, 0};  
  int r = w_out % 6;
  int col = w_out / 6;
  if (r > 0) col++;
  if (r > 0) {
    ...
    for (int c = 0; c < ch_in; c++) {
      ...
      for (int i = 0; i < h_out; i += 4) { //TODO
        ...
        for (int j = 0; j < col; j += 1) {
          ...
          //! process right remain      
          __m256i smask_ = _mm256_set_epi32(0, 0, 0x80000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000);               
          if (j + 1 == col) {
            __m256 rmaski_ = _mm256_loadu_ps(rmask_i);            
            i0 = _mm256_mul_ps (i0, rmaski_);            
            i1 = _mm256_mul_ps (i1, rmaski_);  
            ...
            i3 = _mm256_mul_ps (i3, rmaski_);                          
            i4 = _mm256_mul_ps (i4, rmaski_);  
            i5 = _mm256_mul_ps (i5, rmaski_);    
            dout_ptr = dout_ptr + 4 * w_out;            
            if (right) {             
              smask_ = _mm256_set_epi32(rmask_o[7], rmask_o[6], rmask_o[5], rmask_o[4], rmask_o[3], rmask_o[2], rmask_o[1], rmask_o[0]);                                 
            }                      
          }
          ...
          if (has_active) {
            if (act_type == lite_api::ActivationType::kRelu) {
              ...
              __m256 negative_slope =  _mm256_set1_ps(act_param.Leaky_relu_alpha);
              r0 = _mm256_add_ps(_mm256_and_ps(_mm256_cmp_ps(zero, r0, 18), r0),
                      _mm256_mul_ps(_mm256_and_ps(_mm256_cmp_ps(r0, zero, 17), r0), negative_slope));
              r1 = _mm256_add_ps(_mm256_and_ps(_mm256_cmp_ps(zero, r1, 18), r1),
                      _mm256_mul_ps(_mm256_and_ps(_mm256_cmp_ps(r1, zero, 17), r1), negative_slope));
              r2 = _mm256_add_ps(_mm256_and_ps(_mm256_cmp_ps(zero, r2, 18), r2),
                      _mm256_mul_ps(_mm256_and_ps(_mm256_cmp_ps(r2, zero, 17), r2), negative_slope));
              r3 = _mm256_add_ps(_mm256_and_ps(_mm256_cmp_ps(zero, r3, 18), r3),
                      _mm256_mul_ps(_mm256_and_ps(_mm256_cmp_ps(r3, zero, 17), r3), negative_slope));                        
            } else {
              LOG(FATAL) << "[X86] activation type not supported";   
            }   
          }            
  ...
  float rmask_rm[4] = {1.f,1.f,1.f,1.f};
  float rmask_ro[4] = {0.f,0.f,0.f,0.f};
  float rmask_i[8] = {0.f,0.f,0.f,0.f, 0.f,0.f,0.f,0.f};
  int r = w_out%4;
  int col = w_out/4;
  if (r > 0) col++;
  if (r > 0) {
      ...
      for (int i = 0; i < h_in; i += 2) {   
        ...
        for (int j = 0 ; j < col ; j += 1) {
          ...
          if (j + 1 == col) {
            ...
            i1_0 = _mm_mul_ps(i1_0, mask); 
            i2_0 = _mm_mul_ps(i2_0, mask);
            i3_0 = _mm_mul_ps(i3_0, mask);             

            mask = _mm_loadu_ps(rmask_i+4);
            i0_1 = _mm_mul_ps(i0_1, mask);
            i1_1 = _mm_mul_ps(i1_1, mask); 
            i2_1 = _mm_mul_ps(i2_1, mask);
            i3_1 = _mm_mul_ps(i3_1, mask);             
          ...
          if (has_active) { // process activation
            if (act_type == lite_api::ActivationType::kRelu) {
              ...
              r1 =  _mm_add_ps(_mm_and_ps(_mm_cmple_ps(zero, r1), r1),
                        _mm_mul_ps(_mm_and_ps(_mm_cmplt_ps(r1, zero), r1), negative_slope));
            } else {
              LOG(FATAL) << "[X86] activation type not supported";   
            }              
          }                          
