...
#pragma omp parallel for
  for (int j = 0; j < loop_size; ++j) {
    ...
    __m128 vzero = _mm128_set1_ps(-127.f);
    __m128 vscale = _mm128_set1_ps(inv_scale);
    ...
    for (int i = 0; i < cnt; i++) {
      ...
      // fp32->int32
      __m256i vres0 = mm256_cvttps_epi32(vin0);
      __m256i vres1 = mm256_cvttps_epi32(vin1);
      __m256i vres2 = mm256_cvttps_epi32(vin2);
      __m256i vres3 = mm256_cvttps_epi32(vin3);
    ...
    for (int i = 0; i < cnt; i++) {
      ...
      // fp32->int32
      __m128i vres0 = mm_cvttps_epi32(vin0);
      __m128i vres1 = mm_cvttps_epi32(vin1);
      __m128i vres2 = mm_cvttps_epi32(vin2);
      __m128i vres3 = mm_cvttps_epi32(vin3);
...
#pragma omp parallel for
  for (int64_t n = 0; n < loop_size; ++n) {
...
#ifdef __AVX__
    for (int i = 0; i < cnt; i++) {
      __m128i vin0 = _mm_loadu_si128(din_c);
      __m128i vin1 = _mm_loadu_si128(din_c + 8);
      __m128i vin2 = _mm_loadu_si128(din_c + 16);
      __m128i vin3 = _mm_loadu_si128(din_c + 24);
      ...
      __m256 vout1 = _mm256_mul_ps_(_mm256_cvtepi32_ps(v01), vscale_l);
      __m256 vout2 = _mm256_mul_ps_(_mm256_cvtepi32_ps(v02), vscale_l);
      __m256 vout3 = _mm256_mul_ps_(_mm256_cvtepi32_ps(v03), vscale_l);
    ...
    for (int i = 0; i < cnt; i++) {
      __m128i vin0 = _mm_loadu_si128(din_c);
      __m128i vin1 = _mm_loadu_si128(din_c + 4);
      __m128i vin2 = _mm_loadu_si128(din_c + 8);
      __m128i vin3 = _mm_loadu_si128(din_c + 12);
      // 8bits x 16 -> 32bits x 4
      __m128i v00 = _mm_cvtepi8_epi32(vin0);
      __m128i v01 = _mm256_cvtepi8_epi32(vin1);
      __m128i v02 = _mm256_cvtepi8_epi32(vin2);
      __m128i v03 = _mm256_cvtepi8_epi32(vin3);
      // int32 -> fp32
      __m128 vout0 = _mm_mul_ps(_mm_cvtepi32_ps(v00), vscale_l);
      __m128 vout1 = _mm_mul_ps(mm_cvtepi32_ps(v01), vscale_l);
      __m128 vout2 = _mm_mul_ps(mm_cvtepi32_ps(v02), vscale_l);
      __m128 vout3 = _mm_mul_ps(mm_cvtepi32_ps(v03), vscale_l);
