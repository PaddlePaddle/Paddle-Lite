...
struct SelectedRowsAdd<lite::TargetType::kX86, T> {
  void operator()(const lite::X86Context& context,
    ...
    PADDLELITE_ENFORCE_EQ(in1_height, input2.height());
    ...
    PADDLELITE_ENFORCE_EQ(in1_row_numel, in2_value.numel() / in2_rows.size());
    PADDLELITE_ENFORCE_EQ(in1_row_numel, out_value->numel() / out_rows.size());
...
struct SelectedRowsAddTensor<lite::TargetType::kX86, T> {
  void operator()(const lite::X86Context& context,
    ...
    PADDLELITE_ENFORCE_EQ(in1_height, in2_dims[0]);
    PADDLELITE_ENFORCE_EQ(in1_height, out_dims[0]);

    auto& in1_value = input1.value();
    auto& in1_rows = input1.rows();

    int64_t in1_row_numel = in1_value.numel() / in1_rows.size();
    PADDLELITE_ENFORCE_EQ(in1_row_numel, input2.numel() / in1_height);
    PADDLELITE_ENFORCE_EQ(in1_row_numel, output->numel() / in1_height);
...
struct MergeAdd<lite::TargetType::kX86, T> {
  ...
  void operator()(const lite::X86Context& context,
    ...
    for (auto* input : inputs) {
      ...
      PADDLELITE_ENFORCE_EQ(input_width,
                            input->value().dims()[1],
