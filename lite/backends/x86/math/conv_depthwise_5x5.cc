...
void conv_depthwise_5x5s1s2(const float* din,
  ...
  float* pack_weight = static_cast<float *>(TargetMalloc(TARGET(kX86), 1 * channel_num / block_channel * 5 * 5 * block_channel * sizeof(float)));
  float* pack_input = static_cast<float *>(TargetMalloc(TARGET(kX86), 1 * (h_in + 2 * pad) * (w_in + 2 * pad) * block_channel * sizeof(float)));  
  float* pack_out =  static_cast<float *>(TargetMalloc(TARGET(kX86), 1 * h_out * w_out * block_channel * sizeof(float)));    
  ...
  for (int n = 0; n < num; n++) {
    ...
    for (int c = 0; c < ch_out ; c += block_channel) {
      ...
      packC8_with_Cleft(din_ptr, pack_input, {pad, pad, pad, pad}, h_in, w_in, real_block_channel);  
      int mask_ptr[8] = {0};
      for (int i = 0 ; i < 8 ; i++) {
      ...
      __m256i bias_mask = _mm256_set_epi32(mask_ptr[7], mask_ptr[6], mask_ptr[5], mask_ptr[4], mask_ptr[3], mask_ptr[2], mask_ptr[1], mask_ptr[0]);
      __m256 _bias = flag_bias ? _mm256_maskload_ps(bias + c , bias_mask) : _mm256_set1_ps(0.f); 
      ...
      if (stride == 1) {   
        for (int i = 0; i < h_out; i++) {
          ...
          for (; j + 3 < w_out; j += 4) {    
            ...
            if (has_active) {
              if (act_type == lite_api::ActivationType::kRelu) {
                ...
                r0 = _mm256_blendv_ps(r0, _mm256_mul_ps(negative_slope, r0),_mm256_cmp_ps(r0, zero, 2));
                r1 = _mm256_blendv_ps(r1, _mm256_mul_ps(negative_slope, r1),_mm256_cmp_ps(r1, zero, 2)); 
                r2 = _mm256_blendv_ps(r2, _mm256_mul_ps(negative_slope, r2),_mm256_cmp_ps(r2, zero, 2));
                r3 = _mm256_blendv_ps(r3, _mm256_mul_ps(negative_slope, r3),_mm256_cmp_ps(r3, zero, 2));                               
              } else {
          ...
          for (; j < w_out; j++) {
            ...
            for (int m = 0; m < 5; m++) {
              for (int n = 0; n < 5; n++) {
                __m256 weight = _mm256_loadu_ps(weights_data + 5 * block_channel * m + block_channel * n);           
                __m256 input = _mm256_loadu_ps(block_inr0 + block_channel * (j % 4) + in_len * m + block_channel * n);             
            ...
            if (has_active) {
              if (act_type == lite_api::ActivationType::kRelu) {
                ...
                __m256 negative_slope = _mm256_set1_ps(act_param.Leaky_relu_alpha);
                r = _mm256_blendv_ps(r, _mm256_mul_ps(negative_slope, r),_mm256_cmp_ps(r, zero, 2));                          
              } else {
                LOG(FATAL) << "[X86] activation type not supported";
              }
            }            
        ...
        for (int i = 0; i < h_out; i++) {
          ...
          for (; j < w_out; j++) {
            ...
            for (int m = 0; m < 5; m++) {
              for (int n = 0; n < 5; n++) {
                __m256 weight = _mm256_loadu_ps(weights_data + 5 * block_channel * m + block_channel * n);             
                __m256 input = _mm256_loadu_ps(block_inr0 + block_channel * (j % 4) * 2 + in_len * m + block_channel * n);              
                r = _mm256_fmadd_ps(input, weight, r);           
              }
            }
            __m256 zero = _mm256_setzero_ps();
      ...
      unpackC8_with_Cleft(pack_out, dout_ptr, size_out_channel, real_block_channel); 
    }
  } 
