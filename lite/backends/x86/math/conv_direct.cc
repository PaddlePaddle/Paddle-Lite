...
static inline void my_transpose8_ps(__m256& row0,  // NOLINT
                                 __m256& row1,  // NOLINT
                                 __m256& row2,  // NOLINT
                                 __m256& row3,  // NOLINT
                                 __m256& row4,  // NOLINT
...
// 下面开始拿这个featuremap的
#define window_h  5 //滑窗大小5和7正好能得到2x3个结果！
#define window_w  7
...
/*
            ...
            __m256 w000 = _mm256_load_ps(&trans_weight[kernel_start_index + 0 * BLOCK]);
            __m256 w001 = _mm256_load_ps(&trans_weight[kernel_start_index + 1 * BLOCK]);
            __m256 w002 = _mm256_load_ps(&trans_weight[kernel_start_index + 2 * BLOCK]);
            __m256 w010 = _mm256_loadu_ps(&trans_weight[kernel_start_index + 3 * BLOCK]);
            __m256 w011 = _mm256_loadu_ps(&trans_weight[kernel_start_index + 4 * BLOCK]);
            __m256 w012 = _mm256_loadu_ps(&trans_weight[kernel_start_index + 5 * BLOCK]);  
            __m256 w020 = _mm256_loadu_ps(&trans_weight[kernel_start_index + 6 * BLOCK]);
...
// 开始将第二行的三层参数load进来
w000 = _mm256_load_ps(&trans_weight[kernel_start_index + 9 * BLOCK]);
w001 = _mm256_load_ps(&trans_weight[kernel_start_index + 10 * BLOCK]);
w002 = _mm256_load_ps(&trans_weight[kernel_start_index + 11 * BLOCK]);
...
res01 = _mm256_fmadd_ps(input1, w011, res01);
// 更新w010
w010 = _mm256_load_ps(&trans_weight[kernel_start_index + 12 * BLOCK]);
res02 = _mm256_fmadd_ps(input2, w011, res02);
...
res01 = _mm256_fmadd_ps(input1, w012, res01);
// 更新w011
w011 = _mm256_load_ps(&trans_weight[kernel_start_index + 13 * BLOCK]);
res02 = _mm256_fmadd_ps(input2, w012, res02);
...
res01 = _mm256_fmadd_ps(input1, w020, res01);
// 更新w012
w012 = _mm256_load_ps(&trans_weight[kernel_start_index + 14 * BLOCK]);
res02 = _mm256_fmadd_ps(input2, w020, res02);
...
res01 = _mm256_fmadd_ps(input1, w021, res01);
// 更新w020
w020 = _mm256_load_ps(&trans_weight[kernel_start_index + 15 * BLOCK]);
res02 = _mm256_fmadd_ps(input2, w021, res02);
...
#define window_h  5 //the sliding window is 5x7 and can obtain 2x3 results！
#define window_w  7
    ...
    for (int oc_i = 0; oc_i < oc / BLOCK; oc_i++)
    {
      for (int oh_i = 0; oh_i < oh; oh_i += 1)
      {
        for (int ow_i = 0; ow_i < ow / BLOCK * BLOCK; ow_i += BLOCK)
        {
          ...
          __m256 row0 = _mm256_loadu_ps(&trans_out[start_index + 0 * 8]);
          __m256 row1 = _mm256_loadu_ps(&trans_out[start_index + 1 * 8]);
          __m256 row2 = _mm256_loadu_ps(&trans_out[start_index + 2 * 8]);
          __m256 row3 = _mm256_loadu_ps(&trans_out[start_index + 3 * 8]);
          __m256 row4 = _mm256_loadu_ps(&trans_out[start_index + 4 * 8]);
