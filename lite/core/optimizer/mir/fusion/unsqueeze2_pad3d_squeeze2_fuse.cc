...
void Unsqueeze2Pad3dSqueeze2Fuser::BuildPattern() {
   ...
   auto * p3d = OpNode("pad3d",pad3d_type_)
       ->assert_is_op(pad3d_type_)->AsIntermediate();
...
void Unsqueeze2Pad3dSqueeze2Fuser::InsertNewNode(SSAGraph* graph,
  ...
  auto padding =pad3d_instruct->op_info()->GetAttr<std::vector<int> >("paddings");
  if(padding.size()==6&&padding[4]==0&&padding[5]==0){
      pad3d_op_desc->mutable_inputs()->clear();
      pad3d_op_desc->mutable_outputs()->clear();
      pad3d_op_desc->SetAttr("paddings",std::vector<int>{padding[0],padding[1],padding[2],padding[3]});
      ...
      pad3d_op_desc->SetOutput("Out",{matched.at("sque_out")->arg()->name});
      {
          ...
          case OpDescAPI::AttrType::INT:
              pad3d_op_desc->SetAttr(attr_name_, pad3d_op_desc->GetAttr<int>(attr_name));
              break;
          case OpDescAPI::AttrType::FLOAT:
              pad3d_op_desc->SetAttr(attr_name_, pad3d_op_desc->GetAttr<float>(attr_name));
              break;
          case OpDescAPI::AttrType::BOOLEAN:
              pad3d_op_desc->SetAttr(attr_name_, pad3d_op_desc->GetAttr<bool>(attr_name));
              break;
          case OpDescAPI::AttrType::STRING:
              pad3d_op_desc->SetAttr(attr_name_, pad3d_op_desc->GetAttr<std::string>(attr_name));
              break;
          ...
          case OpDescAPI::AttrType::INTS: {
              pad3d_op_desc->SetAttr(attr_name,
                                     pad3d_op_desc->GetAttr<std::vector<int>>(attr_name));
          } break;
      ...
      auto * new_op_node = graph->GraphCreateInstructNode(pad2d_op,pad2d_op->valid_places());
      new_op_node->stmt()->ResetOp(*(new_op_node->stmt())->op_info(),graph->valid_places());
