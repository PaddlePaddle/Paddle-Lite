...
class XPUConv2dTransFuser : public FuseBase {
 public:
  explicit XPUConv2dTransFuser(const std::string& conv_type,
  ...
  void BuildPattern() override {
    ...
    auto no_outpadding = [](const Node* node) -> bool {
      auto op_desc = *const_cast<Node*>(node)->stmt()->op_info();
      return (!op_desc.HasAttr("output_padding"));
    };
    // only support no output_padding now.
    auto* conv = OpNode("conv2d_trans", conv_type_)
                     ->assert_node_satisfied(no_outpadding)
                     ->AsIntermediate();
    auto* conv_out =
        VarNode("conv2d_trans_out")->assert_is_op_output(conv_type_, "Output");
    // bn
    if (with_bn_) {
    ...
    if (with_bn_) {
      conv_out->assert_is_op_input("batch_norm", "X")->AsIntermediate();
  ...
  void InsertNewNode(SSAGraph* graph, const key2nodes_t& matched) override {
    ...
    // conv
    std::string conv_weight_name = matched.at("conv_filter")->arg()->name;
    auto conv_weight_t =
    ...
    if (with_bn_) {
      ...
      if (with_conv_bias_ && conv_op_desc->HasInput("Bias") &&
        ...
        for (size_t i = 0; i < bn_bias_t->data_size(); ++i) {
          // bias_size == h == conv2d output channls
          fusion_bias_ptr[i] += alpha_data[i] * conv_bias_d[i];
        }
      }
      for (size_t i = 0; i < bn_bias_t->data_size(); ++i) {
        fusion_bias_ptr[i] += beta_data[i];
      }
