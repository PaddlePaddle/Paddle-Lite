 ...
 public:
  void BuildPattern() override {
    ...
    auto* input = VarNode("input")
                      ...
                      ->AsInput();
    auto* input_embedding = VarNode("input_embedding")
    ...
    *qk_matmul >> *qk_matmul_out >> *qk_scale >> *qk_scale_out >> *qk_softmax >>
    ...
    // v_info
    *input_embedding >> *v_mul >> *v_mul_out >> *v_reshape2 >>
  ...
  void InsertNewNode(SSAGraph* graph, const key2nodes_t& matched) override {
    cpp::OpDesc op_desc;
    op_desc.SetType("__xpu__multihead_cross_attn");
    ...
    op_desc.SetInput("Embedding", {matched.at("input_embedding")->arg()->name});
    ...
    auto name = matched.at("k_mul_y")->arg()->name;
    auto k_mul_y_shape = scope->FindMutableTensor(name)->dims();
    hidden_dim = q_mul_y_shape[0];
    embedding_dim = k_mul_y_shape[0];
    ...
    op_desc.SetAttr<int>("embedding_dim", embedding_dim);

    update_weight(scope, fc_weight_names, fc_weight_maxptr_names);

    auto mhca_op = LiteOpRegistry::Global().Create(op_desc.Type());
    mhca_op->Attach(op_desc, scope);
    mhca_op->SetValidPlaces(matched.at("q_mul")->stmt()->op()->valid_places());
    auto kernels = mhca_op->CreateKernels(mhca_op->valid_places());
    matched.at("q_mul")->stmt()->SetOp(mhca_op);
...
/*
fuse original subgraph into __xpu__multihead_cross_attn op.
...
class XPUMHCAfusePass : public ProgramPass {
 public:
...
REGISTER_MIR_PASS(__xpu__multihead_cross_attn_fuse_pass,
                  paddle::lite::mir::XPUMHCAfusePass)
