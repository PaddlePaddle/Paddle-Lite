...
class XPUEmbeddingWithEltwiseAddFuser : public FuseBase {
 public:
  explicit XPUEmbeddingWithEltwiseAddFuser(
      int n_embedding, const std::string& op_type = "lookup_table", const std::string& pre_op_type = "bypass")
      : n_embedding_(n_embedding), op_type_(op_type), pre_op_type_(pre_op_type) {}
  ...
  void BuildPattern() override {
    ...
    for (int i = 2; i < n_embedding_; ++i) {
      ...
      auto preproces_out_name = paddle::lite::string_format("preproces_out%d", i);
      auto preproces_xshape_name = paddle::lite::string_format("preproces_xshape%d", i);
      auto table_name = paddle::lite::string_format("table%d", i);
      auto embedding_name = paddle::lite::string_format("embedding%d", i);
      auto embedding_out_name = paddle::lite::string_format("embedding_out%d", i);
      ...
      if (pre_op_type_ == "squeeze2" || pre_op_type_ == "reshape") {
        ...
        auto* new_preproces_out = VarNode(preproces_out_name)
                                    ...
                                    ->assert_is_op_input(op_type_, "Ids")
                                    ->AsIntermediate();
        auto* new_preproces_xshape = VarNode(preproces_xshape_name)
                                      ->assert_is_op_output(pre_op_type_, "XShape")
                                      ->AsIntermediate();  
...
class XPUEmbeddingWithEltwiseAddFusePass : public ProgramPass {
 public:
  void Apply(const std::unique_ptr<SSAGraph>& graph) override {
    ...
    std::vector<std::string> preoptypes{"squeeze2", "reshape2", "bypass"};
    ...
    for (auto& pre_op_type : preoptypes) {
      for (int n_embedding : {4, 3, 2}) {
        for (auto& op_type : optypes) {
          fusion::XPUEmbeddingWithEltwiseAddFuser fuser(n_embedding, op_type, pre_op_type);
          fuser(graph.get());
        }
      }
    }
