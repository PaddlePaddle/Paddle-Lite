...
class XPUMemoryOptimizePass : public ProgramPass {
 ...
 private:
  void CollectLifeCycleByDevice(
      std::map<std::string, lifecycle_map_t>* lifecycles, SSAGraph*, std::map<std::string, std::string>* squeeze_input2output, std::map<std::string, std::string>* squeeze_output2input);
  void MakeReusePlan(const lifecycle_map_t& lifecycles,
                     std::map<std::string, std::string>* node2cluster, std::map<std::string, std::string>* squeeze_input2output, std::map<std::string, std::string>* squeeze_output2input);
