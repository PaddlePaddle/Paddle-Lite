...
void MemoryOptimizePass::PerformReusePlan(
  ...
  Place valid_places(TargetType::kHost,
                    PrecisionType::kAny,
                    DataLayoutType::kAny);
  ...
  std::shared_ptr<OpLite> memory_optimize_op = LiteOpRegistry::Global().Create("memory_optimize");
  memory_optimize_op->SetValidPlaces({valid_places});
  cpp::OpDesc opdesc;
  opdesc.SetType("memory_optimize");
  opdesc.SetAttr<std::vector<std::string>>("memory_reuse_table", memory_reuse_table);
  memory_optimize_op->Attach(opdesc, exec_scope);

  auto kernels = memory_optimize_op->CreateKernels({valid_places});

  new_node.AsStmt(memory_optimize_op->Type(), std::move(kernels), memory_optimize_op);
...
void MemoryOptimizePass::Apply(const std::unique_ptr<SSAGraph>& graph, Scope* exec_scope) {
#ifdef LITE_WITH_XPU
...
REGISTER_MIR_PASS(memory_optimize_pass, paddle::lite::mir::MemoryOptimizePass)
    .BindTargets({TARGET(kARM), TARGET(kHost), TARGET(kX86), TARGET(kOpenCL), TARGET(kXPU)})
