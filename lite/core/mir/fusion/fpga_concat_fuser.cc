...
std::string FpgaConcatFuser::DebugPatternInfo(const std::vector<NodeInfo>& pattern) {
    std::stringstream ss;
...
int FpgaConcatFuser::enable_jump(Node* opnode) {
    ...
    int groups = 1;
    if(opdesc->HasAttr("groups"))
      groups = opdesc->GetAttr<int>("groups");
    ...
    if(type == "conv2d") {
        ...
        if(cin <= 2047 && cout % 16 == 0 && groups == 1)
            return cout;
    }
    return 0;
}
...
bool FpgaConcatFuser::enable_fuse(Node* varnode) {
    if(varnode->outlinks.size() == 1)
        return true;
    return false;
}


void FpgaConcatFuser::fuse_accumulate(std::vector<std::vector<NodeInfo>>& groups) {
...
std::vector<std::vector<NodeInfo>> FpgaConcatFuser::select_candidate(std::vector<NodeInfo> subgraph) {
    ...
    // if there is only one conv in a group, it means no other conv can be fused with
    // just remove it
    auto iter = std::remove_if(groups.begin(), groups.end(),
                    [](std::vector<NodeInfo> item) {return item.size() == 1;});
    groups.erase(iter, groups.end());
    fuse_accumulate(groups);
    return groups;
}

std::vector<std::vector<NodeInfo>> FpgaConcatFuser::PatternMatch(SSAGraph* graph) {
...
//            jump_op_idx.clear();
		   ...
		   // TODO Currently only the patterns that all upstream ops of concat support jump write are considered
	           subgraph.clear();
                   break;
                }
            }
            std::vector<std::vector<NodeInfo>> grouped_candidate = select_candidate(subgraph);
...
//            std::vector<NodeInfo> select_subgraph;
            if (grouped_candidate.size() == 1) {
                ...
                for (int i = 0; i < fuse_op_num; ++i) {
                    select_subgraph.push_back(NodeInfo(select_subgraph[i].node_->outlinks.front(), false, -1,  i));
                }
...
// in order to be consistent with graph, concat op with op to be concat are wrapped into sub graph
void FpgaConcatFuser::InsertNewNode(SSAGraph *graph, std::vector<std::vector<NodeInfo>>& patterns) {
        ...
        for (auto &subgraph_nodeinfos : patterns) {
            ...
            for (auto &nodeinfo: subgraph_nodeinfos) {
	            ...
	            // TODO "Output" is a common attr name?
                    ...
                    // TODO is this the best way to support other kernel
                    kerneltype << op_type << "/" << "def" << "/" << "7" << "/" << "5" << "/" << "3";
...
//            kKernelTypeAttr.SetAttr<std::string>("__@kernel_type_attr@__", "FPGA");

            // prepare for inputs and outputs of the subgraph
            std::set<Node *> idata_var_nodes;
            std::set<Node *> weight_var_nodes;
            std::set<Node *> odata_var_nodes;
            ExtractInputsOutputs(subgraph_nodeinfos, &idata_var_nodes, &weight_var_nodes, &odata_var_nodes);
...
void FpgaConcatFuser::DeleteInterNodes(SSAGraph *graph, std::vector<std::vector<NodeInfo>>& patterns) {
