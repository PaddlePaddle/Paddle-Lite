...
class XPUSingleEncoderNormBeforeFuser : public FuseBase {
 public:
  explicit XPUSingleEncoderNormBeforeFuser(
      ...
      const std::string& input_pos = "Y",
      const std::string& qkv_add_2_out_pos = "Y",
      const std::string& matmul_type = "matmul",
      const std::string& mul_type = "mul",
      bool with_q_scale = true)
      : act_type_(act_type),
        input_pos_(input_pos),
        qkv_add_2_out_pos_(qkv_add_2_out_pos),
        matmul_type_(matmul_type),
        mul_type_(mul_type),
        with_q_scale_(with_q_scale) {}
  ...
  void BuildPattern() override {
    ...
    auto* ln_before_bias = VarNode("ln_before_bias")
                               ->assert_is_op_input("layer_norm", "Bias")
                               ->AsInput();
    auto* ln_before = OpNode("ln_before", "layer_norm");
    ...
    auto* qkv_add_2 = OpNode("qkv_add_2", "elementwise_add")->AsIntermediate();
    auto* qkv_add_2_out =
        VarNode("qkv_add_2_out")
            ->assert_is_op_output("elementwise_add", "Out")
            ->assert_is_op_input("layer_norm", "X")
            ->assert_is_op_input("elementwise_add", qkv_add_2_out_pos_)
            ->AsIntermediate();
    ...
    // TODO(miaotianxiang): use LinksFrom/LinksTo() instead
    *input >> *ln_before >> *ln_before_out;
    *ln_before_scale >> *ln_before;
    *ln_before_bias >> *ln_before;
    *ln_before >> *ln_before_mean;
    *ln_before >> *ln_before_var;

    if (with_q_scale_) {
      *ln_before_out >> *q_mul >> *q_mul_out >> *q_add >> *q_add_out >>
    ...
    *ln_before_out >> *k_mul >> *k_mul_out >> *k_add >> *k_add_out >>
    ...
    *ln_before_out >> *v_mul >> *v_mul_out >> *v_add >> *v_add_out >>
    ...
    *qkv_mul_y >> *qkv_mul;
    *qkv_add_y >> *qkv_add;
    *input >> *qkv_add_2 >> *qkv_add_2_out;

    *qkv_add_2_out >> *qkv_ln_2 >> *qkv_ln_2_out >> *qkv_mul_3 >>
        *qkv_mul_3_out >> *qkv_add_3 >> *qkv_add_3_out >> *qkv_act >>
        *qkv_act_out >> *qkv_mul_4 >> *qkv_mul_4_out >> *qkv_add_4 >>
        *qkv_add_4_out >> *qkv_add_5;
    *qkv_ln_2_scale >> *qkv_ln_2;
    *qkv_ln_2_bias >> *qkv_ln_2;
    *qkv_ln_2 >> *qkv_ln_2_mean;
    *qkv_ln_2 >> *qkv_ln_2_var;
  ...
  void InsertNewNode(SSAGraph* graph, const key2nodes_t& matched) override {
    cpp::OpDesc op_desc;
    op_desc.SetType("single_encoder_norm_before");
    op_desc.SetInput("Inputs", {matched.at("input")->arg()->name});
    op_desc.SetInput("Mask", {matched.at("qk_mask")->arg()->name});
    op_desc.SetInput("FCWeight",
                     {
    ...
    static_cast<operators::SubgraphOp*>(fake_subgraph_op.get())
        ->SetProgramDesc(sub_program_desc);
    auto* single_encoder_stmt = matched.at("ln_before")->stmt();
    fake_subgraph_op->Attach(op_desc, single_encoder_stmt->op()->scope());
    fake_subgraph_op->SetValidPlaces(single_encoder_stmt->op()->valid_places());
    single_encoder_stmt->SetOp(fake_subgraph_op);
    ...
    for (auto& from : froms) {
      IR_NODE_LINK_TO(matched.at(from), matched.at("ln_before"));
    }
    IR_OP_VAR_LINK(matched.at("ln_before"), matched.at("qkv_add_5_out"));
  }

 private:
  std::string act_type_;
  std::string input_pos_;
  std::string qkv_add_2_out_pos_;
  std::string matmul_type_;
  std::string mul_type_;
  bool with_q_scale_;
};

class XPUMultiEncoderNormBeforeFuser {
 public:
  explicit XPUMultiEncoderNormBeforeFuser(const std::string& fc_precision)
      : fc_precision_(fc_precision) {}
  ...
  void operator()(SSAGraph* graph) {
    ...
    for (auto* node : graph->StmtTopologicalOrder()) {
      CHECK(node->IsStmt());
      if (node->stmt()->op_info()->Type() == "single_encoder_norm_before") {
        all_encoders.push_back(node);
      }
    }
    VLOG(3) << "Found " << all_encoders.size() << " single_encoder_norm_before";
    if (all_encoders.size() == 0) {
      return;
    }
    ...
    auto* multi_encoder_stmt = first_encoder->stmt();
    cpp::OpDesc op_desc;
    op_desc.SetType("__xpu__multi_encoder_norm_before");
    op_desc.SetInput("Input", {in_name});
    for (auto kv : arg_map) {
      op_desc.SetInput(kv.first, kv.second);
    }
    ...
    // q/k/v fusion
    // TODO(mayang02): need xpu api support qkv_fusion
...
class XPUMultiEncoderNormBerforeFusePass : public ProgramPass {
 public:
  void Apply(const std::unique_ptr<SSAGraph>& graph) override {
    if (GetBoolFromEnv("XPU_ENABLE_XTCL")) return;
    // TODO(miaotianxiang): backup graph, recover from failed match
    std::vector<std::string> act_types{"gelu", "relu"};
    std::vector<std::string> input_poss{"X", "Y"};
    std::vector<std::string> qkv_add_2_out_poss{"X", "Y"};
    std::vector<std::string> matmul_types{"matmul", "matmul_v2"};
    std::vector<std::string> mul_types{"mul", "matmul"};
    std::vector<bool> with_q_scales{true, false};
    ...
    if (GetStringFromEnv("XPU_ENCODER_PRECISION", "int16") == "int31" ||
      ...
      fc_precision = "int16";
      VLOG(3) << "Use int16 in XPUMultiEncoderNormBeforeOp, "
              << "lite::TargetWrapperXPU::multi_encoder_precision="
              << lite::TargetWrapperXPU::multi_encoder_precision;
    }
#endif
    ...
    for (auto& act_type : act_types) {
      for (auto& input_pos : input_poss) {
        for (auto& qkv_add_2_out_pos : qkv_add_2_out_poss) {
          for (auto& matmul_type : matmul_types) {
            for (auto& mul_type : mul_types) {
              for (auto with_q_scale : with_q_scales) {
                fusion::XPUSingleEncoderNormBeforeFuser single_encoder_fuser(
                    act_type,
                    input_pos,
                    qkv_add_2_out_pos,
                    matmul_type,
                    ...
                    with_q_scale);
                single_encoder_fuser(graph.get());
                fusion::XPUMultiEncoderNormBeforeFuser multi_encoder_fuser(
                    fc_precision);
                multi_encoder_fuser(graph.get());
              }
            }
...
REGISTER_MIR_PASS(__xpu__multi_encoder_norm_before_fuse_pass,
                  paddle::lite::mir::XPUMultiEncoderNormBerforeFusePass)
    .BindTargets({TARGET(kXPU)})
    .BindKernel("__xpu__multi_encoder_norm_before");
