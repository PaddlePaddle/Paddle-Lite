...
TEST(e2e_server_pgnetA, test_e2e_server_pgnetA_fp32_v2_3_nnadapter) {
...
//   nnadapter_device_names.emplace_back("qualcomm_qnn");
  ...
  std::string raw_data_dir = FLAGS_data_dir;
  std::string out_data_dir = FLAGS_data_dir;
  std::string images_shape_path =
      FLAGS_data_dir + std::string("/images_shape.txt");
  auto input_lines = ReadLines(images_shape_path);

  std::vector<std::string> input_names = {"e2e_server_pgnetA_input"};
  std::vector<std::string> output_names = {"e2e_server_pgnetA_output"};
  ...
  for (size_t i = 0; i < raw_data.size(); ++i) {
    ...
    gt_data.push_back(ReadRawData(out_data_dir, output_names[i], output_shape));
  }

  for (float abs_error : {1e-1}) {
