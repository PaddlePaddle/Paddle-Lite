...
bool test_gemm_s8u8(
  ...
  paddle::lite::x86::math::generate_gemm_s8u8_x86_kern gemm(tra,
                                                            trb,
                                                            m,
                                                            n,
                                                            k,
...
#endif
  for (int mm = 501; mm < 600; mm += 33) {
    for (int nn = 501; nn < 600; nn += 43) {
      for (int kk = 501; kk < 600; kk += 53) {
        for (auto &ta : {true, false}) {
          for (auto &tb : {true, false}) {
            for (auto &bias : {true, false}) {
              for (auto &relu : {true, false}) {
                auto flag = test_gemm_s8u8(ta, tb, mm, nn, kk, bias, relu);
