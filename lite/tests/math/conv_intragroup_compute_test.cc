...
double test_conv_int8(const DDim& dim_in,
  ...
  param_int8_out.is_sparse = is_sparse;
  ...
  for (auto& cls : power_mode) {
    for (auto& th : thread_num) {
      ...
      avg_gops = 1e-6 * gops / t0.LapTimes().Min();
...
#else
double test_conv_int8(const DDim& dims_in,
                    const DDim& weight_dim,
                    int group,
                    const std::vector<int>& strides,
                    const std::vector<int>& pads,
...
TEST(TestConv1x1s1Int8, test_conv1x1s1) {
  if (FLAGS_basic_test) {
    for (auto& cout : {128, 256,512,1024}) {
      LOG(INFO) << "                   64             128            256";
      for (auto& cin : {128, 256,512,1024}) {
        double gops_sparse[5], gops_dense[5];
        int i=0;
        DDim weights_dim({cout, cin, 1, 1});
          for (auto& h : {64, 128, 256}) {
              DDim dims({1, cin, h, h});
              gops_sparse[i]=test_conv_int8(dims,
                            weights_dim,
                            1,
                            {1, 1},
                            {0, 0, 0, 0},
                            ...
                            true);
              gops_dense[i++]=test_conv_int8(dims,
                            weights_dim,
                            1,
                            {1, 1},
                            {0, 0, 0, 0},
          ...
          std::cout<<std::setprecision(3)<<"Input Channel: " << cin << "    "<<
                    gops_sparse[0]<< "/"<<gops_dense[0]<<" "<<(gops_sparse[0]-gops_dense[0])/gops_dense[0]*100<<"%   "<<
                    gops_sparse[1]<< "/"<<gops_dense[1]<<" "<<(gops_sparse[1]-gops_dense[1])/gops_dense[1]*100<<"%   "<<
                    gops_sparse[2]<< "/"<<gops_dense[2]<<" "<<(gops_sparse[2]-gops_dense[2])/gops_dense[2]*100<<"%"<<std::endl;
        }
      }
    }
  }
