...
void test_conv_fp16(const std::vector<DDim>& input_dims,
  ...
  if (flag_bias) {
    ...
    // fill_data_const<float16_t>(b_ptr, 0.f, param.bias->numel());
  ...
  for (auto& cls : power_mode) {
    for (auto& th : thread_num) {
      ...
      for (auto& dim_in : input_dims) {
        ...
        fill_data_rand<float16_t>(b_ptr, -10.f, 10.f, param.x->numel());
        ...
        if (FLAGS_check_result) {
          ...
          if (FLAGS_check_result) {
            ...
            data_diff(dout_basic,  // basic_ptr,
                      saber_ptr,
                      ptr,
                      tout_basic.numel(),
                      max_ratio,
            ...
            if (max_diff > basic_max_diff) {
              ...
              if (!check) {
                ...
                print_tensor(dout_basic, size, width);
