...
*/
//yuv  store hwc bgrabgra dsth * dstw = srch * srcw y_w = srcw, y_h = srch uv_w = srcw uv_h = 1/2 * srch
void nv12_to_bgra(const unsigned char* src, unsigned char* dst, int srcw, int srch){
    int y_h = srch;
    int vu_h = 1 / 2 * srch;
    ...
    for (int i = 0; i < y_h; i += 2){
        ...
        int j = 0;
        for (; j < srcw - 15; j += 16){
            uint8x8x2_t y1 = vld2_u8(ptr_y1);//d8 = y0y2y4y6...y14 d9 = y1y3y5...y15
