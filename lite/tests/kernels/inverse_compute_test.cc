...
class InverseComputeTester : public arena::TestCase {
 ...
 public:
  InverseComputeTester(const Place& place,
...
{
	T* temp = (T *)malloc(2*rows*cols*sizeof(T));
  ...
  void RunBaselineKernel(Scope* scope) {
    ...
    for (int i = 0; i < batch_size; i++)
      GaussInv(x_data + i * rows * cols, output_data + i * rows * cols, rows, cols);
  }
