...
class GenerateProposalsV2ComputeTester : public arena::TestCase {
 ...
 public:
  GenerateProposalsV2ComputeTester(const Place& place, const std::string& alias)
  ...
  template <class T>
  static inline void ClipTiledBoxes(const Tensor& im_shape, Tensor* boxes) {
  ...
  std::pair<Tensor, Tensor> ProposalForOneImage(
    ...
    ClipTiledBoxes<T>(im_shape_slice, &proposals);

    Tensor keep;
    FilterBoxesWithoutScale<T>(&proposals, min_size, im_shape_slice, &keep);
