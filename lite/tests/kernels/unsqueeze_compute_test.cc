...
class UnsqueezeComputeTester : public arena::TestCase {
 ...
 public:
  ...
  void RunBaseline(Scope* scope) override {
    ...
    int output_size = dims_.size() + static_cast<int>(axes_.size());
    int cur_output_size = dims_.size();
    ...
    for (int axis : axes_) {
      ...
      CHECK_GE(cur, 0) << "Invalid axis, the negative axis is out of range.";
      CHECK_LE(cur, cur_output_size)
    ...
    for (int in_idx = 0, out_idx = 0; out_idx < output_size; ++out_idx) {
      if (output_shape[out_idx] == 0) {
        output_shape[out_idx] = dims_[in_idx++];
      }
    }
