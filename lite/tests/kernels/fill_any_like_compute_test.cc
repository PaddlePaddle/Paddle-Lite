...
class FillAnyLikeComputeTester : public arena::TestCase {
 ...
 public:
  ...
  void RunBaseline(Scope* scope) override {
    ...
    out->Resize(x_dims_);
    out->set_lod(x->lod());
    auto* out_data = out->template mutable_data<T_OUT>();
    for (int64_t i = 0; i < out->numel(); i++) {
      out_data[i] = static_cast<T_OUT>(value);
    }
  }
  ...
  void PrepareOpDesc(cpp::OpDesc* op_desc) {
    ...
    op_desc->SetAttr("dtype", dtype_) op_desc->SetOutput("Out", {out_});
  }

  void PrepareData() override {
    std::vector<T_IN> x(x_dims_.production());
...
void TestFillAnyLike(Place place, float abs_error) {
