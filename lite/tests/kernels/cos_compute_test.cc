...
class CosComputeTest : public arena::TestCase {
 protected:
  // common attributes for this op.
  std::string op_type_ = "cos";
  DDim x_dims_{{1, 2, 3, 4}};
  std::string x_ = "x";
  std::string out_ = "out";

 public:
  CosComputeTest(const Place& place, const std::string& alias, DDim x_dims)
...
void test_sin(Place place) {
  ...
  for (auto x_dims : std::vector<std::vector<int64_t>>{
    ...
    std::unique_ptr<arena::TestCase> tester(
        new CosComputeTest(place, "def", DDim(x_dims)));
