  ...
  TileComputeTester(const Place& place,
    ...
    if (use_repeat_times_tensor_list) {
      for (size_t i = 0; i < repeat_times.size(); i++) {
        repeat_times_tensor_list_.push_back("repeat_times_" + std::to_string(i));
      }
    }
  }
...
TEST(tile, precision) {
  ...
  abs_error = 1e-2;
	TestTile<float>(place, abs_error);
  TestTile<float>(place, abs_error, {1, 1, 1}, {2, 3, 4});
  TestTile<float>(place, abs_error, {2, 1, 4}, {2, 3, 4}, true);
	return;
#else
  return;
#endif
  ...
  TestTile<float>(place, abs_error, {1, 1, 1}, {2, 3, 4});
  TestTile<float>(place, abs_error, {2, 1, 4}, {2, 3, 4}, true);
  TestTile<float>(place, abs_error, {2, 1, 4}, {2, 3, 4}, false, true);
}
