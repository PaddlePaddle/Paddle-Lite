...
def sample_program_configs(draw):
    ...
    start_axis = 1
    if start_axis == 0:
        flatten_out_shape = [in_shape[0] * in_shape[1] * in_shape[2], in_shape[3]]
    else:
        flatten_out_shape = [in_shape[0], in_shape[1] * in_shape[2] * in_shape[3]]
    ...
    flatten_op = OpConfig(
        ...
        outputs = {"Out": ["flatten_output_data"], "XShape": ["xshape_data"]},
        attrs = {
            "data_format": 'nchw',
    ...
    if(with_bias and np.random.random() > 0.5):
        act_type = "relu"
    if(with_bias):
         fc_inputs = {"Input":["flatten_output_data"], "W":["weights_data"], "Bias":["bias_data"]}
         program_inputs = {
    ...
    program_config = ProgramConfig(
        ops = ops,
        weights ={
            "xshape_data": TensorConfig(shape=in_shape)
        },
