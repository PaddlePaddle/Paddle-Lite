...
def ConvTransposeOutputSize(in_shape, weight_shape, dilations, paddings, strides):
    oh = (in_shape[2] - 1) * strides[0] - paddings[0] - paddings[1] + (dilations[0] * (weight_shape[2] - 1) + 1)
    ow = (in_shape[3] - 1) * strides[1] - paddings[2] - paddings[3] + (dilations[1] * (weight_shape[3] - 1) + 1)
    return oh,ow
