...
class TestMatmulElementwiseAddFusePass(FusePassAutoScanTest):
    ...
    def sample_program_configs(self, draw):
        matmul_x_shape=draw(st.lists(st.integers(min_value=5, max_value=10), min_size=2, max_size=2))
    ...
    def sample_predictor_configs(self):
        if self.get_target() == 'OpenCL':
            return self.get_predictor_configs(), ['io_copy', 'layout', 'fc', 'layout', 'io_copy'], (1e-5, 1e-5)
        else:
