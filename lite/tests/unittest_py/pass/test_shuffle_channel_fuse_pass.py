...
class TestShuffleChannelFusePass(FusePassAutoScanTest):
    ...
    def sample_program_configs(self, draw):
        reshape1_output_shape = draw(st.lists(st.integers(min_value = 1, max_value = 32), min_size = 5, max_size = 5))
        reshape1_input_shape = draw(st.lists(st.integers(min_value = 1, max_value = 32), min_size = 4, max_size = 4))
        reshape2_output_shape = draw(st.lists(st.integers(min_value = 1, max_value = 32), min_size = 4, max_size = 4))

        reshape1_input_shape[0] = reshape1_output_shape[0]
        reshape1_input_shape[1] = reshape1_output_shape[1] * reshape1_output_shape[2]
        ...
        shuffle_channel_fuser_type = draw(st.sampled_from(["shuffle_channel_fuser1", "shuffle_channel_fuser2"]))
        ...
        if shuffle_channel_fuser_type == "shuffle_channel_fuser1":
            ...
            reshape2_op = OpConfig(
                ...
                attrs = {
                    "shape": reshape2_output_shape
                })
        else: 
            reshape1_op = OpConfig(
                type = "reshape2",
                inputs = {"X": ["reshape1_input_x"]},
                outputs = {"Out": ["reshape1_output"], "XShape": ["reshape1_XShape_data"]},
                attrs = {
                    "shape": reshape1_output_shape
                })
            ...
            transpose_op = OpConfig(
                type = "transpose2",
                inputs = {"X": ["reshape1_output"]},
                outputs = {"Out": ["transpose_output"], "XShape": ["transpose_XShape_data"]},
                attrs = {
                    "use_mkldnn": False,
                    "axis": [0, 2, 1, 3, 4]
                })
            ...
            reshape2_op = OpConfig(
                type = "reshape2",
                inputs = {"X": ["transpose_output"]},
                outputs = {"Out": ["output_data"], "XShape": ["reshape2_XShape_data"]},
                attrs = {
                    "shape": reshape2_output_shape
                })
        ...
        program_config = ProgramConfig(
            ops = ops,
            weights={"reshape1_XShape_data" : TensorConfig(shape=[5]), 
    ...
    def sample_predictor_configs(self):
        if self.get_target() == 'OpenCL':
            return self.get_predictor_configs(), ['io_copy', 'layout', 'shuffle_channel', 'layout', 'io_copy'], (1e-5, 1e-5)
        else:
            return self.get_predictor_configs(), ['shuffle_channel'], (1e-5, 1e-5)
