...
class TestInstanceNormActivationFusePass(FusePassAutoScanTest):
    ...
    def sample_program_configs(self, draw):
        ...
        act_type = draw(st.sampled_from(['relu']))   #opencl kernel only support relu
    ...
    def sample_predictor_configs(self):
        if self.get_target() == 'OpenCL':
            return self.get_predictor_configs(), ['io_copy', 'layout', 'instance_norm', 'layout', 'io_copy'], (1e-5, 1e-5)
        else:
    ...
    def test(self, *args, **kwargs):
        self.run_and_statis(quant=False, max_examples=25, passes=["lite_instance_norm_activation_fuse_pass"])
