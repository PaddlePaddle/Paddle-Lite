...
class TestTransposeSoftmaxTransposeFusePass(FusePassAutoScanTest):
    ...
    def sample_program_configs(self, draw):
        Place = self.valid_places[0]
        print(Place)
        
        dim = draw(st.sampled_from([2, 3, 4]))
        transpose_type = draw(st.sampled_from(["transpose", "transpose2"]))
        #default dim = 4
        transpose1_input_shape = draw(st.lists(st.integers(min_value=1, max_value=64), min_size=4, max_size=4))
        transpose1_axis = draw(st.lists(st.integers(min_value=0, max_value=3), min_size=4, max_size=4))
        assume(sorted(transpose1_axis)==[0, 1, 2, 3])
        transpose2_axis = [transpose1_axis.index(0),transpose1_axis.index(1), transpose1_axis.index(2), transpose1_axis.index(3)] 
  
        if dim == 2:
            transpose1_input_shape = draw(st.lists(st.integers(min_value=1, max_value=64), min_size=2, max_size=2))
            transpose1_axis = draw(st.lists(st.integers(min_value=0, max_value=1), min_size=2, max_size=2))
            assume(sorted(transpose1_axis)==[0, 1])
            transpose2_axis = [transpose1_axis.index(0),transpose1_axis.index(1)]
        elif dim == 3:
            transpose1_input_shape = draw(st.lists(st.integers(min_value=1, max_value=64), min_size=3, max_size=3))
            transpose1_axis = draw(st.lists(st.integers(min_value=0, max_value=2), min_size=3, max_size=3))
            assume(sorted(transpose1_axis)==[0, 1, 2])
            transpose2_axis = [transpose1_axis.index(0),transpose1_axis.index(1), transpose1_axis.index(2)] 
    ...
    def sample_predictor_configs(self):
        if self.get_target() == 'OpenCL':
            return self.get_predictor_configs(), ['io_copy', 'layout', 'softmax', 'layout', 'io_copy'], (1e-5, 1e-5)
        else:
    ...
    def test(self, *args, **kwargs):
        self.run_and_statis(quant=False, max_examples=25, min_success_num=5, passes=["lite_transpose_softmax_transpose_fuse_pass"])
