...
class TestScaleActiveFusePass(FusePassAutoScanTest):
    ...
    def sample_program_configs(self, draw):
        in_shape_x = draw(st.lists(st.integers(min_value = 1, max_value = 64), min_size = 2, max_size = 4))
    ...
    def sample_predictor_configs(self):
        if self.get_target() == 'OpenCL':
            return self.get_predictor_configs(), ['io_copy', 'layout', 'scale', 'layout', 'io_copy'], (1e-5, 1e-5)
        else:
    ...
    def test(self, *args, **kwargs):
        self.run_and_statis(quant=False, max_examples=25, passes=["lite_scales_fuse_pass"])
