...
def sample_program_configs(draw, interp_type):
    interpolate_type = draw(st.sampled_from(['interpolate_type_1','interpolate_type_2']))
    if interpolate_type == "interpolate_type_1":
        #shape params
        shape_op_input_shape = draw(st.lists(st.integers(min_value = 2, max_value = 10), 
                                    min_size = 4, max_size = 4)) #min_value=1, Output has nan
        #slice params
        axes = [0]
        starts = [2]
        ends = [4]
        ...
        cast_op_out_dtype = draw(st.sampled_from([2]))   #bilinear_interp's OutSize must be int
        #fill_constant_op params
        fill_constant_in_shape = draw(st.lists(st.integers(min_value = 1, max_value = 2), min_size = 1, max_size = 1))
        ...
        fill_constant_op = OpConfig(
            ...
            attrs = {
                    ...
                    "value" : fill_constant_value,  #The value of the floating point number must be an integer, otherwise out has diff
                    "force_cpu" : False
                    })

        elementwise_mul_op = OpConfig(
            type = "elementwise_mul",
            inputs = {"X": ["fill_constant_output"],"Y": ["cast_out"]},
        ...
        interpolate_op = OpConfig(
            type = interp_type,
            inputs = {"X" : ["input_data"], "OutSize": ["elementwise_mul_out"]},   #OutSize's dimension[0] must be 2
        ...
        ops = [fill_constant_op, shape_op, slice_op, cast_op, elementwise_mul_op, interpolate_op]
        ...
        #shape params
        shape_op_input_shape = draw(st.lists(st.integers(min_value=2, max_value=10), min_size=4, max_size=4)) #min_value=1, Output has nan
        #slice params
        axes = [0]
        starts = [2]
        ends = [4]
        ...
        interpolate_op = OpConfig(
            type = interp_type,
            inputs = {"X" : ["input_data"], "OutSize": ["scale_output"]},   #OutSize's dimension[0] must be 2
            outputs = {"Out": ["output_data"]},
            attrs = {
                "data_layout" : "NCHW",
                "scale": scale,    #only interpolate_scale == scale_op_scale, out is ok, else out_shape has diff
                "interp_method" : interp_method,
                "align_corners" : align_corners,
                "align_mode" : align_mode
            })
...
class TestInterpolateBilinearFusePass(FusePassAutoScanTest):
    ...
    def sample_predictor_configs(self):
        if self.get_target() == 'OpenCL':
            return self.get_predictor_configs(), ['io_copy', 'layout', 'bilinear_interp', 'layout', 'io_copy'], (1e-5, 1e-5)
        else:
    ...
    def test(self, *args, **kwargs):
        self.run_and_statis(quant=False, max_examples=25, passes=["lite_interpolate_fuse_pass"])
...
class TestInterpolateNearestFusePass(FusePassAutoScanTest):
    ...
    def sample_predictor_configs(self):
        if self.get_target() == 'OpenCL':
            return self.get_predictor_configs(), ['io_copy', 'layout', 'nearest_interp', 'layout', 'io_copy'], (1e-5, 1e-5)
        else:
