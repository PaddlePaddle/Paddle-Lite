...
class TestScaleOp(AutoScanTest):
    ...
    def sample_program_configs(self, *args, **kwargs):
        ...
        scale_op = OpConfig(
            ...
            attrs = {"scale" : kwargs['in_scale'], 
                     "bias" : kwargs['in_bias'], 
                     "bias_after_scale" : kwargs['before']})
    ...
    @given(
        in_shape=st.lists(st.integers(min_value=10, max_value=20), min_size=4, max_size=4),
        in_scale=st.floats(min_value=0.3, max_value=3),
        in_bias=st.floats(min_value=-1, max_value=1),
        before=st.sampled_from([False, True]))
