...
def sample_program_configs(draw):
    ...
    COLUMN_shape = draw(st.lists(st.integers(min_value=6, max_value=6), min_size = 2, max_size=2))
    ROW_shape = draw(st.lists(st.integers(min_value=6, max_value=6), min_size = 2, max_size=2))
    ...
    X_shape = draw(st.lists(st.integers(min_value=1, max_value=50), min_size = 2, max_size=2))
    X_shape[1] = 1
    X_shape[0] = (COLUMN_shape[0] * ROW_shape[0] + COLUMN_shape[1] * ROW_shape[1]) * InputChannel_data
    ...
    lod_data2 = draw(st.sampled_from([[[0,COLUMN_shape[0],sum(COLUMN_shape)]]]))
    ...
    var_conv2d_op = OpConfig(
        ...
        outputs = {"Out": ["Out_data"],
                   "Col":["Col_data"]},
        attrs = {"OutputChannel": OutputChannel_data,
    ...
    program_config = ProgramConfig(
        ...
        weights={},
        inputs={
            "X_data": TensorConfig(shape=X_shape,lod=lod_data1),
            "COLUMN_data": TensorConfig(shape=COLUMN_shape,lod=lod_data2),
            "ROW_data": TensorConfig(shape=ROW_shape,lod=lod_data3),
