...
def sample_program_configs(draw):
    ...
    def gen_lod_data(dimentsions_0_size, dimentsions_1_max_size):
        sequence_length_list = [
            np.random.randint(1, dimentsions_1_max_size + 1)
            for i in range(dimentsions_0_size)
        ]
    ...
    def gen_op_inputs_outputs():
        ...
        inputs_tensor = {
            'input_data_fpn_rois': TensorConfig(
                data_gen=partial(gen_input_fpn_rois), lod=lod_data)
        }
        if draw(st.booleans()):
        ...
        outputs = {
            'MultiFpnRois': ['output_data_multi_fpn_rois'],
        ...
        outputs_tensor_var_name = [
            'output_data_restore_index', 'output_data_multi_fpn_rois'
        ]
