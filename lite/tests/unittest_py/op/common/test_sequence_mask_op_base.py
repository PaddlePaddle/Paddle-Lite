...
def sample_program_configs(draw):
    ...
    x_shape = draw(st.lists(st.integers(min_value=1, max_value=10), min_size=2, max_size=7)) # [2, 3, 5]
    maxlen = draw(st.integers(min_value=-10, max_value=10))
    out_dtype_dict = {"2" : np.int32,
                      "3" : np.int64,
                      "5" : np.float32}
    ...
    input_data_gen_dict = {
                "x_data":
                TensorConfig(data_gen=partial(generate_input, type=input_type, low=1, high=maxlen, shape=x_shape))}
    if has_max_len_tensor:
        input_dict["MaxLenTensor"] = ["max_len_tensor"]
        input_data_gen_dict["max_len_tensor"] = TensorConfig(data_gen=partial(generate_input, type="int32", low=1, high=2, shape=[maxlen]))
