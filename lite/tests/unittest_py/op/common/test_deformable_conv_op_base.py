...
def sample_program_configs(draw):
    ...
    h_out = (in_shape[2] + 2 * paddings[0] - (dilations[0] * (kh - 1) + 1) // strides[0]) + 1
    w_out = (in_shape[3] + 2 * paddings[1] - (dilations[1] * (kw - 1) + 1) // strides[1]) + 1
    ...
    def generate_offset(*args, **kwargs):
        offset_shape = [in_shape[0], deformable_groups * kw * kh * 2, h_out, w_out]
    ...
    program_config = ProgramConfig(
        ...
        weights={
            ...
            "offset_data":
            TensorConfig(data_gen=partial(generate_offset)),
            "mask_data":
            TensorConfig(data_gen=partial(generate_mask)),
        },
        inputs={
            "input_data":
