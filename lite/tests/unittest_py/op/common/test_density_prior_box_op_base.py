...
def sample_program_configs(draw):
    ...
    input_data_image_shape = draw(st.lists(st.integers(min_value=1, max_value=8), min_size = 4, max_size = 4))
    assume(input_data_x_shape[2] < input_data_image_shape[2])
    assume(input_data_x_shape[3] < input_data_image_shape[3])
    
    # attrs
    variances = draw(st.lists(st.floats(min_value=0.1, max_value=1.0), min_size = 4, max_size = 4))
    ...
    fixed_sizes = draw(st.lists(st.floats(min_value = 0.1, max_value = 128.0), min_size = 1, max_size = 5))
    fixed_ratios = draw(st.lists(st.floats(min_value = 0.1, max_value = 5.0), min_size = 1, max_size = 5))
    densities = draw(st.lists(st.integers(min_value = 1, max_value = 10), min_size = 1, max_size = 5))

    density_prior_box_op = OpConfig(
        type = "density_prior_box",
        inputs = {"Input": ["input_data_x"],
                  "Image": ["input_data_image"]},
        outputs = {"Boxes": ["output_data_boxes"],
