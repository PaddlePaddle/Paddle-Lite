...
class TestAssignOp(AutoScanTest):
    ...
    def sample_program_configs(self, draw):
        ...
        process_type = draw(st.sampled_from(["type_int64", "type_float", "type_int32"]))
        ...
        def generate_data(type, size_list):
            if type == "type_int32":
                return np.random.randint(low=0, high=100, size=size_list).astype(np.int32)
            elif type == "type_int64":
                return np.random.randint(low=0, high=100, size=size_list).astype(np.int64)
        ...
        build_ops = OpConfig(
                ...
                attrs = {"axis" : axis, "force_cpu" : True})
    ...
    def sample_predictor_configs(self):
        return self.get_predictor_configs(), ["less_equal_and_than"], (1e-5, 1e-5)
