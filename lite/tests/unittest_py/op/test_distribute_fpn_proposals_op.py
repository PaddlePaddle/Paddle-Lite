...
class TestDistributeFpnProposalsOp(AutoScanTest):
    ...
    def sample_program_configs(self, draw):
        ...
        def gen_lod_info_data(image_num, max_roi_num_per_image):
            lod_info_data_init_list = [np.random.randint(1, max_roi_num_per_image + 1) for i in range(image_num)]
        ...
        def gen_op_inputs_outputs(*args, **kwargs):
            ...
            inputs_tensor = {'input_data_fpn_rois': TensorConfig(data_gen=partial(gen_input_fpn_rois, dtype=input_fpn_rois_dtype), lod=lod_info_data)}
            ...
            outputs = {'MultiFpnRois': ['output_data_multi_fpn_rois_{}'.format(idx) for idx in range(total_level_num)],
                       'RestoreIndex': ['output_data_restore_index']}
            if has_input_rois_num:
                outputs['MultiLevelRoIsNum'] = [('output_data_multi_level_rois_num_{}_level'.format(idx)) for idx in range(total_level_num)]
        ...
        inputs, inputs_tensor, outputs, outputs_tensor_var_name , outputs_dtype = gen_op_inputs_outputs(multi_fpn_rois_dtype = input_fpn_rois_dtype)
    ...
    def sample_predictor_configs(self):
        config = CxxConfig()
        return self.get_predictor_configs(), ["distribute_fpn_proposals"], (1e-5, 1e-5)
