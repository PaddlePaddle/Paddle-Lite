...
class TestAssignOp(AutoScanTest):
    ...
    def sample_program_configs(self, draw):
        ...
        def generate_weight_int8(*args, **kwargs):
            return np.random.randint(low=0, high=10, size=in_shape).astype(np.int8)
        def generate_bias_int8(*args, **kwargs):
            return np.random.randint(low=0, high=10, size=[1, in_shape[1]]).astype(np.int8)
        def generate_h0_int8(*args, **kwargs):
            return np.random.randint(low=0, high=10, size=[h0_1, in_shape[0]]).astype(np.int8)
        ...
        if process_type == "type_fp32":
            ...
            build_op.outputs_dtype = {"hidden" : np.int8, 
                                       "batch_gate" : np.int8, 
            ...
            program_config = ProgramConfig(
                ...
                inputs={
                    "input_data":
                    TensorConfig(data_gen=partial(generate_input_int8), lod=[[0, 2, 6, 9]]),
                    "bias_data":
                    TensorConfig(data_gen=partial(generate_bias_int8)),
                    "h0":
                    TensorConfig(data_gen=partial(generate_h0_int8)),
                    "scale_w":
