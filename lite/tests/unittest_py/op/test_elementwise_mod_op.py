...
class TestElementwiseModOp(AutoScanTest):
    def __init__(self, *args, **kwargs):
        ...
        self.enable_testing_on_place(
            TargetType.ARM,
            [PrecisionType.FP32, PrecisionType.INT64],
            DataLayoutType.NCHW,
            thread=[1, 4])
        opencl_valid_places = [
    ...
    def sample_program_configs(self, draw):
        ...
        if self.get_target().upper() == 'X86':
            ...
            input_data_type = draw(
                st.sampled_from([np.float32, np.int64]))
    ...
    def sample_predictor_configs(self):
        return self.get_predictor_configs(), ["elementwise_mod"], (1e-5, 1e-5)
