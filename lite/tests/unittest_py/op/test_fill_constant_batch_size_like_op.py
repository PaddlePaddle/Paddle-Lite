...
class TestFillConstantBatchSizeLikeOp(AutoScanTest):
    ...
    def sample_program_configs(self, draw):
        ...
        shape = draw(st.lists(st.integers(min_value=1, max_value=10), min_size=2, max_size=4))
        dtype = draw(st.sampled_from([2, 3, 5]))
        input_dim_idx = draw(st.integers(min_value=0, max_value=(len(in_shape) - 1)))
    ...
    def sample_predictor_configs(self):
        return self.get_predictor_configs(), ["fill_constant_batch_size_like"], (1e-5, 1e-5)
