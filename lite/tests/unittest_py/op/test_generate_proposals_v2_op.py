...
class TestGenerateProposalsOp(AutoScanTest):
    ...
    def sample_program_configs(self, draw):
        ...
        pixel_offset = draw(st.sampled_from([True, False]))
        ...
        def generate_im_shape(*args, **kwargs):
            return np.array([in_shape[2] * stride[0], in_shape[3] * stride[1]]).astype(np.float32)

        generate_proposals_v2_op = OpConfig(
