...
class TestCtcAlignOp(AutoScanTest):
    def __init__(self, *args, **kwargs):
        ...
        self.enable_testing_on_place(TargetType.Host, [PrecisionType.INT32, PrecisionType.INT64], DataLayoutType.NCHW, thread=[1,4])
    ...
    def sample_program_configs(self, draw):
        ...
        def gen_lod_info_data(sequence_num, max_sequence_length):
            lod_data_init_list = [np.random.randint(1, max_sequence_length + 1) for i in range(sequence_num)]
        ...
        if is_input_lod_tensor:
            input_lod_data = gen_lod_info_data(sequence_num, max_sequence_length)
        else:
            input_lod_data = None
