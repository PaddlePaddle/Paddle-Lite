...
class AutoScanTest(unittest.TestCase):
    ...
    def run_lite_config(self, model, params, prog_config, pred_config,
                        feed_data) -> Dict[str, np.ndarray]:
        '''
        Test a single case.
        '''
        pred_config.set_model_buffer(model, len(model), params, len(params))
        predictor = create_paddle_predictor(pred_config)
        for name, _ in prog_config.inputs.items():
            input_tensor = predictor.get_input_by_name(name)
            input_tensor.from_numpy(feed_data[name]['data'])
            if feed_data[name]['lod'] is not None:
                input_tensor.set_lod(feed_data[name]['lod'])
        predictor.run()
        result = {}
        for out_name, o_name in zip(prog_config.outputs,
                                    predictor.get_output_names()):
            result[out_name] = predictor.get_output_by_name(o_name).numpy()
        return result
    ...
    def create_config(self,
                      valid_places: Optional[List[Place]]=None):
        config = CxxConfig()
    ...
    def run_test(self, quant=False, *args, **kwargs):
        ...
        for prog_config in self.sample_program_configs(*args, **kwargs):
            ...
            for pred_config, (
                ...
                try:
                    if not hasattr(self.run_lite_custom_config, "__isabstractmethod__"):
                        results.append(self.run_lite_custom_config(model, params, prog_config, pred_config,
                                                 feed_data))
                    else:
                        results.append(self.run_lite_config(model, params, prog_config, pred_config,
                                                 feed_data))
    ...
    @abc.abstractmethod
    def run_lite_custom_config(self, model, params, prog_config, lite_config,
                        feed_data) -> Dict[str, np.ndarray]:
        raise NotImplementedError
