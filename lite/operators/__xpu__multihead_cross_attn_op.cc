...
#include "lite/operators/__xpu__multihead_cross_attn_op.h"
#include <vector>
...
bool XPUMhcaOp::CheckShape() const {
  CHECK_EQ(param_.input->dims().size(), 3UL);
  return true;
}

bool XPUMhcaOp::InferShapeImpl() const {
  param_.output->Resize(param_.input->dims());
  return true;
}

bool XPUMhcaOp::AttachImpl(const cpp::OpDesc& op_desc, lite::Scope* scope) {
...
REGISTER_LITE_OP(__xpu__multihead_cross_attn,
                 paddle::lite::operators::XPUMhcaOp);
