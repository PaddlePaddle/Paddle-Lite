...
//    param_ = *static_cast<FpgaConvParam*>(op_param_);
//    AttachParam(&param_);
  ...
  void AttachKernel(KernelBase* kernel) override {
      ConvOpLite::AttachKernel(kernel);
      static_cast<kernels::fpga::ConvCompute*>(kernel)->SetStrideInfo(stride_info_);
