...
#include "lite/operators/conv3d_op.h"
#include <algorithm>
#include <vector>
...
bool Conv3DOpLite::CheckShape() const {
  ...
  CHECK_EQ_OR_FALSE(filter_dims.size(), 5UL);
...
bool Conv3DOpLite::InferShapeImpl() const {
  const auto in_dims = param_.x->dims();
  const auto filter_dims = param_.filter->dims();
...
REGISTER_LITE_OP(conv3d, paddle::lite::operators::Conv3DOpLite);
