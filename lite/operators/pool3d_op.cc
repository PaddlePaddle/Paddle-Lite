...
#include "lite/operators/pool3d_op.h"
#include <algorithm>
...
bool Pool3DOpLite::CheckShape() const {
  ...
  // "Pooling intput should be 5-D tensor."
  CHECK_OR_FALSE(x_dims.size() == 5);
  // Input size and pooling size should be consistent.
  CHECK_OR_FALSE(x_dims.size() - ksize.size() == 2U);
  // Strides size and pooling size should be the same.
  CHECK_OR_FALSE(ksize.size() == strides.size());
  // Paddings size must be 6.
  CHECK_OR_FALSE(paddings.size() == 6L);

  return true;
}

int Pool3DOutputSize(int input_size,
                     int filter_size,
                     int pad_left,
                     int pad_right,
                     int stride,
...
bool Pool3DOpLite::InferShapeImpl() const {
  const auto x_dims = param_.x->dims();
  std::vector<int>& ksize = param_.ksize;
  // dynamic update 6-pad
  ...
  if (param_.adaptive) {
    ...
    for (size_t i = 0; i < param_.ksize.size(); ++i) {
      output_shape.push_back(Pool3DOutputSize(x_dims[i + 2],
                                              param_.ksize[i],
                                              paddings[2 * i],
                                              paddings[2 * i + 1],
                                              param_.strides[i],
...
REGISTER_LITE_OP(pool3d, paddle::lite::operators::Pool3DOpLite);
