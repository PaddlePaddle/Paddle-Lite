...
static std::pair<int, int> SelectTopBeamSizeItems(
      ...
      const Tensor *pre_scores, const Tensor *ids,
      const Tensor *scores,
      ...
      int decode_length,
      float alpha,
      std::vector<std::vector<Item>> &alive_result,
      std::vector<std::vector<Item>> &finish_result) {
    ...
    auto *ids_data = ids ? ids->data<int>() : nullptr;
    auto *scores_data = scores->data<float>();
    auto cur_len = cur_length->data<int>()[0];
    ...
    for (size_t seq_id = 0; seq_id < num_seqs; ++seq_id) {
      ...
      for (int i = lod_in_finsh[level][seq_id_in_finish - 1];
           i < lod_in_finsh[level][seq_id_in_finish]; i++) {
      ...
      if (cur_len == max_len ||
        ...
        finish_top_beam.insert(finish_top_beam.end(), alive_top_beam.begin(),
                               alive_top_beam.end());
    ...
    return std::make_pair(alive_num, finial_num);
  }
...
void beam_search_special(const Tensor *pre_ids,
    ...
    auto num_instances =
        SelectTopBeamSizeItems(pre_ids, pre_scores, ids, scores, src_length,
                               cur_length, level, beam_size, end_id, 
                               is_accumulated,decode_length, alpha,
                               alive_result, finish_result);
    ...
    auto *selected_ids_data = selected_ids->mutable_data<int>();
    auto *selected_scores_data = selected_scores->mutable_data<float>();
    auto *finish_ids_data = finish_ids->mutable_data<int>();
