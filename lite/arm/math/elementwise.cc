...
template <>
void elementwise_add_relu_scale<float>(
    const float* dinx, const float* diny, float* dout, int num, float scale) {
  int cnt = num >> 4;
  int remain = num % 16;
  float32x4_t vzero = vdupq_n_f32(0.f);
...
#pragma omp parallel for
  for (int i = 0; i < cnt; i++) {
    ...
    // scale
    dinx0 = vmulq_f32(dinx0, vscale);
    dinx1 = vmulq_f32(dinx1, vscale);
    dinx2 = vmulq_f32(dinx2, vscale);
    dinx3 = vmulq_f32(dinx3, vscale);
  ...
  if (remain > 0) {
    ...
    for (int i = 0; i < remain; i++) {
      float tmp = *dinx_ptr + *diny_ptr;
      *dout_ptr = tmp > 0.f ? tmp * scale : 0.f;
      dout_ptr++;
      dinx_ptr++;
      diny_ptr++;
    }
...
template <>
void elementwise_add_relu_scale_broadcast<float>(const float* dinx,
                                                 const float* diny,
                                                 float* dout,
                                                 int batch,
                                                 int channels,
  ...
  for (int i = 0; i < batch; ++i) {
    for (int j = 0; j < channels; ++j) {
      ...
      for (int k = 0; k < cnt; ++k) {
        ...
        // scale
        din0 = vmulq_f32(din0, vscale);
        din1 = vmulq_f32(din1, vscale);
        din2 = vmulq_f32(din2, vscale);
        din3 = vmulq_f32(din3, vscale);
