...
do
  ./${opt_name} --model_dir=./models/$name --valid_targets=arm --optimize_out=./models_opt/$name --record_tailoring_info=true
done
...
cd $workspace
./lite/tools/build_android.sh --with_strip=ON --opt_model_dir=$workspace/model_info --with_log=$WITH_LOG --with_cv=$WITH_CV --toolchain=$TOOL_CHAIN --with_exception=$WITH_EXCEPTION --android_stl=$ANDROID_STL

./lite/tools/build_android.sh --with_strip=ON --opt_model_dir=$workspace/model_info --with_log=$WITH_LOG --arch=armv7 --with_cv=$WITH_CV --toolchain=$TOOL_CHAIN --with_exception=$WITH_EXCEPTION --android_stl=$ANDROID_STL
...
cp ./${opt_name} $result_name/
mv ./optimized_model $result_name
