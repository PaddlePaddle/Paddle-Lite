...
ANDROID_STL="c++_shared"            # c++_shared/c++_static, c++_shared is used by MTK APU adapter
DDK_ROOT="$(pwd)/neuronadapter/"    # MTK APU adapter
TARGET_NAME="test_subgraph_pass"    # default target
BUILD_EXTRA=OFF                     # ON(with sequence ops)/OFF
WITH_JAVA=OFF                       # ON(build jar and jni so)/OFF
...
function print_usage {
    ...
    echo -e "--arm_lang=<gcc>"
    echo -e "--ddk_root=<mtk_apu_adapter_root>"
...
function build_apu {
    ...
    build_dir=$cur_dir/build.lite.apu.${ARM_OS}.${ARM_ABI}.${ARM_LANG}.${stl_dir}.${publish_dir}
    mkdir -p $build_dir
    cd $build_dir

    # APU libs need API LEVEL 24 above
    prepare_workspace
    ${CMAKE_TOOL} .. \
        -DWITH_GPU=OFF \
        -DWITH_MKL=OFF \
